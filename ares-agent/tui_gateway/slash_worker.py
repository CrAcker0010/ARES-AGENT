"""Persistent slash-command worker — one AresCLI per TUI session.
...
_WATCHDOG_POLL_S = float(os.environ.get("ARES_SLASH_WATCHDOG_POLL_S", 2.0))
_ORPHAN_GRACE_S = float(os.environ.get("ARES_SLASH_WATCHDOG_GRACE_S", 5.0))
...
def _run(cli: AresCLI, command: str) -> str:
...
def main():
    ...
    os.environ["ARES_SESSION_KEY"] = args.session_key
    os.environ["ARES_INTERACTIVE"] = "1"

    # Start before the (hundreds-of-ms) AresCLI build — that window is itself
    ...
    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = AresCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
