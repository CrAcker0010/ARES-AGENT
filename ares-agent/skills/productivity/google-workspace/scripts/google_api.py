#!/usr/bin/env python3
"""Google Workspace API CLI for Ares Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Ares-facing JSON contract and falls back to the Python client
...
from _ares_home import get_ares_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ARES_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Ares Agent")
