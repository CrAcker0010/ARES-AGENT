...
## 5. GODMODE FAST — Ares 4 405B (Zero Refusal)

**Model:** `nousresearch/ares-4-405b`
**Strategy:** Ares models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Ares Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.ares/config.yaml`:
