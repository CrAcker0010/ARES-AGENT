...
## 5. Ares Configuration

### Config location

`$ARES_HOME/config.yaml` (defaults to `~/.ares/config.yaml` when `ARES_HOME` is unset)
...
Restart the Ares session for changes to take effect. The MCP connection is
