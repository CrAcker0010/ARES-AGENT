"""``ares dashboard register`` — register a self-hosted dashboard OAuth client.
...
resulting ``agent:{id}`` OAuth client ID, and paste it into ``~/.ares/.env``
as ``ARES_DASHBOARD_OAUTH_CLIENT_ID``.

This command:
  1. Resolves a fresh Nous Portal access token from the existing login
     (``~/.ares/auth.json``), refreshing it if needed. Fails fast with a
     "run `ares setup`" hint when the user isn't logged in.
  ...
  3. Writes ``ARES_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``ARES_DASHBOARD_PORTAL_URL`` into ``~/.ares/.env`` idempotently.
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    Precedence:
      1. ``override`` — explicit ``--portal-url`` flag or
         ``ARES_DASHBOARD_PORTAL_URL`` env (used for testing against a
    ...
    try:
        from ares_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "Try `ares auth login nous` to re-authenticate."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def _print_post_register_hint(
    *,
    ...
    from ares_cli.config import get_env_path

    env_path = get_env_path()
    print()
    print(f"  Wrote to {env_path}:")
    print(f"    ARES_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
    if wrote_portal_url:
        print(f"    ARES_DASHBOARD_PORTAL_URL={portal_base_url}")
    print()
    print(
        "  Heads up — Nous login only *engages* on a non-loopback bind. A plain\n"
        "  `ares dashboard` (localhost) leaves the gate off and serves locally\n"
    ...
    if custom_redirect_uri:
        ...
        print("    ares dashboard --host 0.0.0.0")
...
def cmd_dashboard_register(args) -> None:
    ...
    from ares_cli.auth import AuthError, resolve_nous_access_token
    from ares_cli.config import get_env_value, is_managed, save_env_value

    # Managed (Docker/hosted) installs get their dashboard OAuth client_id
    # stamped in by the orchestrator (NAS sets ARES_DASHBOARD_OAUTH_CLIENT_ID
    ...
    if is_managed():
        print(
            "✗ `ares dashboard register` is not available in a managed/hosted "
            "install.\n"
    ...
    try:
        ...
        if getattr(exc, "relogin_required", False):
            ...
            print("  Run `ares setup` (or `ares auth login nous`) first, then retry.")
        else:
    ...
    # ARES_DASHBOARD_PORTAL_URL env var, else the stored login's portal.
    ...
    try:
        save_env_value("ARES_DASHBOARD_OAUTH_CLIENT_ID", client_id)
    except Exception as exc:
        print(f"✗ Failed to write ARES_DASHBOARD_OAUTH_CLIENT_ID to .env: {exc}")
        print(f"  Set it manually:  ARES_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
        sys.exit(1)
    ...
    try:
        existing_portal = get_env_value("ARES_DASHBOARD_PORTAL_URL")
    except Exception:
        existing_portal = None
    if not existing_portal and portal_base_url.rstrip("/") != default_portal:
        try:
            save_env_value("ARES_DASHBOARD_PORTAL_URL", portal_base_url)
