...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``ares -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``ares profile alias
...
def create_profile(
    ...
    # `ares -p <profile> gateway start` can supervise it via
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from ares_cli.service_manager import get_service_manager
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"ares_{old_name}"
    legacy_old_host = f"ares.{old_name}"
    new_host = f"ares_{new_name}"
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("ares_"):
