...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    patterns = [
        ...
        # ``ares.exe --profile`` / ``ares.exe -p`` would also match
        # ``ares.exe --profile foo dashboard`` and other CLI subcommands,
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"ares_home={current_home_lc}" in command_lc
            )
        ...
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "ares_home=" in command_lc
            and f"ares_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    from ares_cli._subprocess_compat import (
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}ares gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from ares_cli.service_manager import detect_service_manager
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Ares Docker image — that one runs s6-overlay as
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so Ares can't create it from this user session."
            )
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo ares gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo ares gateway install --system --run-as-user <your-user>"
                )
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m ares_cli.main``, so module
    ...
    dir, or a clone that ``ares update`` later relocates/removes — the path
    ...
    ``ARES_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if ARES_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        # Anchor cwd to the target user's ARES_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(ares_home) if ares_home else _remap_path_for_user(working_dir, home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m ares_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Ares install"
    )
    return True
...
def systemd_install(
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}ares gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}ares gateway status{scope_flag}             # Check status"
    )
    print(
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}ares gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}ares gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}ares gateway start{scope_flag}"
        )
...
# (the documented `nohup ares gateway run` workaround). See #23387.
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m ares_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active ARES_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "ares_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup ares gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    if _spawn_detached_gateway():
        ...
        print("  Stop it with: ares gateway stop")
        return True
    print_error("Failed to start the gateway as a background process.")
    print(
        f"  Try manually: nohup ares gateway run --replace "
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Ares install"
    )
    return True
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print(
        "  Set ARES_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from ares_cli.telegram_managed_bot import (
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: ares pairing approve <platform> <code>"
                    )
                else:
                    print_info(
                        "  Skipped — configure later with 'ares gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    print_info(
        "  3. Ares will store the returned account_id/token in ~/.ares/.env."
    )
    print_info(
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `ares pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `ares pairing approve`."
        )
    elif access_idx == 1:
        ...
        if service_running:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    ...
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo ares gateway install --system"
                        )
                ...
                print_info(
                    "  For persistence:   tmux new -s ares 'ares gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: ares gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup ares gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`ares_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from ares_cli.service_manager import (
    ...
    if profile is None:
        ...
        # ARES_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``ares gateway stop --all`` and ``... restart --all``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``ares gateway run`` are unaffected.
      2. ``ARES_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``ares gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``ARES_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("ARES_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("ARES_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${ARES_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if ARES_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or ARES_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-ares service's
    # pattern in docker/s6-rc.d/main-ares/run, and frees the Python
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            if is_wsl():
                ...
                print_info(
                    "  Consider running in foreground instead: ares gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s ares 'ares gateway run'"
                )
                print()
            ...
            print()
            print(
                "  ares gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s ares 'ares gateway run'         # persistent via tmux"
            )
            print(
                "  nohup ares gateway run > ~/.ares/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  ares profile create <name>     # creates the s6 service slot")
                print("  ares -p <name> gateway start   # bring it up via s6")
                print("  ares status                    # see currently-supervised gateways")
                return
        ...
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  ares profile delete <name>     # tears down the s6 service slot")
                print("  ares -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `ares -p <name> gateway
        ...
        if os.getenv("_ARES_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `ares gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_ARES_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `ares gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    print(
                        "  nohup ares gateway run > ~/.ares/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s ares 'ares gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  ares gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  ares gateway install  # Install as user service")
                    print(
                        "  sudo ares gateway install --system  # Install as boot-time system service"
                    )
