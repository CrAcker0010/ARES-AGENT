...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches ares_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``ares --version`` reporting a stale version while
    ...
    """
    try:
        from ares_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != ares_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'ares update', or set "
            "ares_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-ares and dashboard static services are up
    ...
    for static in ("main-ares", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `ares profile create <name>`")
        return
...
def run_doctor(args):
    ...
    # Detect drift between pyproject.toml and ares_cli/__init__.py versions
    ...
    if config_path.exists():
        ...
        # Detect stale ARES_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives ARES_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            import yaml
            from ares_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
            ...
            env_ghost = load_env().get("ARES_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"ARES_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `ares setup` run)",
                )
                if should_fix:
                    if remove_env_value("ARES_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale ARES_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove ARES_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the ARES_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale ARES_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'ares doctor --fix'"
                    )
        except Exception:
            pass
    ...
    try:
        from ares_constants import is_container as _is_container
