...
def switch_model(
    ...
    if explicit_provider:
        ...
        from ares_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from ares_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from ares_cli.providers import resolve_user_provider as _ruser
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from ares_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    for ares_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(ares_id)
        if (
            _alias_target
            and _alias_target != ares_id
        ...
        if not has_creds:
            try:
                ...
                if store and store.get("credential_pool", {}).get(ares_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `ares model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(ares_id)
        if not model_ids:
    ...
    for pid, overlay in ARES_OVERLAYS.items():
        ...
        if ares_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(ares_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(ares_slug)
                model_ids = _ids if _ids else (curated.get(ares_slug, []) or curated.get(pid, []))
            except Exception:
            ...
            # `ares model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from ares_cli.auth import get_provider_auth_state as _nous_state
