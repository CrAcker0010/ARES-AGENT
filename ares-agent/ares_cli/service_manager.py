...
the existing module-level functions in ares_cli.gateway and
ares_cli.gateway_windows directly. This protocol is a thin facade
...
``ares gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``ares gateway start/stop/restart``).
    """
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged ares
    ...
    detection always fails. Since every Ares runtime call inside the
    container drops to ares via ``s6-setuidgid``, that silent failure
...
# in ``ares_cli.gateway`` (systemd/launchd) and ``ares_cli.gateway_windows``
...
# active (set via the ``ares -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in ares_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from ares_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from ares_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from ares_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from ares_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in ares_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from ares_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from ares_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from ares_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from ares_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``ares_cli.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `ares profile create`
# runs inside the container (Phase 4). Static services (main-ares, dashboard)
...
# UID/GID of the in-image ``ares`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the ares user.
    ...
    (the ``ares`` user — UID 10000 — running every Ares runtime
    ...
    is dispatched as the ares user.
    ...
    with ares ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           ares:ares, 0755 (parent must already exist)
    ``svc_dir/event/``                     ares:ares, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 ares:ares, 0755
    ``svc_dir/supervise/event/``           ares:ares, 03730
    ``svc_dir/supervise/control``          ares:ares, 0660    (FIFO)
    ...
    slot's supervise/ was ares-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _ARES_UID, _ARES_GID)
        except PermissionError:
            # Running as the ares user already — directory is ares-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _ARES_UID, _ARES_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is ares-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _ARES_UID, _ARES_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``ares -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`ares profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-ares, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources ARES_HOME (and any extra env) via with-contenv —
             so e.g. ``-e ARES_HOME=/data/ares`` is honored at run
          ...
          4. Drops to the ares user and exec's
             ``ares -p <profile> gateway run`` (or just ``ares
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``ares gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root ARES_HOME profile" (the implicit profile that exists at
        the top of $ARES_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``ares_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$ARES_HOME/profiles/default/`` — a
        ...
        ``ares_cli.profiles._allocate_gateway_port``). PR #30136
        ...
        # guard.
        lines.append("export ARES_S6_SUPERVISED_CHILD=1")
        if profile == "default":
            gateway_cmd = "ares gateway run"
        else:
            gateway_cmd = f"ares -p {shlex.quote(profile)} gateway run"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid ares {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${ARES_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the ARES_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e ARES_HOME=/data/ares``
        gets its logs under /data/ares/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``ares logs`` reads and what
        ...
        return (
            ...
            f': "${{ARES_HOME:=/opt/data}}"\n'
            f'log_dir="$ARES_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            f'chown -R ares:ares "$log_dir" 2>/dev/null || true\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid ares s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with ares ownership
            ...
            # the ares user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is ares-owned (see ``_seed_supervise_skeleton``), and on
