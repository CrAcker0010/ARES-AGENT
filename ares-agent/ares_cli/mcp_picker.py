"""MCP picker — interactive `ares mcp picker` (also the default `ares mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``ares mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `ares plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from ares_cli.cli_output import prompt_yes_no
from ares_cli.curses_ui import curses_single_select
from ares_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Ares session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from ares_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `ares mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: ares mcp install <name>    Picker: ares mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Ares is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Ares — run `ares update` "
...
def show_catalog() -> None:
    """`ares mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`ares mcp picker` (and default `ares mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`ares mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `ares mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
