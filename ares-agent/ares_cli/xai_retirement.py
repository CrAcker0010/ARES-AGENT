...
Pure logic: walks a Ares config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `ares doctor` and a future `ares migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Ares config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Ares config and return retirement issues.
