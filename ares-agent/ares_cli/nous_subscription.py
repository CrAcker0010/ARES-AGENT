...
# coverage category (ares_cli.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `ares tools` picker scope its entitlement gate to the selected backend, so a
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from ares_cli.setup import prompt_checklist
    except Exception:
        return set()
...
# ---------------------------------------------------------------------------
# Inline Nous Portal login for the Tool Gateway picker (`ares tools`)
# ---------------------------------------------------------------------------
...
def ensure_nous_portal_access(
    *,
    ...
    Used by ``ares tools`` when a user selects a Nous-managed Tool Gateway
    backend (e.g. "Firecrawl (Nous Portal)").  Unlike ``ares model``'s Nous
    login, this:
