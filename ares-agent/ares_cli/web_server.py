...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``ares gateway run`` — but the
    desktop app spawns a ``ares dashboard`` backend, not a gateway, so a cron
    ...
    ARES_HOME — whichever process grabs the lock first wins the tick.
    """
...
async def _lifespan(app: "FastAPI"):
    ...
    # Desktop-spawned backends (ARES_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `ares
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("ARES_DESKTOP") == "1":
...
app = FastAPI(title="Ares Agent", version=__version__, lifespan=_lifespan)

# ---------------------------------------------------------------------------
# Session token for protecting sensitive endpoints (reveal).
# The desktop shell mints the token and injects it via
# ARES_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("ARES_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
...
#
# This list is defined in ``ares_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from ares_cli.dashboard_auth.public_paths import (
...
async def _dashboard_auth_gate(request: Request, call_next):
    from ares_cli.dashboard_auth.middleware import gated_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Ares (no terminal prompt), "
...
async def get_status():
    ...
    # and which providers are registered so ``ares status`` and the
    # SPA's StatusPage can show "OAuth gate ON via Nous Research" or
    # "loopback only — no auth gate" with no extra round trips.
    auth_required = bool(getattr(app.state, "auth_required", False))
    auth_providers: list[str] = []
    try:
        from ares_cli.dashboard_auth import list_providers as _list_providers
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the ares home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_ares_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_ares_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def run_prompt_size():
    try:
        proc = _spawn_ares_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_ares_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_ares_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from ares_cli.debug import build_debug_share
...
async def update_ares():
    ...
    if install_method == "docker":
        ...
        _record_completed_action("ares-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "ares-update",
...
@app.get("/api/ares/update/check")
async def check_ares_update(force: bool = False):
    """Report whether a Ares update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/ares/update`` actually runs ``ares update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Ares version string
    ...
    try:
        ...
        if force:
            try:
                (get_ares_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.ares/config.yaml`` under ``tts.``.
    """
...
async def search_sessions(q: str = "", limit: int = 20):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Ares surface. FTS can't find those unless the
...
def get_model_options():
    ...
    try:
        ...
        # onboarding picker) sees the SAME full provider universe `ares model`
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `ares model` does so GUI onboarding lands on a
...
async def set_model_assignment(body: ModelAssignment):
    ...
    try:
        ...
        if scope == "main":
            ...
            # post-model-selection behaviour (ares_cli/main.py
            ...
            if provider.strip().lower() == "nous":
                try:
                    from ares_cli.nous_subscription import apply_nous_managed_defaults
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Ares from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Ares to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Ares from Slack via Socket Mode.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Ares to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Ares in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Ares through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Ares via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Ares through an IMAP/SMTP mailbox.",
        "docs_url": "https://ares-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Ares to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Ares inside Feishu / Lark.",
    ...
    "bluebubbles": {
        ...
        "description": "Use Ares through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Ares to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Ares to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Ares as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://ares-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://ares-agent.nousresearch.com/docs/user-guide/messaging/webhooks/",
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.ares-agent.nousresearch.com"
_TELEGRAM_ONBOARDING_USER_AGENT = f"AresDashboard/{__version__}"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Ares Agent").strip() or "Ares Agent"
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "flow": "loopback",
        "cli_command": "ares auth add xai-oauth",
        "docs_url": "https://ares-agent.nousresearch.com/docs/guides/xai-grok-oauth",
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    tmp_path = _ARES_OAUTH_FILE.with_name(
        f"{_ARES_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _ARES_OAUTH_FILE)
        try:
            _ARES_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
# the tokens exactly like `ares auth add xai-oauth`.
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `ares auth add xai-oauth`'s credential-pool insert.
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from ares_cli.auth import _save_codex_tokens
...
async def get_session_stats():
    """Session-store statistics for the Sessions page (mirrors `ares sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `ares sessions prune`)."""
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, ARES_HOME)."""
    from ares_cli import profiles as profiles_mod
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process ARES_HOME at import time. The dashboard is a single
...
#
# Wraps the same config data layer the CLI uses (ares_cli.mcp_config), so
# servers managed here show up under `ares mcp list` and vice versa.  Secrets
...
async def list_mcp_servers():
    from ares_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate):
    from ares_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def test_mcp_server(name: str):
    ...
    from ares_cli.mcp_config import _get_mcp_servers, _probe_single_server
...
async def list_mcp_catalog():
    ...
    `ares mcp catalog` / `ares mcp install` read.
    """
    try:
        from ares_cli import mcp_catalog
    except Exception as exc:
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall):
    ...
    if entry.install is not None:
        try:
            proc = _spawn_ares_action(["mcp", "install", name], "mcp-install")
        except Exception as exc:
...
#
# Wraps the same JSON store the CLI uses (ares_cli.webhook); the webhook
...
# spawn the real `ares gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway():
    try:
        proc = _spawn_ares_action(["gateway", "start"], "gateway-start")
    except Exception as exc:
...
async def stop_gateway():
    try:
        proc = _spawn_ares_action(["gateway", "stop"], "gateway-stop")
    except Exception as exc:
...
# `ares memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `ares memory setup` to configure a new one.",
            )
...
async def run_doctor():
    try:
        proc = _spawn_ares_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_ares_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_ares_action(args, "backup")
    except Exception as exc:
...
async def run_import(body: ImportRequest):
    ...
    try:
        proc = _spawn_ares_action(["import", archive], "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    form can offer them.
    """
    from ares_cli.config import load_config as _load_config
...
async def list_checkpoints():
    ...
    # Checkpoints live under <ares_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_ares_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_ares_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
async def install_skill_hub(body: SkillInstallRequest):
    ...
    try:
        proc = _spawn_ares_action(["skills", "install", identifier], "skills-install")
    except Exception as exc:
...
async def uninstall_skill_hub(body: SkillUninstallRequest):
    ...
    try:
        proc = _spawn_ares_action(["skills", "uninstall", name, "--yes"], "skills-uninstall")
    except Exception as exc:
...
async def update_skills_hub():
    try:
        proc = _spawn_ares_action(["skills", "update"], "skills-update")
    except Exception as exc:
...
async def preview_skill_hub(identifier: str = ""):
    ...
    def _run():
        from ares_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local ARES_HOME override so the write lands in the target
    ...
    """
    from ares_constants import set_ares_home_override, reset_ares_home_override

    token = set_ares_home_override(str(profile_dir))
    try:
        cfg = load_config()
        cfg["model"] = _apply_main_model_assignment(cfg.get("model", {}), provider, model)
        save_config(cfg)
    finally:
        reset_ares_home_override(token)
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``ares profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from ARES_HOME).
    """
    from ares_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``ares profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the ARES_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``ares profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from ares_cli import profile_describer
...
async def toggle_toolset(name: str, body: ToolsetToggle):
    ...
    helper the CLI ``ares tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str):
    ...
    Surfaces the same provider rows the CLI ``ares tools`` picker shows
...
async def select_toolset_provider(name: str, body: ToolsetProviderSelect):
    ...
    core extracted from the CLI configurator — so the GUI and ``ares tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate):
    ...
    Writes each ``key: value`` to ``~/.ares/.env`` via ``save_env_value`` —
    the same store ``ares tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(name: str, body: ToolsetPostSetup):
    ...
    ``ares tools post-setup <key>`` and the frontend tails the log via
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from ares_cli.dashboard_auth.audit import AuditEvent, audit_log
        from ares_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Session resume is propagated via the ``ARES_TUI_RESUME`` env var —
    matching what ``ares_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``ARES_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``ARES_TUI_SIDECAR_URL`` so
    ...
    env.setdefault("ARES_TUI_INLINE", "1")
    ...
    if gateway_ws_url := _build_gateway_ws_url():
        env["ARES_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from ares_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`ares_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from ares_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``__ARES_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                ...
                f"window.__ARES_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
...
def _discover_dashboard_plugins() -> list:
    ...
    # ``ares_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("ARES_ENABLE_PROJECT_PLUGINS"):
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.ares/plugins/``) ship with the CWD and are
...
from ares_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        from ares_cli.dashboard_auth import list_providers
        if not list_providers():
            # Surface the *specific* reason any bundled provider declined
            # to register (e.g. missing ARES_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Ares Agent exposes a
