...
def remove_node_symlinks(ares_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``ares`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Ares home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (ares_home / "node").resolve()
    removed = []
...
def run_gui_uninstall(args):
    ...
    Mirrors ``ares uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$ARES_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from ares_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(ares_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Ares Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Ares Chat GUI installation was found.")
        print(f"  Checked: {ares_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Ares agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(ares_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Ares agent at {ares_home / 'ares-agent'}")
        print(f"  • Your config, sessions, and secrets under {ares_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Ares agent is still installed. Run 'ares' to use the CLI,")
    print("or 'ares uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    # full wipe (code + ~/.ares data); otherwise keep-data. Named profiles
...
def _perform_uninstall(
    *,
    ...
    ``ares`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$ARES_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    #     (only when they still point into this Ares home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Ares-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(ares_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Ares-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(ares_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE ARES_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from ares_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(ares_home)
        if not gui_removed:
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://ares-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://ares-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m ares_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import ares_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``ares_constants`` + ``ares_cli.colors``
    (and lazily ``ares_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m ares_cli.uninstall")
    parser.add_argument(
        "--mode",
