...
def _resolve_short_name(name: str, sources, console: Console) -> str:
    ...
    if len(exact) > 1:
        ...
        # so users can copy it for `ares skills install`.
...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    # skips the external API sources and serves everything from "ares-index".
    ...
    # The identifier is what you pass to `ares skills install`. Browse used
    ...
    c.print("[dim]Tip: 'ares skills inspect <identifier>' to preview, "
            "'ares skills install <identifier>' to install, "
            "'ares skills search <query>' to search deeper[/]\n")
...
def browse_skills(page: int = 1, page_size: int = 20, source: str = "all") -> dict:
    ...
    page_size = max(1, min(page_size, 100))
    _TRUST_RANK = {"builtin": 3, "trusted": 2, "community": 1}
    # "ares-index" must carry a high limit: when the index is available the
    ...
    _PER_SOURCE_LIMIT = {"ares-index": 5000, "official": 100, "skills-sh": 100,
    ...
    # source-skip logic — querying ares-index AND the external APIs at once
...
def skills_command(args) -> None:
    ...
    if action == "browse":
        ...
        _console.print("Usage: ares skills [browse|search|install|inspect|list|check|update|audit|uninstall|reset|opt-out|opt-in|publish|snapshot|tap]\n")
