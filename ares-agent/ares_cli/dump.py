...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.ares_build_sha`` by the Dockerfile's
    ``ARES_GIT_SHA`` build-arg (see ``ares_cli/build_info.py``).
...
def run_dump(args):
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `ares auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `ares auth list` shows it (#42130).
