...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Ares profile."""
    from ares_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "ares_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("ARES_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("ARES_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("ARES_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["ARES_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["ARES_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["ARES_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("ARES_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("ARES_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("ARES_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``ARES_HOME`` whenever it
    ...
    source checkout only if ``ARES_HOME`` cannot be resolved yet.
    """
    from ares_cli.config import get_ares_home
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - invokes ``pythonw -m ares_cli.main [--profile X] gateway run``
    ...
    prog_args = [pythonw_path, "-m", "ares_cli.main"]
    if profile_arg:
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Ares builds and failed
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m ares_cli.main gateway run``
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("ARES_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("ARES_GATEWAY_INSTALL_START_ON_LOGIN")
    ...
    from ares_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `ares -p <profile> gateway install` must be safe.
    ...
    from ares_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"ares {profile_arg} gateway start" if profile_arg else "ares gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: ares gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Ares gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: ares gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: ares gateway start")
        _print_next_steps()
        return
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Ares gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: ares gateway status")
                    return
...
def _print_deep_probes() -> None:
    ...
    home = Path(get_ares_home()).resolve()
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: ares gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: ares gateway start")
            return
...
def stop() -> None:
    ...
    """
    _assert_windows()
    from ares_cli.gateway import kill_gateway_processes, _get_restart_drain_timeout
    from gateway.status import get_running_pid
