...
def _resolve_plugin_key(name: str) -> Optional[str]:
    ...
    This is the single normalization point so ``ares plugins enable`` /
...
def _filter_plugin_entries(entries: list, args: Any, enabled: set, disabled: set) -> list:
    """Apply ``ares plugins list`` CLI filters."""
    filtered = entries
...
def cmd_list(args: Any | None = None) -> None:
    ...
    console.print(table)
    console.print()
    console.print("[dim]Compact view:[/dim] ares plugins list --plain --no-bundled")
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    plugin-registered engines (third-party engines installed as Ares
    ...
    try:
        from ares_cli.plugins import discover_plugins, get_plugin_context_engine
        discover_plugins()
