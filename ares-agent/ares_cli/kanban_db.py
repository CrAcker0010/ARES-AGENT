...
# ``ARES_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``ARES_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("ARES_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``ARES_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("ARES_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``ARES_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "ARES_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "ares_kanban_current_board_override",
    default=None,
)
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``ARES_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("ARES_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
class Task:
    ...
    # within an agent loop that propagated ``ARES_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``ARES_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("ARES_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Ares treats as user-controlled-but-trusted on the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``ARES_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
# Canonical sort-order mappings for ``ares kanban list --sort``.
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``ares kanban block <id>``).  This is a deliberate handoff that
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``ARES_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Ares' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Ares-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``ares kanban show <id>`` and the dashboard.
...
# without thrashing. Overridable via ``ARES_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
def dispatch_once(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from ares_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `ares kanban tail` — without
...
def _module_ares_argv() -> list[str]:
    """Return the interpreter-bound Ares CLI invocation."""
    # ``ares_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``ares`` package — there is no
    # ``ares`` package to import.
    return [sys.executable, "-m", "ares_cli.main"]


def _absolute_ares_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Ares shim."""
...
def _ares_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Ares executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_ares_argv()
    return [_absolute_ares_path(path)]
...
def _resolve_ares_argv() -> list[str]:
    ...
    1. ``$ARES_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("ares")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\ares.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m ares_cli.main`` — fallback for setups where
    ...
    env_bin = os.environ.get("ARES_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _ares_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _ares_path_argv(resolved_env_bin)
        return _module_ares_argv()

    ares_bin = _safe_which_no_cwd("ares") if _IS_WINDOWS else shutil.which("ares")
    if ares_bin:
        return _ares_path_argv(ares_bin)
    return _module_ares_argv()


def _kanban_worker_skill_available(ares_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``ares -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset ARES_HOME means the worker falls back to the default root
    # home (``~/.ares``), which ships the bundled skill.
    base = _Path(ares_home) if ares_home else (_Path.home() / ".ares")
...
def _default_spawn(
    ...
    *,
    ...
    if task.branch_name:
        env["ARES_KANBAN_BRANCH"] = task.branch_name
    ...
    if task.goal_mode:
        env["ARES_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["ARES_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    cmd = [
        *_resolve_ares_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped ARES_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("ARES_HOME")):
