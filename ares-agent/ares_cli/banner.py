...
def check_for_updates() -> Optional[int]:
    ...
    # ARES_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `ares update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/ares/update/check`
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``ares_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from ares_cli.build_info import get_build_sha
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    # Pip-install warning — `pip install ares-agent` is not the supported
