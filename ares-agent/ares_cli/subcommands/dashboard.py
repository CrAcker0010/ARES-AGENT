"""``ares dashboard`` subcommand parser.

Extracted verbatim from ``ares_cli/main.py:main()`` (god-file Phase 2).
...
def build_dashboard_parser(
    ...
    # Backward-compat shim: older Ares desktop app shells (<= 0.15.x) spawn the
    # backend as `ares dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Ares couldn't start" with no actionable cause. Accept and silently
    ...
    # `ares dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.ares/.env.
    # Nested subparser so bare `ares dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "ARES_DASHBOARD_OAUTH_CLIENT_ID into ~/.ares/.env, and prints "
            "how to engage the login gate. Requires being logged in (ares setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://ares.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via ARES_DASHBOARD_PORTAL_URL. Mainly for "
