"""``ares gateway`` and ``ares proxy`` subcommand parsers.
...
from ares_cli.subcommands._shared import add_accept_hooks_flag
...
def build_gateway_parser(subparsers, *, cmd_gateway: Callable, cmd_proxy: Callable) -> None:
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if ARES_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `ares proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
