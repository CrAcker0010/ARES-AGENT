...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``ares config set`` (which rewrites
    ...
    ``config.yaml`` in place: ares never silently mutates the user's config,
...
#   the next ``subprocess.run([...])`` Ares makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Ares itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``ares update``, the TUI build.
...
#   on every plugin install / ``ares update``.
...
# * ``ARES_HOME`` / ``ARES_PROFILE`` / ``ARES_CONFIG`` /
#   ``ARES_ENV`` — Ares runtime location flags. Writing these into
...
#
# IMPORTANT: ``ARES_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (ARES_GEMINI_CLIENT_ID,
# ARES_LANGFUSE_PUBLIC_KEY, ARES_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Ares runtime location — never via dashboard env writer.
    # NOT a ARES_* blanket: integration credentials (ARES_GEMINI_*,
    # ARES_LANGFUSE_*, ARES_SPOTIFY_*, ...) ARE allowed.
    "ARES_HOME", "ARES_PROFILE", "ARES_CONFIG", "ARES_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Ares runtime location "
            "(ARES_HOME, ARES_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # observability/langfuse` or `ares tools → Langfuse`); credentials gate
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    container detection here:
      ...
      - the published ``nousresearch/ares-agent`` image stamps ``docker``
    ...
    so ``ares update`` bailed with "doesn't apply inside the Docker
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Ares lives in a ``uv tool`` layout.

    ``uv tool install ares-agent`` places the install at
    ``.../uv/tools/ares-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``ares-agent`` happens to be uv-tool-installed on the machine while
    the *active* Ares is a regular pip/venv install, causing
    ``ares update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/ares-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade ares-agent"
        import shutil
        if shutil.which("uv"):
...
# Long-form text for ``ares update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# ares_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Ares, it doesn't update
...
✗ ``ares update`` doesn't apply inside the Docker container.

Ares Agent runs as a published image (nousresearch/ares-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/ares-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate ares-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/ares-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/ares-agent/tags
  • Your config and session history live under ``$ARES_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``ares update`` inside Docker.
...
def _resolve_ares_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the ARES_UID / ARES_GID env vars set by Docker deployments.

    Docker containers running Ares commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level ARES_HOME once, but
    subdirectories created at runtime by ``ensure_ares_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("ARES_UID", "").strip()
    gid_str = os.environ.get("ARES_GID", "").strip()
    try:
...
def _chown_to_ares_uid(path) -> None:
    """Chown ``path`` to ``ARES_UID:ARES_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_ares_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_ares_uid_gid()
...
def _secure_dir(path):
    ...
    Also applies ``ARES_UID``/``ARES_GID``-based ownership when those env
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # prompt's environment-hints block. Lets a host that wraps Ares
        ...
        # identity slot (SOUL.md). Empty by default. The ARES_ENVIRONMENT_HINT
    ...
    "terminal": {
        ...
        # bundled Ares image, which drops to the `ares` user via
    ...
    "display": {
        ...
        # Which interface bare `ares` (and `ares chat`) launches by default:
        ...
        # REPL and `--tui` (or ARES_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        ...
        # When true (default), `ares --tui` drops a one-time hint
    ...
    "dashboard": {
        ...
        # ``ARES_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``ARES_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``ARES_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # Public URL override (env: ``ARES_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/ares``) —
    ...
    "curator": {
        ...
        # normally restored on every `ares update`, so pruning them only
    ...
    "kanban": {
        ...
        # one `ares` launches with no -p flag). This does not control the
    ...
    #
    # Core Ares tools (terminal, read_file, write_file, patch,
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.ares/.env,
        ...
        # behavior: files must live under the Ares cache, under
        ...
        # user. Bridged to ARES_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Ares
        # cache (~/.ares/cache/{documents,images,audio,video,screenshots})
        ...
        # to ARES_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # ARES_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to ARES_MEDIA_TRUST_RECENT_SECONDS.
    ...
    "sessions": {
        ...
        # rewrites ``~/.ares/sessions/session_{sid}.json`` on every turn
    ...
    "updates": {
        ...
        "backup_keep": 5,
        # What `ares update` does with uncommitted local changes to the
    ...
    # rather than storing them in ~/.ares/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.ares/.env (or your shell) and never in config.yaml.
            ...
            # ~/.ares/bin/ on first use.  When False you must install
...
def show_config():
    ...
    # Warn on stale ARES_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("ARES_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale ARES_MAX_ITERATIONS={_env_ghost} "
                f"(run 'ares doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
