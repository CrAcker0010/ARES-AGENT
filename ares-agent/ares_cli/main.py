...
def _set_process_title() -> None:
    """Set the process title to 'ares' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``ares tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``ares.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("ares")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"ares", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"ares")
        # Windows: the .exe name is already ``ares.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* ares_cli.config is importable. Mirrors the explicit
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``ARES_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
        return False
    if os.environ.get("ARES_TUI") == "1" or "--tui" in argv:
        return True
...
# `entry.tsx`; this is just the earlier cousin. ``ARES_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("ARES_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`ares --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from ares_cli import __release_date__, __version__
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``ares --version`` before config/logging imports on Termux."""
    if os.environ.get("ARES_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from ares_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from ares_cli.subcommands.cron import build_cron_parser
from ares_cli.subcommands.gateway import build_gateway_parser
from ares_cli.subcommands.profile import build_profile_parser
from ares_cli.subcommands.model import build_model_parser
from ares_cli.subcommands.setup import build_setup_parser
from ares_cli.subcommands.postinstall import build_postinstall_parser
from ares_cli.subcommands.whatsapp import build_whatsapp_parser
from ares_cli.subcommands.slack import build_slack_parser
from ares_cli.subcommands.login import build_login_parser
from ares_cli.subcommands.logout import build_logout_parser
from ares_cli.subcommands.auth import build_auth_parser
from ares_cli.subcommands.status import build_status_parser
from ares_cli.subcommands.webhook import build_webhook_parser
from ares_cli.subcommands.hooks import build_hooks_parser
from ares_cli.subcommands.doctor import build_doctor_parser
from ares_cli.subcommands.security import build_security_parser
from ares_cli.subcommands.dump import build_dump_parser
from ares_cli.subcommands.debug import build_debug_parser
from ares_cli.subcommands.backup import build_backup_parser
from ares_cli.subcommands.import_cmd import build_import_cmd_parser
from ares_cli.subcommands.config import build_config_parser
from ares_cli.subcommands.version import build_version_parser
from ares_cli.subcommands.update import build_update_parser
from ares_cli.subcommands.uninstall import build_uninstall_parser
from ares_cli.subcommands.dashboard import build_dashboard_parser
from ares_cli.subcommands.gui import build_gui_parser
from ares_cli.subcommands.logs import build_logs_parser
from ares_cli.subcommands.prompt_size import build_prompt_size_parser
from ares_cli.subcommands.memory import build_memory_parser
from ares_cli.subcommands.acp import build_acp_parser
from ares_cli.subcommands.tools import build_tools_parser
from ares_cli.subcommands.insights import build_insights_parser
from ares_cli.subcommands.skills import build_skills_parser
from ares_cli.subcommands.pairing import build_pairing_parser
from ares_cli.subcommands.plugins import build_plugins_parser
from ares_cli.subcommands.mcp import build_mcp_parser
from ares_cli.subcommands.claw import build_claw_parser
...
if _FORCE_IPV4_EARLY:
    try:
        ...
        pass  # best-effort — don't crash if ares_constants not importable yet
...
# Provider model-selection wizard flows extracted to ares_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (ares_cli.main._model_flow_*) keep resolving unchanged.
from ares_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_ares_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("ARES_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("ARES_TERMUX_PREFETCH_UPDATES") == "1"
...
def _tui_need_npm_install(root: Path) -> bool:
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@ares" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/ares-ink/package.json",
    "packages/ares-ink/index.js",
    "packages/ares-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``ARES_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("ARES_TUI_FORCE_BUILD") or "").strip().lower()
...
def _launch_tui(
    ...
    if tui_toolsets:
        env["ARES_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["ARES_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["ARES_TUI_SKILLS"] = value
    if query:
        env["ARES_TUI_QUERY"] = query
    if image:
        env["ARES_TUI_IMAGE"] = image
    if checkpoints:
        env["ARES_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["ARES_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["ARES_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["ARES_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["ARES_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
    ...
    # ARES_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `ares --tui` try to
    ...
    # still set ARES_TUI_RESUME themselves.
    env.pop("ARES_TUI_RESUME", None)
    if resume_session_id:
        env["ARES_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `ares update` so
...
def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.ares/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Ares — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Ares still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``ARES_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("ARES_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    try:
        from ares_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from ares_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'ares doctor' for details.\033[0m\n\n")
    except Exception:
        pass
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from ares_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    # PROVIDER_GROUPS in ares_cli/models.py). A multi-member group shows one
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`ares_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from ares_cli.plugins import get_plugin_auxiliary_tasks
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from ares_cli.secret_prompt import masked_secret_prompt
...
# downstream call sites read `ares_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `ares` invocation — including
# fast paths like `ares --version` and slash-command dispatch that never
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    from ares_cli.env_loader import format_secret_source_suffix
...
def cmd_security(args):
    """Dispatch `ares security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from ares_cli.security_audit import cmd_security_audit
...
def _print_version_info(*, check_updates: bool = True) -> None:
    from ares_cli.banner import format_banner_version_label
...
def cmd_uninstall(args):
    """Uninstall Ares Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from ares_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from ares_cli.uninstall import run_gui_uninstall
...
# Critical files that every ``ares`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``ares update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "ares_cli/main.py",
    "ares_cli/config.py",
    "ares_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``ares`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="ares-syntax-check-") as tmpdir:
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``ares_cli/web_dist/`` (per vite.config.ts
    outDir: "../ares_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "ares_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``ares`` launcher present but ``ares_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the ares venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — ares venv python3, no nix-shell overhead
    ...
    # a ares venv (pip / non-managed / bare-git installs).  The resolved
...
#   - ``ares update`` can unconditionally call ``ares desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``ares desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $ARES_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $ARES_HOME."""
    from ares_constants import get_ares_home
    return get_ares_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``ares update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Ares.app/Contents/MacOS/Ares"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Ares.exe",
            release_dir / "win-ia32-unpacked" / "Ares.exe",
            release_dir / "win-arm64-unpacked" / "Ares.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "ares",
            release_dir / "linux-unpacked" / "Ares",
            release_dir / "linux-arm64-unpacked" / "ares",
            release_dir / "linux-arm64-unpacked" / "Ares",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Ares'`` desktop build failure: a corrupt zip in the
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Ares is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Ares.app/Contents/MacOS/Ares  ->  app bundle = .../Ares.app
    app = exe.parents[2]
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Ares Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Ares Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from ares_logging import setup_logging as _setup_logging_gui
    ...
    if getattr(args, "fake_boot", False):
        env["ARES_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["ARES_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "ares_root", None):
        env["ARES_DESKTOP_ARES_ROOT"] = str(Path(args.ares_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["ARES_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  ares gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if build_result.returncode != 0 and not source_mode:
                ...
                # on the final `electron` -> `Ares` rename: unpack-electron
            ...
            if not source_mode:
                ...
                # an in-place self-update (otherwise macOS reports "Ares is
    ...
    if source_mode:
        print("→ Launching Ares Desktop from source build...")
    ...
    print(f"→ Launching packaged Ares Desktop: {packaged_executable}")
...
def _find_stale_dashboard_pids(
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Ares Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``ares dashboard`` as
    ...
    environment variable ``ARES_DESKTOP_CHILD_PID`` on the spawned
...
def _kill_stale_dashboard_processes(
    ...
    """
    # When the Ares Desktop Electron app spawns this dashboard as a
    # backend child, it sets ARES_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("ARES_DESKTOP_CHILD_PID")
    if raw_pid:
...
def _update_via_zip(args):
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `ares update "
            f"--branch {branch}`, or update against main with `ares update`."
        )
        sys.exit(1)
    zip_url = (
    ...
    try:
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for ares-agent itself
    ...
    from ares_cli.managed_uv import ensure_uv, update_managed_uv
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Ares couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Ares couldn't drop "
...
def _detect_concurrent_ares_instances(
    ...
    The Ares Desktop Electron app spawns ``ares.EXE`` as a backend child,
    so during ``ares update`` the user-invoked process and the desktop's
    ...
    shims (``ares.exe`` / ``ares-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``ares update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``ares.exe``) is a separate process from the Python
    ...
    # setuptools-generated ares.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``ares update``
    ...
    #      ares.exe sitting *under* a non-Ares parent (e.g. a Ares
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "ares.exe"
    lines = ["✗ Another ares.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Ares Desktop, exit any open `ares` REPLs, and")
    lines.append("  stop the gateway (`ares gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `ares update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_ares_exe(
    ...
    handles (recovers in <1s), or the Ares Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Ares Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _ares_exe_shims(scripts_dir):
        ...
        print(
            "    Close Ares Desktop, exit other `ares` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `ares update`."
        )
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``ares.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``ares.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``ares`` drops off PATH. This wraps
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    # hours later inside ``ares update``'s desktop-rebuild or skill-sync
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``ares update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``ares.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``ares`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `ares update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `ares update --force` after closing other ares processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from ares_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$ARES_HOME/bin/uv``, but on Termux the official
    installer may not work (glibc vs bionic).  Fall back to ``pip install uv``
    which gets a Termux-compatible binary.
    """
    from ares_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `ares update`, so we install root-only first
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``ares update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `ares update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    ...
    from ares_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "ares-agent"]
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # On Windows, abort early if another ares.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_ares_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        # orphan merge-conflict markers in ares_cli/config.py bricked
        # every user who ran ``ares update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``ares
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``ares update`` again later once a fix lands.")
                    else:
        ...
        # build.  ``ares desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``ares desktop`` shouldn't be forced into a full
        # Electron build by ``ares update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        if (desktop_dir / "package.json").exists() and shutil.which("npm") and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "ares_cli.main", "desktop", "--build-only"]
            ...
            if build_result.returncode != 0:
                build_result = subprocess.run(_desktop_build_cmd, cwd=PROJECT_ROOT, check=False)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `ares desktop` to retry)")
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'ares config migrate' to retry.")
        ...
        try:
            from ares_cli.backup import restore_cron_jobs_if_emptied
...
def cmd_profile(args):
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → ares -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        if alias_name:
            ...
            print(f"Alias:   {alias_name} → ares -p {name}  ({wrapper})")
        print()
...
def cmd_dashboard(args):
    ...
    # the same logs directory as every other Ares surface.
    try:
        from ares_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
...
def cmd_dashboard_register(args):
    ...
    from ares_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_prompt_size(args):
    ...
    from ares_cli.prompt_size import cmd_prompt_size as _impl
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("ARES_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    if _is_tui_chat_launch(args):
        ...
        try:
            from ares_cli.mcp_startup import start_background_mcp_discovery
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["ARES_DEFER_AGENT_STARTUP"] = "1"
            os.environ["ARES_FAST_STARTUP_BANNER"] = "1"
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `ares --tui` is the hot path on phones. The full parser setup imports
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        ...
        from ares_cli.tools_config import run_post_setup_command
...
def main():
    ...
    # Cosmetic: make the process show up as 'ares' instead of 'python3.11'
    ...
    # =========================================================================
    # model command  (parser built in ares_cli/subcommands/model.py)
    # =========================================================================
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.ares/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://ares-agent.nousresearch.com/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from ares_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from ares_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in ares_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in ares_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in ares_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in ares_cli/subcommands/whatsapp.py)
    # =========================================================================
    build_whatsapp_parser(subparsers, cmd_whatsapp=cmd_whatsapp)

    # =========================================================================
    # slack command  (parser built in ares_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    # login command  (parser built in ares_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in ares_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in ares_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in ares_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in ares_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in ares_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from ares_cli.portal_cli import add_parser as _add_portal_parser
    ...
    # =========================================================================
    # hooks command  (parser built in ares_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in ares_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in ares_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in ares_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in ares_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in ares_cli/subcommands/backup.py)
    # =========================================================================
    ...
    # =========================================================================
    # import command  (parser built in ares_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in ares_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # pairing command  (parser built in ares_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in ares_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from ares_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in ares_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # =========================================================================
    # memory command  (parser built in ares_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in ares_cli/subcommands/tools.py)
    # =========================================================================
    ...
    # =========================================================================
    # mcp command  (parser built in ares_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    # =========================================================================
    # insights command  (parser built in ares_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in ares_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in ares_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in ares_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in ares_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in ares_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in ares_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in ares_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Ares-Setup.exe success screen tells users to
    # run `ares desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in ares_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in ares_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in ares_cli/subcommands/prompt_size.py)
    # =========================================================================
