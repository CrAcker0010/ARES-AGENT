...
from ares_cli.nous_account import format_nous_portal_entitlement_message
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``ares tools`` checklist actually offers
    for ``platform``.
    ...
    so ``ares tools`` never claims to add or remove a toolset the user was
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "ARES_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "ARES_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "ARES_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "ARES_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "ARES_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("ARES_CUA_DRIVER_CMD", "").strip() or "cua-driver"
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_ares_home()}/ares-agent && npm install --workspaces=false")
                if result.stderr:
        ...
        try:
            from ares_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: ares plugins enable observability/langfuse")
        _print_info("    Restart Ares for tracing to take effect.")
        _print_info("    Verify: ares plugins list")
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``ares tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``ares tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: ares tools post-setup <key>")
        return 2
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `ares model`).
    if managed_feature:
        from ares_cli.nous_subscription import (
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, ares mcp
