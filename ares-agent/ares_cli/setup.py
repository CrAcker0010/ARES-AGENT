...
from ares_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    # on demand via `ares auth add`, `ares setup` vision, and
    # `ares setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``ares model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        # configurable later via `ares setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `ares setup terminal`.
...
def _setup_telegram_auto_result():
    ...
    try:
        from ares_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
    ...
    try:
        ...
        if "/profiles/" in ares_home:
            profile_name = ares_home.rstrip("/").rsplit("/", 1)[-1]
    except Exception:
        pass
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``ares setup --portal`` and ``ares portal``. This is the
    ...
    Ares session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``ares setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``ares model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``ares portal`` always offers a model picker,
    ...
    print(color("│     ⚕ Ares Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `ares portal` == quick setup's Nous step.
    try:
        from ares_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `ares portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `ares portal`: %s", exc)
        print()
    ...
    print_info("  Run `ares portal info` to inspect routing.")
    print_info("  Run `ares` to start chatting.")
...
def run_setup_wizard(args):
    ...
    if is_existing:
        ...
        setup_mode = prompt_choice(
            "How would you like to set up Ares?",
            [
    ...
    # Tune later with `ares setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, ares_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``ares setup <section>``
    or switch providers with ``ares model``.
    """
    from ares_cli.config import load_config
