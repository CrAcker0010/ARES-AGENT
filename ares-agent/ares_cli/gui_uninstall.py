"""
Ares Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``ares desktop`` / ``ares gui``)
     Built inside the agent checkout under ``$ARES_HOME/ares-agent/``:
       ...
       - ``$ARES_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Ares.app`` or ``~/Applications/Ares.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Ares`` (NSIS per-user)
...
the app name ("Ares"), separate from ``$ARES_HOME``:
  - macOS:   ``~/Library/Application Support/Ares``
  - Windows: ``%APPDATA%\\Ares``
  - Linux:   ``$XDG_CONFIG_HOME/Ares`` (default ``~/.config/Ares``)
...
``ares uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def desktop_userdata_dir() -> Path:
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Ares"
...
def source_built_gui_artifacts(ares_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``ares desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``ares-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(ares_home)
    ...
    return [
        ...
        ares_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Ares" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Ares.app"),
            home / "Applications" / "Ares.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Ares).
            local_base / "Programs" / "Ares",
            # Older / alternate layout some builds used.
            local_base / "ares-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Ares")
    else:
        ...
        paths += [
            data_base / "applications" / "ares.desktop",
            data_base / "applications" / "Ares.desktop",
        ]
    return paths


def agent_is_installed(ares_home: Path) -> bool:
    """Return True when a usable Python agent install exists under ARES_HOME.
...
def gui_is_installed(ares_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(ares_home):
        if p.exists():
            return True
...
def gui_install_summary(ares_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = ares_home if ares_home is not None else get_ares_home()
...
def uninstall_gui(ares_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``ares-agent/ares_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$ARES_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove ares' or "
            "'sudo dnf remove ares'). AppImage builds are a single file you "
