...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.ares/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``ares mcp login`` to distinguish a genuine authentication
    ...
    try:
        ...
        return AresTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_login(args):
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `ares mcp login " + name + "`.")
            return
        if tools:
...
def mcp_command(args):
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `ares plugin`.
        from ares_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("ares mcp                                    Open the catalog picker (default)")
        _info("ares mcp catalog                            List Nous-approved MCPs")
        _info("ares mcp install <name>                     Install a catalog MCP")
        _info("ares mcp serve                              Run as MCP server")
        _info("ares mcp add <name> --url <endpoint>        Add a custom MCP server")
        ...
        _info("ares mcp list                               List configured servers")
