...
# xAI's public API is OpenAI-compatible for the endpoints Ares commonly
# uses. The Responses endpoint is included because Ares' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Ares-managed OAuth credentials."""

    auth_hint = "ares auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`ares auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`ares auth reset xai-oauth` or re-authenticate with "
                    "`ares auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `ares auth add xai-oauth --type oauth`."
            )
