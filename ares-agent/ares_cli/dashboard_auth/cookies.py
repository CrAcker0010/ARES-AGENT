...
Three cookies in play:
  ...
  - ares_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
setter wrote ``__Secure-ares_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "ares_session_at"
SESSION_RT_COOKIE = "ares_session_rt"
PKCE_COOKIE = "ares_session_pkce"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /ares`` we want ``Path=/ares`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    issues a 24h rotating refresh token (ares #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/ares``)
