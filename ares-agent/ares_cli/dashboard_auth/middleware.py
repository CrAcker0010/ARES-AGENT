...
from ares_cli.dashboard_auth.base import ProviderError, RefreshExpiredError
from ares_cli.dashboard_auth.cookies import read_session_cookies
from ares_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /ares``, the
    ``login_url`` is prefixed (``/ares/login?next=...``) so the
    ...
    """
    from ares_cli.dashboard_auth.prefix import prefix_from_request
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["ares.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # sends only ``ares_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if session is None:
        ...
        from ares_cli.dashboard_auth.cookies import clear_session_cookies
        from ares_cli.dashboard_auth.prefix import prefix_from_request
