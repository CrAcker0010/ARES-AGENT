...
Profile-aware location: ``$ARES_HOME/logs/dashboard-auth.log``.
...
from ``ares_constants`` or other ares_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$ARES_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``ares_constants.get_ares_home`` semantics: env var wins,
    else ``~/.ares``. A local copy avoids an import cycle with the
    middleware which lives below ``ares_cli``.
    """
    home = os.environ.get("ARES_HOME") or str(Path.home() / ".ares")
