...
_GATEWAY_LIFECYCLE_PATTERNS = re.compile(
    r"(?i)"
    r"(ares\s+gateway\s+(restart|stop|start))"
    r"|(launchctl\s+(kickstart|unload|load|stop|restart)\s+.*ares)"
    r"|(systemctl\s+(restart|stop|start)\s+.*ares)"
    r"|(p?kill\s+.*ares.*gateway)"
)
...
def cron_create(args):
    ...
    if _contains_gateway_lifecycle_command(combined):
        print(color(
            ...
            "Use `ares gateway restart` from a shell outside the gateway.",
            Colors.RED,
        ))
        return 1
