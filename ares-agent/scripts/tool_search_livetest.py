#!/usr/bin/env python3
"""Live test harness for Ares Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any ares imports.
ORIGINAL_HOME = os.environ.get("ARES_HOME")
ORIGINAL_AUTH = Path.home() / ".ares" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.ares/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.ares/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="ares_ts_live_"))
    ares_home = home_dir / ".ares"
    ares_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, ares_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".ares" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, ares_home / ".env")
        ...
        load_ares_dotenv(ares_home=str(Path.home() / ".ares"))
    ...
    (ares_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return ares_home
...
def reset_module_state():
    """Drop cached modules so the new ARES_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "ares_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original ARES_HOME
    if ORIGINAL_HOME is not None:
        os.environ["ARES_HOME"] = ORIGINAL_HOME
    else:
