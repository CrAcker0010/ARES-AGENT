...
# Python-driven tool session (e.g. Ares terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/ares-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/ares-agent.git"
ARES_HOME="${ARES_HOME:-$HOME/.ares}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${ARES_INSTALL_DIR:-}" ]; then
...
#   code at /usr/local/lib/ares-agent, command at /usr/local/bin/ares,
#   data still at /root/.ares (ARES_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$ARES_HOME/.no-bundled-skills so future"
            echo "                   'ares update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Ares.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.ares/ares-agent"
            echo "                   default (root, Linux): /usr/local/lib/ares-agent"
            echo "  --ares-home PATH  Data directory (default: ~/.ares, or \$ARES_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Ares installs the code under"
            echo "  /usr/local/lib/ares-agent and links the command into"
            echo "  /usr/local/bin/ares (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$ARES_HOME"
            echo "  (default /root/.ares).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$ARES_HOME/ares-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs ares setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# intentional, but they leave the checkout dirty — which forces `ares update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Ares-Setup) passes it so
...
# Decide where the repo checkout + venv live, and where the `ares` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $ARES_HOME/ares-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $ARES_HOME/ares-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/ares-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $ARES_HOME/ares-agent — then preserve it)
#
# Always no-op when the user set --dir or $ARES_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in ARES_HOME.
    if is_termux; then
        INSTALL_DIR="$ARES_HOME/ares-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$ARES_HOME/ares-agent/.git" ]; then
            INSTALL_DIR="$ARES_HOME/ares-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/ares-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/ares-agent"
        ...
        # /usr/local/bin/ares wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/ares"
        log_info "  Data:    $ARES_HOME (unchanged)"
...
get_ares_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/ares" ]; then
        echo "$link_dir/ares"
    else
        echo "ares"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://ares-agent.nousresearch.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Ares owns its own uv at $ARES_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (ares_cli/managed_uv.py) looks in the same
    # place, so install.sh and `ares update` stay in sync.
    local _managed_uv="$ARES_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $ARES_HOME/bin ..."
    mkdir -p "$ARES_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/ares-uv-install.$$.log")"
    ...
    # directly into $ARES_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$ARES_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the Ares-
...
check_node() {
    ...
    if [ -x "$ARES_HOME/node/bin/node" ] && node_satisfies_build "$("$ARES_HOME/node/bin/node" --version)"; then
        export PATH="$ARES_HOME/node/bin:$PATH"
        log_success "Node.js $("$ARES_HOME/node/bin/node" --version) found (Ares-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Ares-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.ares/node/..."
    ...
    # Place into ~/.ares/node/ and symlink binaries into the same bin dir
    # the ares command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$ARES_HOME/node"
    mkdir -p "$ARES_HOME"
    mv "$extracted_dir" "$ARES_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$ARES_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$ARES_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$ARES_HOME/node/bin/npx"  "$node_link_dir/npx"

    export PATH="$ARES_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$ARES_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.ares/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Ares install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Ares Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="ares-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Ares behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up ares command..."

    if [ "$USE_VENV" = true ]; then
        ARES_BIN="$INSTALL_DIR/venv/bin/ares"
    else
        ARES_BIN="$(which ares 2>/dev/null || echo "")"
        if [ -z "$ARES_BIN" ]; then
            log_warn "ares not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$ARES_BIN" ]; then
        log_warn "ares entry point not found at $ARES_BIN"
    ...
    # Create a user-facing shim for the ares command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $ARES_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$ARES_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/ares"
    cat > "$command_link_dir/ares" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ARES_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/ares"
    log_success "Installed ares launcher → $command_link_display_dir/ares"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "ares command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v ares' \
                >/dev/null 2>&1; then
        ...
        log_info "ares not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Ares Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Ares Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so ares works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.ares directory structure (config at top level, code in subdir)
    mkdir -p "$ARES_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.ares/.env (top level, easy to find)
    if [ ! -f "$ARES_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$ARES_HOME/.env"
            log_success "Created ~/.ares/.env from template"
        else
            touch "$ARES_HOME/.env"
            log_success "Created ~/.ares/.env"
        fi
    else
        log_info "~/.ares/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$ARES_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.ares/config.yaml (top level, easy to find)
    if [ ! -f "$ARES_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$ARES_HOME/config.yaml"
            log_success "Created ~/.ares/config.yaml from template"
        fi
    else
        log_info "~/.ares/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$ARES_HOME/SOUL.md" ]; then
        cat > "$ARES_HOME/SOUL.md" << 'SOUL_EOF'
# Ares Agent Persona
...
-->
SOUL_EOF
        log_success "Created ~/.ares/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.ares/"
    ...
    if [ "$NO_SKILLS" = true ]; then
        ...
        # skills_sync.py and `ares update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            > "$ARES_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $ARES_HOME/.no-bundled-skills"
        log_info "  Future 'ares update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.ares/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.ares/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$ARES_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$ARES_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.ares/skills/"
            fi
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Ares Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Ares will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (ares --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `ares update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'ares setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m ares_cli.main setup < /dev/tty
    else
        python -m ares_cli.main setup < /dev/tty
    fi
}
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Ares to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$ARES_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'ares whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                ARES_CMD="$(get_ares_command_path)"
                $ARES_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'ares whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'ares gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $ARES_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $ARES_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: ares gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: ares gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $ARES_CMD gateway > "$ARES_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.ares/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: ares gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: ares gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $ARES_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $ARES_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $ARES_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}ares${NC}              Start chatting"
    echo -e "   ${GREEN}ares setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}ares config${NC}       View/edit configuration"
    echo -e "   ${GREEN}ares config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}ares gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}ares update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'ares' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'ares' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'ares' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$ARES_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$ARES_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$ARES_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$ARES_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$ARES_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Ares for pip install"
    ...
    ARES_CMD="$(command -v ares 2>/dev/null || echo "")"
    if [ -n "$ARES_CMD" ]; then
        log_info "Running ares setup..."
        "$ARES_CMD" setup
    else
        log_warn "ares command not found on PATH"
        log_info "Try: python -m ares_cli.main setup"
    fi
}
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Ares-managed Node to PATH, so
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Ares" ]; then
            app="$desktop_dir/release/linux-unpacked/Ares"
        elif [ -x "$desktop_dir/release/linux-unpacked/ares" ]; then
            app="$desktop_dir/release/linux-unpacked/ares"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Ares.app" \
            "$desktop_dir/release/mac/Ares.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("Ares is damaged and can't be opened").
    ...
    # checkout stays clean for the next `ares update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Ares-managed Node
            # provisioned during prerequisites/node-deps (at $ARES_HOME/node/bin)
        ...
        complete)
            ...
            resolve_install_layout
            print_success
            echo "git" > "$ARES_HOME/.install_method"
            ;;
        *)
