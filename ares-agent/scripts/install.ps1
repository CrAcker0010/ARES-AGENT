# ============================================================================
# Ares Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$AresHome = $(if ($env:ARES_HOME) { $env:ARES_HOME } else { "$env:LOCALAPPDATA\ares" }),
    [string]$InstallDir = $(if ($env:ARES_HOME) { "$env:ARES_HOME\ares-agent" } else { "$env:LOCALAPPDATA\ares\ares-agent" }),
    ...
    # builds apps/desktop into a launchable Ares.exe.
    #
    # Why opt-in:
    #   * Ares-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Ares.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Ares.exe
    ...
    #     `ares desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:NousResearch/ares-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/ares-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * Ares Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $AresHome)) {
        New-Item -ItemType Directory -Force -Path $AresHome | Out-Null
    }
    $envFile = Join-Path $AresHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $AresHome "node"
...
function Install-Uv {
    # Ares owns its own uv at $AresHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (ares_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `ares update` stay in sync.
    $managedUv = Join-Path $AresHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $AresHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $AresHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $AresHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $AresHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and ARES_HOME wasn't propagated).
...
function Install-Git {
    ...
    which Ares uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\ares\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Ares needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Ares copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\ares\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``ARES_GIT_BASH_PATH`` (User scope) so Ares can find it in a fresh
    ...
    # Download PortableGit into $AresHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $AresHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Ares features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Ares needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``ARES_GIT_BASH_PATH`` (User env scope) so Ares can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $AresHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $AresHome\git\usr\bin\bash.exe
    $candidates += "$AresHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$AresHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("ARES_GIT_BASH_PATH", $candidate, "User")
            $env:ARES_GIT_BASH_PATH = $candidate
            Write-Info "Set ARES_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Ares may not find Git Bash."
    Write-Info "If needed, set ARES_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    # Prefer a Ares-managed Node from a previous run over a too-old system one.
    $managedNode = "$AresHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$AresHome\node;$env:Path"
        Write-Success "Node.js $version found (Ares-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Ares-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $AresHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $AresHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\ares-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$AresHome\node") { Remove-Item -Recurse -Force "$AresHome\node" }
                Move-Item $extractedDir.FullName "$AresHome\node"
                ...
                $version = & "$AresHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $AresHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\ares-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    $stashName = "ares-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Ares behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running ares processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NousResearch/ares-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NousResearch/ares-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NousResearch/ares-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\ares-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\ares-agent-extract"
    ...
    # next `ares update` checkout aborts on a "dirty" tree the user never
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `ares.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install ares-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and ares.exe broken with
    ...
    # here, not 30 seconds later when the user runs `ares`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `ares dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `ares dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `ares dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up ares command..."
    
    if ($NoVenv) {
        $aresBin = "$InstallDir"
    } else {
        $aresBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so ares is globally available
    # On Windows, the ares.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$aresBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$aresBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $aresBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set ARES_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\ares instead
    # of the Unix default ~/.ares
    $currentAresHome = [Environment]::GetEnvironmentVariable("ARES_HOME", "User")
    if (-not $currentAresHome -or $currentAresHome -ne $AresHome) {
        [Environment]::SetEnvironmentVariable("ARES_HOME", $AresHome, "User")
        Write-Success "Set ARES_HOME=$AresHome"
    }
    $env:ARES_HOME = $AresHome
    
    # Update current session
    $env:Path = "$aresBin;$env:Path"
    
    Write-Success "ares command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.ares-bootstrap-complete which tells the Ares
    ...
    $markerPath = Join-Path $InstallDir ".ares-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    New-Item -ItemType Directory -Force -Path "$AresHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AresHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AresHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AresHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AresHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AresHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AresHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AresHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AresHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$AresHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.ares/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.ares/.env"
        }
    } else {
        Write-Info "~/.ares/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$AresHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.ares/config.yaml from template"
        }
    } else {
        Write-Info "~/.ares/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and Ares's prompt-injection scanner
...
Edit this to customize how Ares communicates with you.
...
"@
        ...
        Write-Success "Created ~/.ares/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.ares/"
    
    # Seed bundled skills into ~/.ares/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.ares/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.ares/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$AresHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.ares/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Ares-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'ares setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\ares-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\ares-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\ares-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Ares` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Ares.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_ares_desktop command
    # resolves apps/desktop/release/win-unpacked/Ares.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Ares-managed
    ...
    # The --dir mode produces an unpacked Ares.exe in
    ...
    # are NOT trying to work around). The Ares icon + product name are
    # stamped onto Ares.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\ares-desktop-build-$(Get-Random).log"
    ...
    try {
        ...
        if ($code -ne 0) {
            ...
            # ENOENT on the final `electron` -> `Ares` rename: app-builder's
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Ares.exe",
        "$desktopDir\release\win-arm64-unpacked\Ares.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Ares.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Ares icon + identity are stamped onto Ares.exe by the
    ...
    #    Ares.exe. We deliberately do NOT point them at `ares desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Ares.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Ares.lnk')
        )
        ...
        # with the Ares icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.ares/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'ares setup'."
        return
    }
    ...
    # Run ares setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m ares_cli.main setup
    } else {
        python -m ares_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $aresCmd = "$InstallDir\venv\Scripts\ares.exe"
    if (-not (Test-Path $aresCmd)) {
        $aresCmd = "ares"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$AresHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'ares whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: ares gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$AresHome\logs\gateway.log"
            Start-Process -FilePath $aresCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$AresHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: ares gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: ares gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$AresHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$AresHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$AresHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$AresHome\ares-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   ares              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   ares setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   ares config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   ares config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   ares gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   ares update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Ares repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Ares-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Ares to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://ares-agent.nousresearch.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
