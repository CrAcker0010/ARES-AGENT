#!/usr/bin/env bash
# Canonical test runner for ares-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.ares/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.ares/ares-agent/venv"; do
...
if [ -f "$HOME/.ares/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.ares"
