...
# Bootstrap Open WebUI against Ares Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.ares/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-ares.sh
...
#   ARES_API_MODEL_NAME='Ares Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Ares Agent WebUI}"
...
ARES_ENV_FILE="${ARES_ENV_FILE:-$HOME/.ares/.env}"
ARES_API_PORT="${ARES_API_PORT:-8642}"
ARES_API_HOST="${ARES_API_HOST:-127.0.0.1}"
ARES_API_CONNECT_HOST="${ARES_API_CONNECT_HOST:-127.0.0.1}"
ARES_API_MODEL_NAME="${ARES_API_MODEL_NAME:-Ares Agent}"
ARES_API_BASE_URL="http://${ARES_API_CONNECT_HOST}:${ARES_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-ares.sh"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$ARES_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$ARES_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.ares.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.ares</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.ares"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.ares"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-ares.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Ares Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-ares.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.ares/logs/openwebui.log
StandardError=append:%h/.ares/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-ares.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$ARES_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Ares API server is configured...'
  upsert_env API_SERVER_ENABLED true "$ARES_ENV_FILE"
  upsert_env API_SERVER_HOST "$ARES_API_HOST" "$ARES_ENV_FILE"
  upsert_env API_SERVER_PORT "$ARES_API_PORT" "$ARES_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$ARES_API_MODEL_NAME" "$ARES_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$ARES_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Ares gateway so API server settings take effect...'
  ares gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${ARES_API_CONNECT_HOST}:${ARES_API_PORT}/health" >/dev/null; then
    log 'Ares API server did not answer on the first check. Trying to start gateway in the background...'
    nohup ares gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${ARES_API_CONNECT_HOST}:${ARES_API_PORT}/health" >/dev/null
  ...
  log "Ares API endpoint: ${ARES_API_BASE_URL}"
