...
as ``kind: backend``) or ``~/.ares/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``ares tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$ARES_HOME/cache/videos/``, creating parents as needed."""
    from ares_constants import get_ares_home

    path = get_ares_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$ARES_HOME/cache/videos/``.
