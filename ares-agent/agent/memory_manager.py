...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_ARES_CORE_TOOLS)
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``ares_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_ares_home()`` themselves.
        """
        if "ares_home" not in kwargs:
            from ares_constants import get_ares_home
            kwargs["ares_home"] = str(get_ares_home())
        for provider in self._providers:
            try:
