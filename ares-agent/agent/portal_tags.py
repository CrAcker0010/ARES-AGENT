...
Every Ares request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Ares Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=ares-agent",
        "client=ares-client-v<__version__>",
    ]

The version is sourced live from ``ares_cli.__version__`` so it auto-aligns
...
``ares_cli.__version__`` is the canonical source of truth.
"""
...
def _ares_version() -> str:
    """Return the current Ares release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``ares_cli`` cannot be imported (should
...
def ares_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=ares-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=ares-client-v{_ares_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=ares-agent", ares_client_tag()]
