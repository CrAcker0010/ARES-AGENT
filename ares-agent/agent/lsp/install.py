...
appropriate.  All installs go to a Ares-owned bin staging dir,
``<ARES_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``ares lsp
  status``.
...
# ``<ARES_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def ares_lsp_bin_dir() -> Path:
    """Return the Ares-owned bin staging dir for LSP servers."""
    home = os.environ.get("ARES_HOME")
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".ares")
    p = Path(home) / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(ares_lsp_bin_dir() / name):
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = ares_lsp_bin_dir().parent  # <ARES_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a ares-owned target dir.
    ...
    console script.
    """
    pip_target = ares_lsp_bin_dir().parent / "python-packages"
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = ares_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``ares lsp status`` CLI to give users a quick
