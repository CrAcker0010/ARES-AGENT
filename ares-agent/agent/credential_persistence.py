...
``auth.json``.  They intentionally have no dependency on ``ares_cli.auth`` so
...
# Sources Ares owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Ares-owned OAuth/device-code state)
