...
def _ares_home_path() -> Path:
    """Resolve the active ARES_HOME (profile-aware) without circular imports."""
    try:
        from ares_constants import get_ares_home  # local import to avoid cycles
        return get_ares_home()
    except Exception:
        return Path(os.path.expanduser("~/.ares"))


def _ares_root_path() -> Path:
    """Resolve the Ares root dir (always the parent of any profile, never per-profile)."""
    try:
        from ares_constants import get_default_ares_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    ares_home = _ares_home_path()
    ares_root = _ares_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(ares_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(ares_root / ".anthropic_oauth.json"),
...
def get_safe_write_root() -> Optional[str]:
    """Return the resolved ARES_WRITE_SAFE_ROOT path, or None if unset."""
    root = os.getenv("ARES_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    # Ares control-plane files: block both the ACTIVE profile's view
    # (ares_home) AND the global root view. Without the root pass, a
    ...
    ares_dirs = []
    for base in (_ares_home_path(), _ares_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in ares_dirs:
                ares_dirs.append(real)
        except Exception:
            continue

    for base_real in ares_dirs:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Ares path.

    Three categories are blocked:

      * Internal Ares cache files under ``ARES_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under ARES_HOME and the global Ares
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.ares/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active ARES_HOME (profile-aware) AND the global
    # Ares root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (ARES_HOME points at
    ...
    for hd in ares_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Ares cache file "
    ...
    for hd in ares_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Ares credential store "
    ...
    for hd in ares_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Ares MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Ares MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Ares profiles are separate ARES_HOME dirs under
...
#
# Reference: May 2026 incident where a ares-security profile session
# edited skills under both ``~/.ares/profiles/ares-security/skills/``
# AND ``~/.ares/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under ARES_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from ARES_HOME.
    ...
    """
    try:
        home_real = _ares_home_path().resolve()
        root_real = _ares_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Ares scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Ares entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Ares profile {info['target_profile']!r}, but the "
...
#
#   <ARES_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.ares/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.ares/…`` segment and warns
# regardless of which Ares profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.ares/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.ares`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.ares/…`` and returns the
    index where the inner Ares-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .ares / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".ares":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Ares state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.ares``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.ares`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Ares resolver to
    succeed, so it works correctly even when called from contexts where
    ARES_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Ares process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real ARES_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.ares/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.ares/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host ARES_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Ares state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Ares "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real ARES_HOME. Use the "
