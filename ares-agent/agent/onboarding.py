...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Ares starts and finds ``~/.openclaw/``.

    Points users at ``ares claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``ares claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Ares, run "
        "`ares claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `ares claw cleanup` (renames it to ~/.openclaw.pre-migration — "
