...
def _state_file() -> Path:
    return get_ares_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.ares/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `ares update` re-seeds.
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``ares update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``ares curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `ares curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Ares' background skill CURATOR. This is an "
    ...
    "into ~/.ares/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: ares-config-*, ares-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.ares/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.ares/logs/curator/``)
    ...
    authored skill data in ``~/.ares/skills/``.

    ``ensure_ares_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_ares_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every ares install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        keep an umbrella stable: ares curator pin document-tools
    ...
    lines.append("full report: ares curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: ares curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.ares/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.ares/skills/.archive/` for "
            "safety and can be restored via `ares curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.ares/skills/.archive/`. "
            "Restore any via `ares curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `ares curator restore <name>`")
    lines.append("- All archives live under `~/.ares/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `ares curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `ares update` only if "
        ...
        # recorded in state so `ares curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``ares model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `ares model` → auxiliary
