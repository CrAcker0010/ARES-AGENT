...
upstream prefix cache warm.  See ``ares-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Ares never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want ARES_HOME persona while keeping
    ...
    # Pointer to the ares-agent skill + docs for user questions about Ares itself.
    stable_parts.append(ARES_AGENT_HELP_GUIDANCE)
    ...
    # ARES_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Active-profile hint — names the Ares profile the agent is running
    # under so it doesn't conflate ~/.ares/skills/ (default profile) with
    # ~/.ares/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Ares profile: default. Other profiles (if any) live "
            "under ~/.ares/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Ares profile: {active_profile}. This session reads "
            f"and writes ~/.ares/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.ares/skills/, ~/.ares/plugins/, "
            f"~/.ares/cron/, ~/.ares/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Ares never rebuilds or reinjects parts of it
