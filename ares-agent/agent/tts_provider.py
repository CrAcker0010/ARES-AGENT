...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Ares with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.ares/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Ares
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``ares tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``ares setup`` for
