...
projection into Ares' display, approval bridging, transcript projection into
...
"codex_app_server"`. Ares' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        if spawn_env.get("ARES_KANBAN_TASK"):
            kanban_db = spawn_env.get("ARES_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "ARES_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("ARES_HOME", os.path.expanduser("~/.ares")),
                        "kanban",
                    ),
                )
            )
