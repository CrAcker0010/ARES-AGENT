...
Owns one Codex thread per Ares session. Drives `turn/start`, consumes
...
# Ares' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_ARES_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Ares/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Ares session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _ARES_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("ARES_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="ares",
            client_title="Ares Agent",
            client_version=_get_ares_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Ares' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Ares' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own ares-tools callback we
            # auto-accept — the user already approved Ares' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Ares approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Ares returns 'once', 'session', 'always', or 'deny'.
...
def _get_ares_version() -> str:
    """Best-effort Ares version string for codex's userAgent line."""
    try:
