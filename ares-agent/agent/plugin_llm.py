...
Plugins built on Ares Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Ares
...
:class:`~ares_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Ares' main config
...
handles every provider, fallback chain, and per-task override Ares
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`ares_cli.plugins.PluginContext`
