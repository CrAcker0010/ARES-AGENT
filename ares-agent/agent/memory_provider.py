...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - ares_home (str): The active ARES_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.ares``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "ares").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'ares memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], ares_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'ares memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``ares_home`` is the active ARES_HOME directory path.
