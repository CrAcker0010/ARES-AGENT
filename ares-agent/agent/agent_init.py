...
from ares_cli.timeouts import get_provider_request_timeout
...
def _build_codex_gpt55_autoraise_notice(autoraise: Dict[str, float]) -> str:
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: ares config set compression.codex_gpt55_autoraise false"
    )
...
def init_agent(
    ...
    Args:
        ...
        load_soul_identity (bool): If True, still use ~/.ares/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from ares_cli.config import load_config as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind ARES_DEV_CREDITS.
    ...
    # both live under ~/.ares/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from ares_logging import setup_logging, setup_verbose_logging
    setup_logging(ares_home=_ra()._ares_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that ares_logging.setup_logging() does not
        ...
        # handler level inside ares_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from ares_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from ares_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `ares model` to "
                        "select a provider, or run `ares setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff ARES_KANBAN_TASK is set).
    ...
    try:
        ...
        os.environ["ARES_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.ares/sessions/ alongside gateway sessions
    ares_home = get_ares_home()
    agent.logs_dir = ares_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.ares/sessions/session_{sid}.json)
    ...
    try:
        from ares_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from ares_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "ares"
                    except Exception:
                        pass
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            try:
                from ares_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Ares Agent.  Choose a model with at least "
