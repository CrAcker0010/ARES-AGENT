...
from ares_constants import get_ares_home, get_skills_dir, is_wsl
...
_ARES_MD_NAMES = (".ares.md", "ARES.md")


def _find_ares_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.ares.md`` or ``ARES.md``.
...
ARES_AGENT_HELP_GUIDANCE = (
    "You run on Ares Agent (by Nous Research). When the user needs help with "
    "Ares itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://ares-agent.nousresearch.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `ares-agent` skill with skill_view(name='ares-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.ares/kanban.db`. Your task id is in "
    "`$ARES_KANBAN_TASK`; your workspace is `$ARES_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $ARES_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `ares kanban <verb>` for board operations. Use "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Ares "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Ares WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Ares itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Ares runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Ares itself is running. The host OS, home, and cwd "
                f"of the Ares process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Ares "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Ares
    ...
    extra = (os.getenv("ARES_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_ares_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.ares/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("ARES_PLATFORM")
        or get_session_env("ARES_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Ares Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `ares-agent` skill "
            "first. It has the actual commands (e.g. `ares config set …`, `ares tools`, "
            "`ares setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: ares setup, ares setup tools, ares setup terminal, ares status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from ARES_HOME and return its content, or None.
    ...
    """
    try:
        from ares_cli.config import ensure_ares_home
        ensure_ares_home()
    except Exception as e:
        logger.debug("Could not ensure ARES_HOME before loading SOUL.md: %s", e)

    soul_path = get_ares_home() / "SOUL.md"
...
def _load_ares_md(cwd_path: Path) -> str:
    """.ares.md / ARES.md — walk to git root."""
    ares_md_path = _find_ares_md(cwd_path)
    if not ares_md_path:
        return ""
    try:
        content = ares_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = ares_md_path.name
        try:
            rel = str(ares_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".ares.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", ares_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .ares.md / ARES.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from ARES_HOME is independent and always included when present.
    ...
    # SOUL.md from ARES_HOME only — skip when already loaded as identity
    if not skip_soul:
