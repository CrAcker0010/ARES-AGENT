...
Storage
-------
Bundles live in ``~/.ares/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``ARES_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``ares bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under ARES_HOME.

    Honors ``ARES_BUNDLES_DIR`` for tests; falls back to
    ``<ARES_HOME>/skill-bundles``.
    """
    override = os.environ.get("ARES_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_ares_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `ares bundles` CLI subcommand.
# ---------------------------------------------------------------------------
