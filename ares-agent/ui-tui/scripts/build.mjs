...
await build({
  ...
  // Skip the prebuilt @ares/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@ares/ink': resolve(root, 'packages/ares-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The ares_cli launcher
