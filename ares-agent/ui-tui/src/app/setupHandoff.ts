import type { RunExternalProcess } from '@ares/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`ares ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching ares: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`ares ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
