...
React + Ink terminal UI for Ares. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `ARES_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.ares/.ares_history` or under `ARES_HOME`.
...
```text
ui-tui/
  packages/ares-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      ares-ink.d.ts    type declarations for @ares/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent AresCLI subprocess for slash commands
```
