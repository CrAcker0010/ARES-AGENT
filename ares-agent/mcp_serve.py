"""
Ares MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Ares-specific extra)
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ARES_HOME."""
    try:
        from ares_constants import get_ares_home
        return get_ares_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ARES_HOME", Path.home() / ".ares")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from ares_constants import get_ares_home
        directory_file = get_ares_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("ARES_HOME", Path.home() / ".ares")
...
class EventBridge:
    ...
    This is the Ares equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from ares_constants import get_ares_home
            db_file = get_ares_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("ARES_HOME", Path.home() / ".ares")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    ...
    mcp = FastMCP(
        "ares",
        instructions=(
            "Ares Agent messaging bridge. Use these tools to interact with "
