...
#
# Wired into the image as /etc/cont-init.d/01-ares-setup by the
...
ARES_HOME="${ARES_HOME:-/opt/data}"
INSTALL_DIR="/opt/ares"

# Drop to ares via s6-setuidgid, but skip it when already non-root.
as_ares() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid ares "$@"; }
...
# the container starts non-root. The baked image trees (/opt/data, /opt/ares/
# .venv, ui-tui, node_modules) stay owned by the ares build UID (10000), so an
...
# into $ARES_HOME). See #34837 for the supervision-tree side of this.
#
# The supported way to match host-side ownership is to start as root (the image
# default) and pass ARES_UID/ARES_GID — or the PUID/PGID aliases — which the
...
# `id -u ares` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# ares UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u ares)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-ares UID).
...
and the baked image directories are owned by the ares user (UID $(id -u ares)),
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e ARES_UID=\$(id -u) -e ARES_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the ares user to that UID/GID at boot and chowns the data
...
# --- Bootstrap ARES_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid ares mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom ARES_HOME paths whose parents only
# root can create (e.g. `ARES_HOME=/home/ares/.ares` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for ARES_UID/ARES_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  ARES_UID/
# ARES_GID still win when both are set.  See #15290, salvages #25872.
ARES_UID="${ARES_UID:-${PUID:-}}"
ARES_GID="${ARES_GID:-${PGID:-}}"

if [ -n "${ARES_UID:-}" ] && validate_uid_gid "$ARES_UID" && [ "$ARES_UID" != "$(id -u ares)" ]; then
    echo "[stage2] Changing ares UID to $ARES_UID"
    usermod -u "$ARES_UID" ares
fi
if [ -n "${ARES_GID:-}" ] && validate_uid_gid "$ARES_GID" && [ "$ARES_GID" != "$(id -g ares)" ]; then
    echo "[stage2] Changing ares GID to $ARES_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$ARES_GID" ares 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised ares user (UID 10000)
...
# ares process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes ares. Idempotent across container
...
#   - ares is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G ares 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] ares already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" ares 2>/dev/null; then
        echo "[stage2] Added ares to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group ares failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When ARES_UID is remapped or the top-level $ARES_HOME isn't owned by
# the runtime ares UID, restore ownership to ares — but ONLY for the
# directories ares actually writes to. The full $ARES_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of ares-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_ares_uid=$(id -u ares)
needs_chown=false
if [ "$(stat -c %u "$ARES_HOME" 2>/dev/null)" != "$actual_ares_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $ARES_HOME (targeted) to ares ($actual_ares_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $ARES_HOME: chown the directory itself (not its contents)
    # so ares can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    chown ares:ares "$ARES_HOME" 2>/dev/null || \
        echo "[stage2] Warning: chown $ARES_HOME failed (rootless container?) — continuing"
    # Ares-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by ares (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing; do
        if [ -e "$ARES_HOME/$sub" ]; then
            chown -R ares:ares "$ARES_HOME/$sub" 2>/dev/null || \
                echo "[stage2] Warning: chown $ARES_HOME/$sub failed (rootless container?) — continuing"
        fi
    done
fi

# --- Fix ownership of build trees under $INSTALL_DIR ---
# Ares-owned trees under $INSTALL_DIR must be re-chowned whenever the
# runtime ares UID no longer owns them — otherwise:
...
#     the source mtime is newer than dist/ or when ARES_TUI_FORCE_BUILD
#     is set) and writes to ui-tui/dist/. Without this chown the new
#     ares UID can't write the build output (#28851).
...
# The set mirrors the build-time `chown -R ares:ares` line in the
# Dockerfile — keep them in sync if the Dockerfile chown set changes.
# These are under $INSTALL_DIR (not $ARES_HOME), so the bind-mount
# concern doesn't apply — recursive is fine.
#
# This MUST be gated independently of the $ARES_HOME ownership check
# above. `usermod -u <new> ares` re-chowns the ares home dir
# ($ARES_HOME == /opt/data) to the new UID as a side effect, so after a
# ARES_UID/PUID remap `stat $ARES_HOME` always already matches the new
# UID and `needs_chown` is false — but the build trees under /opt/ares
# are NOT touched by usermod and remain owned by the build-time UID
# (10000). Gating them on $ARES_HOME ownership (as #35027 did) silently
...
# when the venv is not already owned by the runtime ares UID. Idempotent
...
if [ -n "$venv_owner" ] && [ "$venv_owner" != "$actual_ares_uid" ]; then
    echo "[stage2] Fixing ownership of build trees under $INSTALL_DIR to ares ($actual_ares_uid)"
    chown -R ares:ares \
        "$INSTALL_DIR/.venv" \
        "$INSTALL_DIR/ui-tui" \
        "$INSTALL_DIR/gateway" \
...
# Always reset ownership of $ARES_HOME/profiles to ares on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> ares …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as ares and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$ARES_HOME/profiles" ]; then
    chown -R ares:ares "$ARES_HOME/profiles" 2>/dev/null || true
fi

# Reset ownership of ares-owned top-level state files on every boot.
# The targeted data-volume chown above only covers ares-owned
# *subdirectories*; loose state files living directly under $ARES_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> ares …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged ares runtime then hits
...
# sweep so host-owned files in a bind-mounted $ARES_HOME are never
...
# entries of ares_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$ARES_HOME/$f" ]; then
        chown ares:ares "$ARES_HOME/$f" 2>/dev/null || true
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the ares runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$ARES_HOME/config.yaml" ]; then
    chown ares:ares "$ARES_HOME/config.yaml" 2>/dev/null || true
    chmod 640 "$ARES_HOME/config.yaml" 2>/dev/null || true
fi

# --- Seed directory structure as ares user ---
# Run as ares via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $ARES_HOME values
...
as_ares mkdir -p \
    ...
    "$ARES_HOME/home" \
    "$ARES_HOME/pairing" \
    "$ARES_HOME/platforms/pairing"

# --- Install-method stamp (read by detect_install_method() in ares status) ---
# Preserved from the tini-era entrypoint (PR #27843). Must be written as
# the ares user so ownership matches the file's documented owner.
# tee is invoked directly via s6-setuidgid (no `sh -c` wrapper) for the
# same shell-metacharacter safety described above.
printf 'docker\n' | as_ares tee "$ARES_HOME/.install_method" >/dev/null \
    || true
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$ARES_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        as_ares cp "$INSTALL_DIR/$src" "$ARES_HOME/$dest"
    fi
}
...
if [ -f "$ARES_HOME/.env" ]; then
    chown ares:ares "$ARES_HOME/.env" 2>/dev/null || true
    chmod 600 "$ARES_HOME/.env" 2>/dev/null || true
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $ARES_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `ares update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set ARES_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$ARES_HOME/config.yaml" ]; then
    s6-setuidgid ares "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$ARES_HOME/auth.json" ] && [ -n "${ARES_AUTH_JSON_BOOTSTRAP:-}" ]; then
    printf '%s' "$ARES_AUTH_JSON_BOOTSTRAP" > "$ARES_HOME/auth.json"
    chown ares:ares "$ARES_HOME/auth.json" 2>/dev/null || true
    chmod 600 "$ARES_HOME/auth.json"
fi
...
# can set ARES_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# ARES_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$ARES_HOME/gateway_state.json" ] && \
        [ "${ARES_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    printf '{"gateway_state":"running"}\n' > "$ARES_HOME/gateway_state.json"
    chown ares:ares "$ARES_HOME/gateway_state.json" 2>/dev/null || true
    chmod 644 "$ARES_HOME/gateway_state.json"
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_ares "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/ares/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Ares spawns for the
...
# main-wrapper.sh propagates it into the supervised ``ares`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-ares, dashboard, etc.).
