...
# by /etc/cont-init.d/01-ares-setup (installed by the Dockerfile). This
...
# on the pre-s6 contract "entrypoint.sh sets up state then execs ares"
...
echo "[ares] WARNING: docker/entrypoint.sh is a deprecated shim under " \
...
exec /opt/ares/docker/stage2-hook.sh "$@"
