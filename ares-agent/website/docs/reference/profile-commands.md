...
This page covers all commands related to [Ares profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `ares profile` without a subcommand shows help.
...
```bash
ares profile use <name>
```

Sets `<name>` as the active profile. All subsequent `ares` commands (without `-p`) will use this profile.
...
ares profile use default
```

## `ares profile create`

```bash
ares profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `ares profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `ares update` runs won't re-seed the bundled set, and refuses to combine with `--clone` / `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.ares`), use `ares skills opt-out` / `ares skills opt-in`. |
...
ares profile create work2 --clone --clone-from work
```

## `ares profile describe`

```bash
ares profile describe [<name>] [options]
```
...
ares profile describe researcher

# Set it explicitly
ares profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
ares profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
ares profile describe --all --auto
```

## `ares profile delete`

```bash
ares profile delete <name> [options]
```
...
```bash
ares profile delete mybot
ares profile delete mybot --yes
```
...
```bash
ares profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Ares home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ ares profile show work
Profile: work
Path:    ~/.ares/profiles/work
...
```bash
ares profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Ares installation.
...
```bash
ares profile alias work
# Creates/updates ~/.local/bin/work

ares profile alias work --name mywork
# Creates ~/.local/bin/mywork

ares profile alias work --remove
...
```bash
ares profile rename <old-name> <new-name>
```
...
```bash
ares profile rename mybot assistant
# ~/.ares/profiles/mybot → ~/.ares/profiles/assistant
...
```bash
ares profile export <name> [options]
```
...
```bash
ares profile export work
# Creates work.tar.gz in the current directory

ares profile export work -o ./work-2026-03-29.tar.gz
```

## `ares profile import`

```bash
ares profile import <archive> [options]
```
...
```bash
ares profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

ares profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`ares profile export` / `import` are still the right commands for
...
### `ares profile install`

```bash
ares profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `ares -p telemetry`). |
...
ares profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
ares profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
ares profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
ares profile install ./telemetry/
```
...
Ares version, author, env var requirements, the source URL/path, and
...
`ares profile list` also shows the distribution name and version in a
`Distribution` column, and `ares profile show <name>` / `delete <name>`
...
ares profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
ares profile install https://github.com/your-org/internal-assistant.git
```
...
`ares_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Ares version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Ares can clone from.
3. Tell recipients to run `ares profile install <your-repo-url>`.
...
```bash
ares -p <name> <command> [options]
ares --profile <name> <command> [options]
```

Global flag to run any Ares command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
ares -p work chat -q "Check the server status"
ares --profile dev gateway start
ares -p personal skills list
ares -p work config edit
```

## `ares completion`

```bash
ares completion <shell>
```
...
- `ares profile <TAB>` — subcommands (list, use, create, etc.)
- `ares profile use <TAB>` — profile names
- `ares -p <TAB>` — profile names
