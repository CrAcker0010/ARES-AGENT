...
The ACP adapter wraps Ares' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
ares acp / ares-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.ares/.env
  -> configure stderr logging
  -> construct AresACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'ares-agent[acp]==<version>' ares-acp`, pointed at the `ares-agent` PyPI release.
...
- `allow_once` -> Ares `once`
- `allow_always` -> Ares `always`
- reject options -> Ares `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Ares tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["ares-acp"])
...
Instead it reuses Ares' runtime resolver:

- `acp_adapter/auth.py`
- `ares_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Ares provider/credentials. It also always advertises a terminal setup auth method (`ares-setup`, args `--setup`) so first-run registry clients can open Ares' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.ares/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `ares-acp` toolset definition
- `ares_cli/main.py` — `ares acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `ares-acp` script
