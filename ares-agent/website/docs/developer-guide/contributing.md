...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Ares Agent — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Ares Agent! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Ares core? Start with [Build a Ares Plugin](../guides/build-a-ares-plugin.md)
- Building a new built-in core tool for Ares itself? Start with [Adding Tools](./adding-tools.md)
...
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.ares/.env
```
...
- **Profile-safe paths**: Never hardcode `~/.ares` — use `get_ares_home()` from `ares_constants` for code paths and `display_ares_home()` for user-facing messages. See [AGENTS.md](https://github.com/NousResearch/ares-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.

## Cross-Platform Compatibility

Ares officially supports **Linux, macOS, WSL2, and native Windows (via PowerShell install)**.  Native Windows uses Git Bash (from [Git for Windows](https://git-scm.com/download/win)) for shell commands.  A few features require POSIX kernel primitives and are gated: the dashboard's embedded PTY terminal pane (`/chat` tab) is WSL2-only. If you're doing Windows-heavy dev, run the Windows-footgun lint (`scripts/check-windows-footguns.py`) before pushing.
...
By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/NousResearch/ares-agent/blob/main/LICENSE).
