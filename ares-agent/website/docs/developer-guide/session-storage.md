...
Ares Agent uses a SQLite database (`~/.ares/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `ares_state.py`


## Architecture Overview

```
~/.ares/state.db (SQLite, WAL mode)
...
Multiple ares processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.ares/state.db
...
Default path: `~/.ares/state.db`

This is derived from `ares_constants.get_ares_home()` which resolves to
`~/.ares/` by default, or the value of `ARES_HOME` environment variable.
