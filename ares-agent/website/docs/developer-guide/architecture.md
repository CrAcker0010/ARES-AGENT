...
sidebar_position: 1
title: "Architecture"
description: "Ares Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Ares Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ ares_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # AresCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── ares_state.py           # SQLite session/state database with FTS5
├── ares_constants.py       # ARES_HOME, profile-aware paths
...
│
├── ares_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `ares` subcommands (large file)
...
│   ├── skills_config.py      # ares skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # ares tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # ares gateway start/stop
│
...
```text
User input → AresCLI.process_input()
...
Three discovery sources: `~/.ares/plugins/` (user), `.ares/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `ares plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-ares-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Ares as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`ares -p <name>`) gets its own ARES_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
