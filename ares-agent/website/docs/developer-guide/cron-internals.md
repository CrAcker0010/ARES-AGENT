...
description: "How Ares stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `ares_cli/cron.py` | CLI `ares cron` subcommands |
...
Jobs are stored in `~/.ares/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `ares cron` commands are run or during active CLI sessions.
...
```python
# ~/.ares/scripts/check_competitors.py
...
2. **Environment variable** — `ARES_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.ares/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `ares cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `ares cron` CLI provides direct job management:

```bash
ares cron list                    # Show all jobs
ares cron create                  # Interactive job creation (alias: add)
ares cron edit <job_id>           # Edit job configuration
ares cron pause <job_id>          # Pause a running job
ares cron resume <job_id>         # Resume a paused job
ares cron run <job_id>            # Trigger immediate execution
ares cron remove <job_id>         # Delete a job
```
