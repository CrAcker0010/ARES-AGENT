...
Copy-paste recipes for common automation patterns. Each template uses Ares's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`ares webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `ares webhook subscribe`) |
...
ares cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/ares-agent GitHub repo.

1. Run: gh issue list --repo NousResearch/ares-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
ares webhook subscribe github-pr-review \
  --events "pull_request" \
...
ares cron create "0 9 * * 1" \
  "Scan the NousResearch/ares-agent repo for documentation drift.

1. Run: gh pr list --repo NousResearch/ares-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (ares_cli/commands.py, ares_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (ares_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
ares cron create "0 6 * * *" \
  "Run a dependency security audit on the ares-agent project.

1. cd ~/.ares/ares-agent && source .venv/bin/activate
...
```bash
ares webhook subscribe deploy-verify \
  --events "deployment" \
...
```python title="~/.ares/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Ares-Monitor/1.0"})
...
ares cron create "every 30m" \
  ...
  --script ~/.ares/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
ares cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Ares Agent development." \
...
```bash
ares webhook subscribe github-issues \
  --events "issues" \
...
```bash
ares webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
ares webhook subscribe stripe-payments \
...
```bash
ares cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the ares-agent codebase.
