...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.ares/.env` at the end.
...
4. Fill in:
   - **Name:** `Ares Teams Meeting Pipeline` (or any name you'll recognize).
...
3. **Description:** `ares-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
Put the three values you collected into `~/.ares/.env`:
...
Ares ships a Graph auth smoke-test. From your Ares install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.ares/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `ares gateway restart`.
