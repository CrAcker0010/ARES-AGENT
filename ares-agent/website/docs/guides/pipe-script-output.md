...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `ares send`."
---

# Pipe Script Output to Messaging Platforms

`ares send` is a small, scriptable CLI that pushes a message to any
messaging platform Ares is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | ares send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `ares
...
ares send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
ares send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
ares send --list

# Filter by platform
ares send --list telegram
```
...
Any platform Ares ships adapters for works as a target:
...
`ares send` resolves the message body in this order:

1. **Positional argument** — `ares send --to telegram "hi"`
2. **`--file PATH`** — `ares send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | ares send --to telegram`

When stdin is a TTY (no pipe), Ares does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  ares send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `ares send` reuses your Ares config, the same script works on
any host where Ares is installed — no need to export bot tokens into
...
`ares send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  ares send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | ares send \
...
```bash
./train.py --epochs 200 && \
  ares send --to telegram "training done" || \
  ares send --to telegram "training failed (exit $?)"
```
...
ares send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(ares send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `ares send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `ares send` calls
the platform's REST endpoint directly using credentials from
`~/.ares/.env` and `~/.ares/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `ares gateway start` and retry.
...
The listing is built from `~/.ares/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`ares gateway
...
| Approach | Multi-platform | Reuses Ares creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `ares send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`ares send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `ares send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `ares send` shares with cron delivery.
