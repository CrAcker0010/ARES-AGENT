...
This tutorial walks you through setting up a Telegram bot powered by Ares Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Ares Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.ares/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Ares itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Ares Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_ares_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Ares Agent. DM me for help with code, research, debugging, and more.
   ```
...
# View live logs
journalctl --user -u ares-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `ares gateway install` to update the plist.
:::
...
```bash
ares gateway stop && ares gateway start
```
...
ares pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.ares/.env`:
...
Control how much detail the bot shows when using tools. In `~/.ares/config.yaml`:
...
Customize how the bot communicates by editing `~/.ares/SOUL.md`:

For a full guide, see [Use SOUL.md with Ares](/guides/use-soul-with-ares).
...
```bash
# From the CLI
ares cron list          # View all scheduled jobs
ares cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u ares-gateway` (Linux) or `~/.ares/logs/gateway.log` (macOS) |
| Cron job output | `~/.ares/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.ares/cron/jobs.json` |
| Pairing data | `~/.ares/pairing/` |
| Session history | `~/.ares/sessions/` |
