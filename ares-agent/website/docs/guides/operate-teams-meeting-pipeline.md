...
```bash
ares teams-pipeline token-health
ares teams-pipeline token-health --force-refresh
```
...
```bash
ares teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
ares teams-pipeline maintain-subscriptions
ares teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Ares cron (recommended if you already run the Ares gateway)

Ares ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.ares/scripts/`. First create the script:

```bash
mkdir -p ~/.ares/scripts
cat > ~/.ares/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec ares teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.ares/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
ares cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/ares-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Ares Teams pipeline subscription maintenance
...
ExecStart=/usr/local/bin/ares teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/ares-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Ares Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now ares-teams-pipeline-maintain.timer
systemctl list-timers ares-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/ares teams-pipeline maintain-subscriptions >> /var/log/ares/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.ares/.env` at the top of a wrapper script that crontab calls.
...
```bash
ares teams-pipeline subscriptions   # should show expirationDateTime advanced
ares teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
ares teams-pipeline list
ares teams-pipeline list --status failed
ares teams-pipeline show <job-id>
```

### Replay a stored job

```bash
ares teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
ares teams-pipeline fetch --meeting-id <meeting-id>
ares teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `ares teams-pipeline maintain-subscriptions --dry-run`
- inspect `ares teams-pipeline list --status failed`
...
- run `ares teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `ares teams-pipeline run`
...
- [ ] `ares teams-pipeline validate` returns an OK snapshot
- [ ] `ares teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Ares cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
