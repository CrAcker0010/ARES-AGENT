...
Ares Agent can drive your Mac's desktop — clicking, typing, scrolling,
...
Use `ares computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `ares tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
4. Start a session with the toolset enabled:
   ...
   ```
   or add `computer_use` to your enabled toolsets in `~/.ares/config.yaml`.

## Keeping cua-driver up to date

The cua-driver project ships fixes regularly (e.g. v0.1.6 fixed a Safari
window-focus bug for UTM workflows). Ares refreshes the binary in two
places so you don't get stuck on a stale release:

- **`ares update`** — when you update Ares itself, if `cua-driver` is
...
- **`ares computer-use install --upgrade`** — manual force-refresh.
...
`ares computer-use status` shows the installed version next to the
binary path.
...
Ares applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.ares/config.yaml` if you want every action confirmed.

## Token efficiency

Screenshots are expensive. Ares applies four layers of optimisation:
...
- **Private SPI risk.** Apple can change SkyLight's symbol surface in any
  OS update. Pin the driver version with the `ARES_CUA_DRIVER_VERSION`
...
`ares computer-use install` to fetch the cua-driver binary, or run
`ares tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/NousResearch/ares-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
