...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Ares's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Ares instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
ares memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `ares` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Ares syncs them back at session init.
...
The `ares honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `ares memory setup honcho` (or run `ares memory setup` and pick it from the list); the `ares honcho` subcommand then appears on the next invocation.

```bash
ares memory setup honcho    # Configure Honcho directly (works before activation)
ares honcho status          # Connection status, config, and key settings
ares honcho setup           # Redirects to `ares memory setup` (post-activation alias)
ares honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
ares honcho peer            # Show or update peer names + dialectic reasoning level
ares honcho mode            # Show or set recall mode (hybrid/context/tools)
ares honcho tokens          # Show or set token budget for context and dialectic
ares honcho identity        # Seed or show the AI peer's Honcho identity
ares honcho sync            # Sync Honcho config to all existing profiles
ares honcho peers           # Show peer identities across all profiles
ares honcho sessions        # List known Honcho session mappings
ares honcho map             # Map current directory to a Honcho session name
ares honcho enable          # Enable Honcho for the active profile
ares honcho disable         # Disable Honcho for the active profile
ares honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `ares honcho`

If you previously used the standalone `ares honcho setup`:
...
No re-login or re-setup needed. Run `ares memory setup` and select "honcho" — the wizard detects your existing config.
