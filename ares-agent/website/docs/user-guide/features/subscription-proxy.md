...
chat completions — use your Ares-managed provider subscription as their
...
| Use case | "Use Ares as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Ares stores
the refresh token in `~/.ares/auth.json` — the same place all Ares
...
Model:      Ares-4-70B    # or Ares-4.3-36B, Ares-4-405B
```
...
interface in `ares_cli/proxy/adapters/`.
...
If you see `not logged in`, run `ares portal`. If you see
...
`ares_cli/proxy/adapters/<provider>.py` and registering it in
