...
Ares runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Ares
...
1. Ares captures a baseline of current diagnostics for the file.
...
…). Ares auto-detects the binary on PATH or in
`<ARES_HOME>/lsp/bin/`.
...
`node_modules` tree — Ares installs both packages together when you
run `ares lsp install typescript` or auto-install fires on first
use.

## CLI

```
ares lsp status          # service state + per-server install status
ares lsp list            # registry, optionally --installed-only
ares lsp install <id>    # eagerly install one server
ares lsp install-all     # try every server with a known recipe
ares lsp restart         # tear down running clients
ares lsp which <id>      # print resolved binary path
```

`ares lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <ARES_HOME>/lsp/bin
...
When `install_strategy: auto`, Ares installs binaries into
`<ARES_HOME>/lsp/bin/`. NPM packages land in
`<ARES_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Ares-owned and is
...
Servers are kept alive for the life of the Ares process. There's
...
**`ares lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<ARES_HOME>/lsp/bin/`. Run
`ares lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `ares lsp status`**
...
When `ares lsp status` shows a `Backend warnings` section, install
...
Check `~/.ares/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `ares lsp restart` to clear the set;
