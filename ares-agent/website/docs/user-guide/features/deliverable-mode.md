...
When Ares Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.ares/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.ares/config.yaml` (switchable per session
via `/personality`).
...
If you use Ares' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.ares/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Ares Agent's deliverable
