...
A **profile distribution** packages a complete Ares agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## What this means

Before distributions, sharing a Ares agent meant sending someone:
...
```bash
ares profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `ares profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running Ares in 2026, that's already true.
...
- **You just want to back up a profile on your own machine.** Use [`ares profile export` / `import`](../reference/profile-commands.md#ares-profile-export) — that's what those are for.
...
```bash
ares profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.ares/profiles/research-bot/SOUL.md
...
Create `~/.ares/profiles/research-bot/distribution.yaml`:
...
Recipients who run `ares profile update research-bot` will pull the latest.
...
```bash
ares profile install github.com/you/research-bot --alias
```
...
5. Copies distribution-owned files into `~/.ares/profiles/research-bot/` (or wherever the manifest's `name` resolves).
...
```bash
# GitHub shorthand
ares profile install github.com/you/research-bot

# Full HTTPS
ares profile install https://github.com/you/research-bot.git

# SSH
ares profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
ares profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
ares profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
ares profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
ares profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
ares profile install github.com/acme/support-bot --name support-eu --alias
```
...
```bash
cp ~/.ares/profiles/research-bot/.env.EXAMPLE ~/.ares/profiles/research-bot/.env
...
```bash
ares profile info research-bot
```
...
`ares profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
ares profile update research-bot
```
...
```bash
ares profile delete research-bot
```
...
Path:    ~/.ares/profiles/research-bot
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `ares profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
# Each engineer
ares profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `ares profile update pr-reviewer` and everyone's on the new version within minutes.
...
```bash
# You
cd ~/.ares/profiles/polymarket-trader
# Write a solid README.md at the repo root — GitHub shows it on the repo page
git init && git add . && git commit -m "v1.0"
git tag v1.0.0
# Publish to a public GitHub repo
git remote add origin https://github.com/you/ares-polymarket-trader.git
git push -u origin main --tags

# Anyone
ares profile install github.com/you/ares-polymarket-trader --alias
```
...
You built Ares-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
# Each on-call
ares profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
ares profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `ares profile info <name>` and hold off on updates until you're ready.
:::
...
ares profile info research-bot | grep Version
...
```yaml
# ~/.ares/profiles/research-bot/local/my-overrides.yaml
...
ares profile delete research-bot --yes
ares profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
ares profile update research-bot --force-config --yes
```
...
ares profile install github.com/yourname/forked-research-bot --alias

# Iterate locally in ~/.ares/profiles/forked-research-bot/
...
ares profile install ~/.ares/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
ares profile delete research-bot-test --yes
ares profile install ~/.ares/profiles/research-bot --name research-bot-test
```
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `ares -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `ares profile` — not a parallel command tree.
...
- Reserved profile names (`ares`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`ares profile export` / `import`](../reference/profile-commands.md#ares-profile-export) — local backup / restore (not distribution)
- [Using SOUL with Ares](../guides/use-soul-with-ares.md) — authoring personalities
