...
| | |
|---|---|
| Source | Optional — install with `ares skills install official/blockchain/hyperliquid` |
| Path | `optional-skills/blockchain/hyperliquid` |
| Version | `0.1.0` |
| Author | Hugo Sequier (Hugo-SEQUIER), Ares Agent |
| License | MIT |
...
The script reads `~/.ares/.env` for two optional defaults:
...
Helper script: `~/.ares/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `~/.ares/.env`.
