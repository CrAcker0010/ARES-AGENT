...
| | |
|---|---|
| Source | Optional — install with `ares skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${ARES_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ARES_SKILL_DIR}/scripts/drive.sh` for private Drive storage
...
| `--client {name}`      | Agent name for attribution (e.g. `ares`)    |
