...
| | |
|---|---|
| Source | Optional — install with `ares skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Ares Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Ares terminal calls (desktop app flow only)
...
### Service Account (recommended for Ares)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.ares/.env` (the skill will prompt for this on first load).
...
## Ares Execution Pattern (desktop app flow)

Ares terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/ares-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/ares-op.sock"
