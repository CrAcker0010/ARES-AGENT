...
| | |
|---|---|
| Source | Optional — install with `ares skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Ares or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Ares. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.ares/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.ares/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Ares integration, either:

- configure the server in `~/.ares/config.yaml` using the `native-mcp` skill, or
...
### Ares cannot see the deployed server

The server-building part may be correct while the Ares config is not. Load the `native-mcp` skill and configure the server in `~/.ares/config.yaml`, then restart Ares.
