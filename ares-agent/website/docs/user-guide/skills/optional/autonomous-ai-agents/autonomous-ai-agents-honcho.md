...
description: "Configure and use Honcho memory with Ares -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Ares -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `ares skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Ares

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Ares profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/ares#running-honcho-locally-with-ares

### Verify

```bash
ares honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Ares profile's AI peer
...
Honcho models conversations as interactions between **peers**. Ares creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Ares instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Ares run |
| `global` | Single session across all directories |

Manual override: `ares honcho map my-project-name`
...
Each Ares profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block ares.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `ares.coder` host block in `honcho.json`
...
```bash
ares honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Ares when Honcho memory is active.
...
Config file: `$ARES_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `ares honcho setup`. Ensure `memory.provider: honcho` is in `~/.ares/config.yaml`.

### Memory not persisting across sessions
Check `ares honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `ares profile create <name> --clone`. For existing profiles: `ares honcho sync`.
...
| Command | Description |
|---------|-------------|
| `ares honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `ares honcho status` | Show resolved config, connection test, peer info for active profile |
| `ares honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `ares honcho disable` | Disable Honcho for the active profile |
| `ares honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `ares honcho peers` | Show peer identities across all profiles |
| `ares honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `ares honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `ares honcho sessions` | List known directory-to-session-name mappings |
| `ares honcho map <name>` | Map current working directory to a Honcho session name |
| `ares honcho identity` | Seed AI peer identity or show both peer representations |
| `ares honcho sync` | Create host blocks for all Ares profiles that don't have one yet |
| `ares honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Ares + Honcho |
| `ares memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `ares memory status` | Show active memory provider and config |
| `ares memory off` | Disable external memory provider |
