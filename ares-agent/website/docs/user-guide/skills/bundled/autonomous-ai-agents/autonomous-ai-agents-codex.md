...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`ares-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-ares-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Ares terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Ares itself, `model.provider: openai-codex` uses Ares-managed Codex
OAuth from `~/.ares/auth.json` after `ares auth add openai-codex`. For the
...
When invoking the Codex CLI from a Ares gateway/service context (for example,
