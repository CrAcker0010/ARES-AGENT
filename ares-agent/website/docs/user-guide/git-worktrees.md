...
title: "Git Worktrees"
description: "Run multiple Ares agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Ares Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Ares so each session has a clean, isolated working directory.

## Why Use Worktrees with Ares?

Ares treats the **current working directory** as the project root:

- CLI: the directory where you run `ares` or `ares chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.ares/config.yaml`
...
git worktree add ../repo-feature feature/ares-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/ares-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Ares there:

```bash
cd ../repo-feature

# Start Ares in the worktree
ares
```
...
git worktree add ../repo-experiment-a feature/ares-a
git worktree add ../repo-experiment-b feature/ares-b
```
...
- Works on its own branch (`feature/ares-a` vs `feature/ares-b`).
...
- Ares checkpoint data under `~/.ares/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Ares experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/ares-checkpoints-docs`, `feature/ares-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Ares from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `ares -w` (Automatic Worktree Mode)

Ares has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `ares/ares-<hash>`).
...
```bash
ares -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `ares -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Ares session its own clean checkout.
