...
description: "Chat with Ares from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---
...
For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Ares](/guides/use-voice-mode-with-ares).
...
```bash
ares gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
ares gateway              # Run in foreground
ares gateway setup        # Configure messaging platforms interactively
ares gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo ares gateway install --system   # Linux only: install a boot-time system service
ares gateway start        # Start the default service
ares gateway stop         # Stop the default service
ares gateway status       # Check default service status
ares gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Ares Agent to the latest version |
...
Configure per-platform overrides in `~/.ares/gateway.json`:
...
ares pairing approve telegram XKGH5N7P

# Other pairing commands:
ares pairing list          # View pending + approved users
ares pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Ares appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.ares/config.yaml`:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.ares/config.yaml`:
...
```bash
ARES_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
ares gateway install               # Install as user service
ares gateway start                 # Start the service
ares gateway stop                  # Stop the service
ares gateway status                # Check status
journalctl --user -u ares-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo ares gateway install --system
sudo ares gateway start --system
sudo ares gateway status --system
journalctl -u ares-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Ares will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Ares installations on the same machine (with different `ARES_HOME` directories), each gets its own systemd service name. The default `~/.ares` uses `ares-gateway`; other installations use `ares-gateway-<hash>`. The `ares gateway` commands automatically target the correct service for your current `ARES_HOME`.
:::

### macOS (launchd)

```bash
ares gateway install               # Install as launchd agent
ares gateway start                 # Start the service
ares gateway stop                  # Stop the service
ares gateway status                # Check status
tail -f ~/.ares/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.ares.gateway.plist`. It includes three environment variables:
...
- **ARES_HOME** — scopes the gateway to your Ares installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `ares gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `ARES_HOME` directory gets its own launchd label. The default `~/.ares` uses `ai.ares.gateway`; other installations use `ai.ares.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `ares-cli` | Full access |
| Telegram | `ares-telegram` | Full tools including terminal |
| Discord | `ares-discord` | Full tools including terminal |
| WhatsApp | `ares-whatsapp` | Full tools including terminal |
| Slack | `ares-slack` | Full tools including terminal |
| Google Chat | `ares-google_chat` | Full tools including terminal |
| Signal | `ares-signal` | Full tools including terminal |
| SMS | `ares-sms` | Full tools including terminal |
| Email | `ares-email` | Full tools including terminal |
| Home Assistant | `ares-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `ares-mattermost` | Full tools including terminal |
| Matrix | `ares-matrix` | Full tools including terminal |
| DingTalk | `ares-dingtalk` | Full tools including terminal |
| Feishu/Lark | `ares-feishu` | Full tools including terminal |
| WeCom | `ares-wecom` | Full tools including terminal |
| WeCom Callback | `ares-wecom-callback` | Full tools including terminal |
| Weixin | `ares-weixin` | Full tools including terminal |
| BlueBubbles | `ares-bluebubbles` | Full tools including terminal |
| QQBot | `ares-qqbot` | Full tools including terminal |
| Yuanbao | `ares-yuanbao` | Full tools including terminal |
| Microsoft Teams | `ares-teams` | Full tools including terminal |
| API Server | `ares-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `ares-webhook` | Full tools including terminal |
...
1. **Gateway log** (`~/.ares/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
