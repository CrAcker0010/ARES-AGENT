...
description: "Set up Ares Agent as a Microsoft Teams bot"
---

# Microsoft Teams Setup

Connect Ares Agent to Microsoft Teams as a bot. Unlike Slack's Socket Mode, Teams delivers messages by calling a **public HTTPS webhook**, so your instance needs a publicly reachable endpoint — either a dev tunnel (local dev) or a real domain (production).
...
> Run `ares gateway setup` and pick **Microsoft Teams** for a guided walk-through.
...
Teams delivers @mentions as regular messages with `<at>BotName</at>` tags, which Ares strips automatically before processing.
...
```bash
# devtunnel (Microsoft)
devtunnel create ares-bot --allow-anonymous
devtunnel port create ares-bot -p 3978 --protocol https  # replace 3978 with TEAMS_PORT if changed
devtunnel host ares-bot
...
```bash
ARES_UID=$(id -u) ARES_GID=$(id -g) docker compose up -d gateway
```
...
Alternatively, configure via `~/.ares/config.yaml`:
...
| `No inference provider configured` | Check that `ANTHROPIC_API_KEY` (or another provider key) is set in `~/.ares/.env` |
...
| Tunnel URL changes on restart | devtunnel URLs are persistent if you use a named tunnel (`devtunnel create ares-bot`). ngrok and cloudflared generate a new URL each run unless you have a paid plan — update the bot endpoint with `teams app update` when it changes |
| Teams shows "This bot is not responding" | The webhook returned an error. Check `docker logs ares` for tracebacks |
...
- Store credentials in `~/.ares/.env` with permissions `600` (`chmod 600 ~/.ares/.env`)
