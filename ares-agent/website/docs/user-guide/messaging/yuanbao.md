...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Ares Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Ares to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.ares/.env`:
...
All standard Ares commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.ares/logs/gateway.log`
...
4. Consider enabling verbose logging: `ARES_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Ares automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
ARES_LOG_LEVEL=debug ares gateway
```
...
```bash
ares chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
