...
Connect Ares to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Ares supports two WeCom integration modes:
...
> Run `ares gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Ares decrypts the message, queues it for the agent
...
(Use `ares gateway start` only after `ares gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Ares and the token the
...
in `~/.ares/.env` values around `=` will also break signature checks. After
fixing, restart `ares gateway run`.
...
Check `ares gateway run` logs for the bound host/port. If the adapter bound to
