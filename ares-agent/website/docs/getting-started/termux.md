...
description: "Run Ares Agent directly on an Android phone with Termux"
---

# Ares on Android with Termux

This is the tested path for running Ares Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Ares from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Ares now ships a Termux-aware installer path:
...
- links `ares` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `ares` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/ares" "$PREFIX/bin/ares"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `ares` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.ares/.env`.
...
### `ares doctor` says ripgrep or Node is missing
