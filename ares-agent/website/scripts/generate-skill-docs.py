...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/ares-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    ares_meta = (fm.get("metadata") or {}).get("ares") or {}
    tags = ares_meta.get("tags") or []
    related = ares_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `ares skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Ares Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Ares ships with a large built-in skill library copied into `~/.ares/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Ares also syncs bundled skills on `ares update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.ares/skills/` tree, it is still shipped with Ares; restore it with `ares skills reset <name> --restore`.",
        "",
