/**
 * ChatPage — embeds `ares --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (ares_cli/web_server.py)                          .
 *          ▼                                                              .
...
import { ARES_BASE_PATH, buildWsAuthParam } from "@/lib/api";
...
function buildWsUrl(
  ...
  return `${proto}//${window.location.host}${ARES_BASE_PATH}/api/pty?${qs.toString()}`;
}
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__ARES_SESSION_TOKEN__ &&
    !window.__ARES_AUTH_REQUIRED__
  ...
  useEffect(() => {
    ...
    const gated = !!window.__ARES_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Ares TUI has enabled xterm mouse-events
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[ares-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__ARES_AUTH_REQUIRED__``) this
    ...
    // forwarding them into Ares. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="ares-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
...
declare global {
  interface Window {
    ...
    __ARES_AUTH_REQUIRED__?: boolean;
  }
}
