#!/usr/bin/env python3
"""
Ares Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Ares Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills ares-agent-dev,github-auth
...
os.environ["ARES_QUIET"] = "1"  # Our own modules

import yaml

from ares_cli.fallback_config import get_fallback_chain
from ares_cli.cli_agent_setup_mixin import CLIAgentSetupMixin
from ares_cli.cli_commands_mixin import CLICommandsMixin
...
try:
    from ares_cli.pt_input_extras import (
...
from ares_cli.banner import _format_context_length, format_banner_version_label
...
from ares_cli.browser_connect import (
...
load_ares_dotenv(ares_home=_ares_home, project_env=_project_env)
...
def _resolve_prefill_messages_file(config: Dict[str, Any]) -> str:
    ...
    """
    env_path = os.getenv("ARES_PREFILL_MESSAGES_FILE", "").strip()
    if env_path:
        return env_path
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.ares/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If ARES_IGNORE_USER_CONFIG=1 is set (via ``ares chat --ignore-user-config``),
    the user config at ``~/.ares/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({ARES_HOME}/config.yaml)
    user_config_path = _ares_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("ARES_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see ares_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
    ...
    if config_path.exists():
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                from ares_cli.config import _normalize_root_model_keys
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and AresCLI.__init__ checks "default" first.
    ...
    # Normalize config key: the new config system (ares_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && ares` to control it.
    ...
    # UNLESS we're inside a gateway process (detected by _ARES_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_ARES_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["ARES_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.ares/logs/.
# This ensures CLI sessions produce a log trail even before AIAgent is instantiated.
try:
    from ares_logging import setup_logging
...
try:
    from ares_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``ares --help`` paths inside the chat command flow).  See
...
def get_tool_definitions(*args, **kwargs):
    from ares_cli.mcp_startup import wait_for_mcp_discovery
...
from ares_cli.banner import build_welcome_banner
from ares_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from ares_cli.callbacks import prompt_for_secret
...
def _prepare_deferred_agent_startup() -> None:
    ...
    if _deferred_agent_startup_done:
        return
    if os.environ.get("ARES_DEFER_AGENT_STARTUP") != "1":
        return
    _deferred_agent_startup_done = True
    _accept_hooks = os.environ.get("ARES_ACCEPT_HOOKS", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run ares -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"ares-{short_id}"
    branch_name = f"ares/{wt_name}"
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`ares_cli.config.load_config` (the authoritative loader that
    ...
    try:
        ...
        from ares_constants import get_ares_home as _get_ares_home
        _ares_home_maint = _get_ares_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_ares_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`ares_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``ares/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("ares-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``ares/ares-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``ares -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("ares/ares-") or b.startswith("pr-"))
    ]
...
#
# Detection priority:
#   1. ARES_LIGHT / ARES_TUI_LIGHT env (true/false) — explicit override
#   2. ARES_TUI_THEME=light|dark — explicit theme
#   3. ARES_TUI_BACKGROUND=#RRGGBB — explicit bg hint
#   4. COLORFGBG env (set by xterm/Konsole/urxvt) — bg slot 7/15 = light
#   5. OSC 11 query (\x1b]11;?\x1b\\) — ask the terminal directly
#   6. Default: assume dark (matches the legacy Ares assumption)
#
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        for var in ("ARES_LIGHT", "ARES_TUI_LIGHT"):
            ...
            if _FALSE_RE.match(v):
                ...
                return result
        # 2. Theme hint
        theme = (os.environ.get("ARES_TUI_THEME") or "").strip().lower()
        if theme == "light":
            result = True
            _LIGHT_MODE_CACHE = result
            return result
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("ARES_TUI_BACKGROUND") or ""
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from ares_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
    if getattr(SkinConfig, "_ares_light_mode_hook_installed", False):
        return
    ...
    SkinConfig._ares_light_mode_hook_installed = True  # type: ignore[attr-defined]
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in Ares output it's common to display cron schedules verbatim.
    #
...
from ares_constants import is_termux as _is_termux_environment
...
def _apply_bracketed_paste_timeout_patch() -> None:
    ...
    ``_ares_bp_timeout_patched`` sentinel on the module.
    """
    try:
        ...
        if getattr(_vt100_mod, "_ares_bp_timeout_patched", False):
            return
        ...
        def _patched_vt100_feed(self_parser, data: str) -> None:
            if self_parser._in_bracketed_paste:
                ...
                if end_mark in self_parser._paste_buffer:
                    ...
                    self_parser._ares_bp_start = None
                    if remaining:
                        _patched_vt100_feed(self_parser, remaining)
                else:
                    bp_start = getattr(self_parser, "_ares_bp_start", None)
                    now = time.monotonic()
                    if bp_start is None:
                        self_parser._ares_bp_start = now
        ...
        _vt100_mod._ares_bp_timeout_patched = True
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``AresCLI._busy_command()``.
        """
        yield self

# ASCII Art - ARES-AGENT logo (full width, single line - requires ~95 char terminal)
...
# ASCII Art - Ares Caduceus (compact, fits in left panel)
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "⚕ NOUS ARES - AI Agent Framework"
        tiny_line = "⚕ NOUS ARES"
    else:
        agent_name = _skin.get_branding("agent_name", "Ares Agent") if _skin else "Ares Agent"
        line1 = f"{agent_name} - AI Agent Framework"
        tiny_line = agent_name

    if os.environ.get("ARES_FAST_STARTUP_BANNER") == "1":
        from ares_cli import __release_date__ as _release_date
        from ares_cli import __version__ as _version

        version_line = f"Ares Agent v{_version} ({_release_date})"
    else:
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.ares/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.ares/config.yaml on first use)
...
class AresCLI(CLIAgentSetupMixin, CLICommandsMixin):
    """
    Interactive CLI for the Ares Agent.
    ...
    def __init__(
        ...
        # Read max_tokens from config (env var override: ARES_MAX_TOKENS)
        ...
        self.requested_provider = (
            ...
            or os.getenv("ARES_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("ARES_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("ARES_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        # by `ares chat --ignore-rules` in ares_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("ARES_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("ARES_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        # it's shared across all Ares processes for this ARES_HOME.
        ...
        self._history_file = _ares_home / ".ares_history"
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            return f"⚕ {self.model if getattr(self, 'model', None) else 'Ares'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        if resolved_provider == "copilot":
            try:
                from ares_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from ares_cli.models import normalize_opencode_model_id, opencode_model_api_mode
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from ares_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "⚕ Ares")
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - AresCLI._status_bar_display_width(label)
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``ares doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``ares doctor`` so the banner stays
        small.
        """
        try:
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Ares needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        # Warn if the configured model is a Nous Ares LLM (not agentic)
        from ares_cli.model_switch import is_nous_ares_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_ares_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Nous Research Ares 3 & 4 models are NOT agentic and are not "
                "designed for use with Ares Agent.[/]"
            )
            self._console_print(
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.ares/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from ares_cli.clipboard import save_clipboard_image

        img_dir = get_ares_home() / "images"
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'ares setup' to configure[/]")
        except Exception:
    ...
    def _fast_command_available(self) -> bool:
        try:
            from ares_cli.models import model_supports_fast_mode
        except Exception:
            return False
        agent = getattr(self, "agent", None)
    ...
    def show_help(self):
        ...
        from ares_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from ares_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Ares!{_RST}")
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from ares_cli.main import _relative_time
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("ARES_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.ares/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``ares --resume <id>``
        ...
        saved_dir = get_ares_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"ares_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: ares --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from ares_cli.curses_ui import curses_single_select
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `ares model` and gateway pickers).
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from ares_cli.model_switch import switch_model, parse_model_flags
    ...
    def _handle_codex_runtime(self, cmd_original: str) -> None:
        ...
        Usage:
            ...
            /codex-runtime auto                  — Ares default (chat_completions)
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_ares_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_ares_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in ares_cli/commands.py instead of touching every dispatch site.
        from ares_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            from ares_cli.main import _print_version_info
            ...
            from ares_cli.config import reload_env
            count = reload_env()
            ...
            try:
                from ares_cli.plugins import get_plugin_manager
                mgr = get_plugin_manager()
                plugins = mgr.list_plugins()
                if not plugins:
                    print("No plugins installed.")
                    print(f"Drop plugin directories into {display_ares_home()}/plugins/ to get started.")
                else:
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from ares_cli.commands import COMMANDS
    ...
    def _toggle_verbose(self):
        ...
        from ares_cli.colors import Colors as _Colors
        labels = {
    ...
    def _is_session_yolo_active(self) -> bool:
        ...
        ``ARES_YOLO_MODE`` into ``_YOLO_MODE_FROZEN`` before tool imports
        happen.
        """
        try:
            from tools.approval import (
    ...
    def _toggle_yolo(self):
        ...
        ``ARES_YOLO_MODE`` here — that env var is read once at module import
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        * ``/compress here [N]`` — boundary-aware compression. Summarize
        ...
        from ares_cli.partial_compress import (
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by ares_logging not
    ...
    def _print_nous_credits_block(self) -> bool:
        ...
        wall-clock-bounded inside the helper; also honors ARES_DEV_CREDITS_FIXTURE
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from ares_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.ares/skills/ and queue a note for the
        next user turn.
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _ares_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard ARES_HOME, so no -p needed.
            try:
            ...
            print(f"  ares --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  ares -c \"{session_title}\"{profile_flag}")
            print()
            ...
            try:
                from ares_cli.skin_engine import get_active_goodbye
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from ares_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Ares Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Ares Agent! Type your message or /help for commands."
        ...
        try:
            from ares_cli.model_switch import prewarm_picker_cache_async
        ...
        try:
            ...
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    ...
                    f"(ARES_REDACT_SECRETS={_redact_raw}). "
        ...
        # after an OpenClaw→Ares migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from ares_cli.config import get_config_path as _get_cfg_path_resid
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in ares_cli.pt_input_extras
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Ares binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from ares_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Ares Agent")
        ...
        # Dynamic prompt: shows Ares symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Ares needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Ares needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Ares needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = AresCLI._compute_model_picker_viewport(
        ...
        try:
            ...
            if not getattr(_pt_renderer, "_ares_osd_patched", False):
                ...
                _pt_renderer._ares_osd_patched = True
        except Exception:
            pass
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``ARES_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("ARES_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the ares process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: ares setup"
            )
            _run_cleanup()
...
def _run_kanban_goal_loop_q(cli: "AresCLI", first_response: str) -> None:
    """Drive a kanban goal_mode worker through the Ralph-style goal loop.

    Called from the quiet single-query path AFTER the worker's first turn,
    only when ``ARES_KANBAN_GOAL_MODE`` is set (dispatcher-spawned
    ...
    task_id = (_os.environ.get("ARES_KANBAN_TASK") or "").strip()
    if not task_id:
        return

    from ares_cli import kanban_db as _kb
    from ares_cli.goals import run_kanban_goal_loop as _run_loop, DEFAULT_MAX_TURNS as _DEF_TURNS
...
def main(
    ...
    """
    Ares Agent CLI - Interactive AI Assistant
    ...
    os.environ["ARES_INTERACTIVE"] = "1"
    
    # Handle gateway mode (messaging + cron)
    if gateway:
        import asyncio
        from gateway.run import start_gateway
        print("Starting Ares Gateway (messaging platforms)...")
    ...
    # Default to ares-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
    ...
    # mode registers its own inside AresCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  ARES_SIGTERM_GRACE overrides the 1.5 s
    ...
    if query or image:
        ...
        # Kanban workers spawn with ``ares chat -q "work kanban task <id>"``;
        ...
        _kanban_task_id = os.environ.get("ARES_KANBAN_TASK", "").strip()
        if _kanban_task_id:
            try:
                from ares_cli import kanban_db as _kb
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if cli._init_agent(
                    ...
                    # machine-readable (no styled "Ares" box, no tool-gen
                    ...
                    if os.environ.get("ARES_KANBAN_GOAL_MODE") == "1":
                        try:
                    ...
                    if isinstance(result, dict) and result.get("failed"):
                        _exit_code = 1
                        if os.environ.get("ARES_KANBAN_TASK") and result.get(
                            "failure_reason"
                        ) in ("rate_limit", "billing"):
                            try:
                                from ares_cli.kanban_db import (
            ...
            # Single-query mode (`ares chat -q "…"`): skip the welcome
