...
``{title, url, description, position}`` rows every other Ares web
provider produces.
...
prefers Ares-managed xAI Grok OAuth (via ``ares auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.ares/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``ares tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `ares auth` to sign in with "
