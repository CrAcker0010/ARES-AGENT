...
Let the ares agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where ares runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `ares meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `ares meet` CLI |
...
| `cli.py` | `ares meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$ARES_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `ares meet node {run,list,approve,remove,status,ping}` |
...
ares meet install                                      # pip + Chromium
ares meet setup                                        # preflight
ares meet auth                                         # optional
ares meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
ares meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.ares/.env
ares meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
ares meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
ares meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.ares/.env
ares meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, ares will **not** switch your system audio input automatically — the
...
ares plugins enable google_meet
ares meet node run --display-name my-mac --host 0.0.0.0 --port 18789
