...
session-scoped state file under ``$ARES_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $ARES_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_ares_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this ares install, leave it first —
    ...
    env = os.environ.copy()
    env["ARES_MEET_URL"] = url
    env["ARES_MEET_OUT_DIR"] = str(out)
    env["ARES_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["ARES_MEET_HEADED"] = "1"
    if auth_state:
        env["ARES_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["ARES_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if ARES_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["ARES_MEET_MODE"] = mode
    if realtime_model:
        env["ARES_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["ARES_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["ARES_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["ARES_MEET_REALTIME_KEY"] = realtime_api_key
