...
Auto-tracks and cleans up ephemeral files created during Ares Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ARES_HOME` and `/tmp/ares-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ARES_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `ARES_HOME` or `/tmp/ares-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ARES_HOME/disk-cleanup/` is itself excluded
- `$ARES_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
