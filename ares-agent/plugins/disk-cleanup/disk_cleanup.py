"""disk_cleanup — ephemeral file cleanup for Ares Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under ARES_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ARES_HOME and /tmp/ares-*
Never touches: ~/.ares/logs/ or any system directory.
"""
...
try:
    ...
    def get_ares_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ARES_HOME") or "").strip()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ARES_HOME/logs/``."""
    return get_ares_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ARES_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ARES_HOME or ``/tmp/ares-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    ares_home = get_ares_home()
    try:
        path.resolve().relative_to(ares_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/ares-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("ares-"):
        return True
    return False
...
# Paths under $ARES_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    """
    # Lazily build the set once per process so ARES_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        ares_home = get_ares_home()
        for parent in ("cron", "cronjobs"):
            base = ares_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ARES_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ARES_HOME (but leave ARES_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "ares-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(ares_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == ares_home:
                continue
            try:
                rel_parts = dirpath.relative_to(ares_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    ares_home = get_ares_home()
    try:
        rel = path.resolve().relative_to(ares_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "ares-agent",
        }:
            return None
        ...
        # Path isn't under ARES_HOME (e.g. /tmp/ares-*) — fall through.
        pass
