...
def _load_supermemory_config(ares_home: str) -> dict:
    config = _default_config()
    config_path = Path(ares_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, ares_home: str) -> None:
    config_path = Path(ares_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "ares"},
        )
    ...
    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        ...
        merged = {"sm_source": "ares", **(metadata or {})}
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `ares memory setup`.
        # All other options are documented for $ARES_HOME/supermemory.json
    ...
    def save_config(self, values, ares_home):
        ...
        _save_supermemory_config(sanitized, ares_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from ares_constants import get_ares_home
        self._ares_home = kwargs.get("ares_home") or str(get_ares_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._ares_home)
