...
Config file: `$ARES_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `ares` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `ares-{identity}` → `ares-coder`). |
...
All Supermemory API calls send `x-sm-source: ares`, and document writes stamp
`metadata.sm_source: ares`. This is a **functional routing key, not telemetry**:
it groups Ares-written memories into a dedicated "Ares" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Ares profile:

```json
{
  "container_tag": "ares-{identity}"
}
```

For a profile named `coder`, this resolves to `ares-coder`. The default profile resolves to `ares-default`. Without `{identity}`, all profiles share the same container.
