...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/ares>
...
```bash
ares memory setup honcho   # configure Honcho directly (works on a fresh install)
ares memory setup          # generic picker, choose Honcho from the list
```

Or manually:
```bash
ares config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.ares/.env
```

> `ares honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `ares memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ARES_HOME/honcho.json` | Profile-local (isolated Ares instances) |
| 2 | `~/.ares/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Ares profile: `ares` (default) or `ares_<profile>`.
...
**Deployment shapes** (`ares memory setup honcho` asks one prompt to set these):

- **Single-operator** — `pinUserPeer: true`. All gateway users → `peerName`. Recommended for personal use where you connect Ares to your own Telegram/Discord/etc.
...
| 4 | `per-session` strategy | Ares session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`ares-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`ares`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-ares-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening ares in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Ares session ID (timestamp + hex). Every `ares` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Ares profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`ares`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Ares profile: `ares` (default) or `ares_<profile>` (e.g. `ares -p coder` -> host key `ares_coder`). Older `ares.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `ARES_HONCHO_HOST` | Host key override |
...
|---------|-------------|
| `ares memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `ares honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `ares memory setup`) |
| `ares honcho status` | Show resolved config for active profile |
| `ares honcho enable` / `disable` | Toggle Honcho for active profile |
| `ares honcho mode <mode>` | Change recall or observation mode |
| `ares honcho peer --user <name>` | Update user peer name |
| `ares honcho peer --ai <name>` | Update AI peer name |
| `ares honcho tokens --context <N>` | Set context token budget |
| `ares honcho tokens --dialectic <N>` | Set dialectic max chars |
| `ares honcho map <name>` | Map current directory to a session name |
| `ares honcho sync` | Create host blocks for all Ares profiles |
