...
``_check_spotify_available()`` — when the user has not run ``ares auth
spotify``, the tools remain registered (so they appear in ``ares
...
The Spotify auth flow (``ares auth spotify``), CLI plumbing, and docs
