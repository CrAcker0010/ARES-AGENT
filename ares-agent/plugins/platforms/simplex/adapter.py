"""SimpleX Chat platform adapter (Ares plugin).
...
This adapter ships as a Ares platform plugin under
``plugins/platforms/simplex/``. The Ares plugin loader scans the
...
discoverable and `ares setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Ares — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"ares-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``ares setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.ares/.env`` via ``ares_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("ares_cli.config not available; set SIMPLEX_* vars manually in ~/.ares/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `ares gateway status` without
