"""
LINE Messaging API platform adapter for Ares Agent.
...
to Ares Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``ARES_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            ares_home = Path.home().joinpath(".ares").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``ares status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``ares status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``ares setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.ares/.env`` via ``ares_cli.config``.
    """
    print()
    ...
    try:
        from ares_cli.config import get_env_var, set_env_var
    except ImportError:
        print("ares_cli.config not available; set LINE_* vars manually in ~/.ares/.env")
        return
