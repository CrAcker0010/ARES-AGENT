"""
Microsoft Teams platform adapter for Ares Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Ares"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        ares_action = data.get("ares_action", "")
        session_key = data.get("session_key", "")

        if not ares_action or not session_key:
        ...
        choice = choice_map.get(ares_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="ares_approve",
                    data={**btn_data_base, "ares_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="ares_approve",
                    data={**btn_data_base, "ares_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="ares_approve",
                    data={**btn_data_base, "ares_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="ares_approve",
                    data={**btn_data_base, "ares_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Ares\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.ares/.env")
