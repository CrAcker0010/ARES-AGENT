...
# ``model_tools`` import time, so ``ares status``, ``ares chat``, etc.
...
# ``ares_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Ares is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a ares MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${ARES_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _ares_home = _get_ares_home()
        except (ModuleNotFoundError, ImportError):
            _ares_home = _Path.home() / ".ares"
        self._thread_count_store = _ThreadCountStore(
            _ares_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Ares is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("ARES_HOME", str(_Path.home() / ".ares"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${ARES_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a ares MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Ares is thinking…' marker message.
        ...
        "Ares is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Ares is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Ares is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Ares is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Ares is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``ares setup``.

    The setup wizard at ``ares_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.ares/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. ares-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.ares/.env")
    print_info("Restart the gateway: ares gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``ares cron`` running as a
    separate process from ``ares gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Ares plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'ares-agent[google_chat]'",
        ...
        # the "Ares is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Ares is thinking…' marker message "
