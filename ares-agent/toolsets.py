...
_ARES_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (ARES_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `ares tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (ARES_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full Ares toolsets (CLI + messaging platforms)
    #
    ...
    "ares-cli": {
        ...
        "tools": _ARES_CORE_TOOLS,
        "includes": []
    },

    "ares-cron": {
        # Mirrors ares-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `ares tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as ares-cli; gated by `ares tools`",
        "tools": _ARES_CORE_TOOLS,
        "includes": []
    },
    ...
    "ares-email": {
        "description": "Email bot toolset - interact with Ares via email (IMAP/SMTP)",
        "tools": _ARES_CORE_TOOLS,
        "includes": []
    },
    ...
    "ares-sms": {
        "description": "SMS bot toolset - interact with Ares via SMS (Twilio)",
        "tools": _ARES_CORE_TOOLS,
        "includes": []
    },
    ...
    "ares-webhook": {
        ...
        "tools": _ARES_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "ares-gateway": {
        ...
        "tools": [],
        "includes": ["ares-telegram", "ares-discord", "ares-whatsapp", "ares-slack", "ares-signal", "ares-bluebubbles", "ares-homeassistant", "ares-email", "ares-sms", "ares-mattermost", "ares-matrix", "ares-dingtalk", "ares-feishu", "ares-wecom", "ares-wecom-callback", "ares-weixin", "ares-qqbot", "ares-webhook", "ares-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (ares-<name>).
        # Gives them _ARES_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("ares-"):
            platform_name = name[len("ares-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_ARES_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
