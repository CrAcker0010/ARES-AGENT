# nix/desktop.nix — Ares Desktop (Electron) app build + wrapper
#
# `aresAgent` is the fully-built `.#default` package — it ships the
# `ares` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `ARES_DESKTOP_ARES` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Ares CLI").
# No reimplementation of the agent resolution in this wrapper.
{ pkgs, lib, stdenv, makeWrapper, aresNpmLib, electron, aresAgent, ... }:
let
  npm = aresNpmLib.mkNpmPassthru { folder = "apps/desktop"; attr = "desktop"; pname = "ares-desktop"; };
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/ares-desktop $out/bin
    cp -r ${renderer}/* $out/share/ares-desktop/

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # ARES_DESKTOP_ARES to the absolute path of the nix-built `ares`
    # binary so the desktop's resolver step 4 ("existing Ares CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/ares-desktop \
      --add-flags "$out/share/ares-desktop" \
      --set ARES_DESKTOP_ARES "${lib.getExe aresAgent}" \
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Ares Agent";
    homepage = "https://github.com/NousResearch/ares-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "ares-desktop";
  };
}
