...
let
  ...
  # at by ARES_BUNDLED_LOCALES so the wrapped binary always resolves human
...
stdenv.mkDerivation (finalAttrs: {
  ...
  installPhase = ''
    ...
    cp -r ${bundledLocales} $out/share/ares-agent/locales
    ...
    ${lib.concatMapStringsSep "\n"
      (name: ''
        makeWrapper ${aresVenv}/bin/${name} $out/bin/${name} \
          ...
          --set ARES_BUNDLED_LOCALES $out/share/ares-agent/locales \
  ...
  passthru = {
    ...
    # `aresDesktop` references `finalAttrs.finalPackage` (this whole
    # derivation, after all overrides are applied) so the desktop wrapper
    # can prepend its `/bin` to PATH.  The desktop's resolver step 4
    # ("existing ares on PATH") then picks up the fully wrapped
    # `ares` binary — venv with all deps, bundled skills/plugins,
    # runtime PATH (ripgrep/git/ffmpeg/etc).  No re-implementation
    # of the agent resolution in the desktop wrapper.
    aresDesktop = callPackage ./desktop.nix {
      inherit aresNpmLib electron;
      aresAgent = finalAttrs.finalPackage;
    };
