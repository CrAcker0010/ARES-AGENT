...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      ares-agent = self'.packages.default;
      aresVenv = ares-agent.aresVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "ares-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${aresVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "ares-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "ares-build-package" { } ''
          echo "PASS: package built at ${ares-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "ares-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "ares-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${ares-agent}/bin/ares || (echo "FAIL: ares binary missing"; exit 1)
          test -x ${ares-agent}/bin/ares-agent || (echo "FAIL: ares-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${ares-agent}/bin/ares version 2>&1 | grep -qi "ares" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "ares-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in ares ares-agent ares-acp; do
            test -x ${ares-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "ares-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking ares --help ==="
          ${ares-agent}/bin/ares --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${ares-agent}/bin/ares --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "ares-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${ares-agent}/share/ares-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${ares-agent}/share/ares-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "ARES_BUNDLED_SKILLS" ${ares-agent}/bin/ares || \
            (echo "FAIL: ARES_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: ARES_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "ares-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${ares-agent}/share/ares-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${ares-agent}/share/ares-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "ARES_BUNDLED_PLUGINS" ${ares-agent}/bin/ares || \
            (echo "FAIL: ARES_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: ARES_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "ares-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${ares-agent}/share/ares-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${ares-agent}/share/ares-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${ares-agent}/share/ares-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "ARES_BUNDLED_LOCALES" ${ares-agent}/bin/ares || \
            (echo "FAIL: ARES_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: ARES_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (ARES_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && ARES_BUNDLED_LOCALES=${ares-agent}/share/ares-agent/locales \
            ${aresVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with ARES_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${aresVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${aresVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without ARES_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "ares-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${ares-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${ares-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "ARES_TUI_DIR" ${ares-agent}/bin/ares || \
            (echo "FAIL: ARES_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: ARES_TUI_DIR set in wrapper"
        ...
        # Verify ARES_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        ares-node = pkgs.runCommand "ares-node-version" { } ''
          set -e
          echo "=== Checking ARES_NODE in wrapper ==="
          grep -q "ARES_NODE" ${ares-agent}/bin/ares || \
            (echo "FAIL: ARES_NODE not set in wrapper"; exit 1)
          echo "PASS: ARES_NODE present in wrapper"

          ARES_NODE=$(sed -n "s/^export ARES_NODE='\(.*\)'/\1/p" ${ares-agent}/bin/ares)
          test -x "$ARES_NODE" || (echo "FAIL: ARES_NODE=$ARES_NODE not executable"; exit 1)
          echo "PASS: ARES_NODE executable at $ARES_NODE"

          NODE_MAJOR=$("$ARES_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All ARES_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify ARES_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "ares-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(ARES_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking ARES_MANAGED guards ==="
          check_blocked "config set" ${ares-agent}/bin/ares config set model foo
          check_blocked "config edit" ${ares-agent}/bin/ares config edit
        ...
        extra-python-packages = let
          ...
          aresWithExtra = ares-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "ares-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${aresWithExtra}/bin/ares || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${aresWithExtra}/bin/ares || \
          ...
          if grep -q "PYTHONPATH" ${ares-agent}/bin/ares; then
        ...
        extra-dependency-groups = let
          aresWithGroups = ares-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "ares-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${aresWithGroups}"
          echo "venv: ${aresWithGroups.aresVenv}"
        ...
        messaging-variant = pkgs.runCommand "ares-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.aresVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "ares-config-roundtrip" {
          ...
          merge_and_load() {
            ...
            ${configMergeScript} ${nixSettings} "$ares_home/config.yaml"
            ${aresVenv}/bin/python3 -c '
import json, sys
