# nix/nixosModules.nix — NixOS module for ares-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: ares runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $ARES_HOME/.env
# and read by ares at startup — no container recreation needed for env changes.
#
# Tool resolution: the ares wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.ares-agent;
    effectivePackage =
    ...
    ares-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-ares-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "ares-config-attrs";
      description = "Ares YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "ares-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # ARES_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "ares-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'ARES_DOC_EOF'\n${value}\nARES_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "ares-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/ares";
    ...
    # ares user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "ares-container-entrypoint" ''
      set -eu

      ARES_UID="''${ARES_UID:?ARES_UID must be set}"
      ARES_GID="''${ARES_GID:?ARES_GID must be set}"

      # ── Group: ensure a group with GID=$ARES_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$ARES_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="ares"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$ARES_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$ARES_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$ARES_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$ARES_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="ares"
        TARGET_HOME="/home/ares"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$ARES_UID" -g "$ARES_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$ARES_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$ARES_UID:$ARES_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure ARES_HOME is owned by the target user.
      ...
      if [ -n "''${ARES_HOME:-}" ] && [ -d "$ARES_HOME" ]; then
        find "$ARES_HOME" \! -user "$ARES_UID" -exec chown "$ARES_UID:$ARES_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/ares-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/ares-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/ares ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/ares
        chmod 0440 /etc/sudoers.d/ares
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$ARES_UID" --regid="$ARES_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $ARES_HOME/.env.
    ...
    # Default: /var/lib/ares/workspace → /data/workspace.
  ...
  in {
    options.services.ares-agent = with lib; {
      enable = mkEnableOption "Ares Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = ares-agent;
        description = "The ares-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/ares";
        description = "State directory. Contains .ares/ subdir (ARES_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Ares config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $ARES_HOME/.env at activation time.
          Ares reads this file on every startup via load_ares_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $ARES_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $ARES_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `ares gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the ares user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the ares plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Ares discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          ares_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as ares (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in ares-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the ares CLI to environment.systemPackages and export
          ARES_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.ares symlink to the service
            stateDir. These users are automatically added to the ares group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.ares-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the ares CLI to system PATH and export ARES_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.ares/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.ARES_HOME = "${cfg.stateDir}/.ares";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.ares-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the ares user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.ares-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed ares
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure ares is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.ares        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.ares/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.ares/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.ares/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.ares/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.ares/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."ares-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.ares
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.ares ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.ares ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.ares -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.ares/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.ares/$_subdir"
            chmod 2770 "${cfg.stateDir}/.ares/$_subdir"
            find "${cfg.stateDir}/.ares/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # ares-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.ares/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.ares/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.ares/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.ares/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.ares/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.ares/.managed
          chmod 0644 ${cfg.stateDir}/.ares/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.ares/.container-mode <<'ARES_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    ares_bin=${containerDataDir}/current-package/bin/ares
    ARES_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.ares/.container-mode
            chmod 0644 ${cfg.stateDir}/.ares/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.ares/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.ares";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.ares" ]; then
                  rm -f "${symlinkPath}"
                  echo "ares-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.ares -> stateDir/.ares for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.ares";
                target = "${cfg.stateDir}/.ares";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "ares-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.ares/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.ares/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.ares/auth.json
              fi
            ''}
          ''}
          ...
          # Ares reads $ARES_HOME/.env at startup via load_ares_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.ares/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'ARES_NIX_ENV_EOF'
    ${envFileContent}
    ARES_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.ares/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.ares/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.ares/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.ares-agent = {
          description = "Ares Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $ARES_HOME/.env by the activation script. load_ares_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/ares"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the ares group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.ares-agent = {
          description = "Ares Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ARES_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              ARES_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env ARES_UID="$ARES_UID" \
                --env ARES_GID="$ARES_GID" \
                --env ARES_HOME=${containerDataDir}/.ares \
                --env ARES_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/ares gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
