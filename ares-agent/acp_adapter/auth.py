"""ACP auth helpers — detect and advertise Ares authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "ares-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Ares runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Ares can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Ares.
    ...
    not have Ares provider credentials configured yet, so Ares always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Ares using the currently configured "
