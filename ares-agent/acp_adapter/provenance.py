...
This is an additive Ares extension surfaced under ACP ``_meta.ares`` so
...
compression rotates the internal Ares head, ``build_session_provenance`` lets
...
def build_session_provenance(
    ...
    *,
    ...
    """Build ``_meta.ares.sessionProvenance`` for an ACP session.

    Args:
        db: A ``SessionDB`` (must expose ``get_session``).
        acp_session_id: The stable ACP/editor-facing session handle.
        current_ares_session_id: The live internal Ares DB session id
            (``state.agent.session_id``).
        previous_ares_session_id: The internal id from before the most recent
            turn, when known. Supplied by ``prompt()`` to flag a rotation.

    Returns:
        A dict suitable for ``{"ares": {"sessionProvenance": <dict>}}`` under
        ACP ``_meta``, or ``None`` if the session can't be read.
    """
    try:
        row = db.get_session(current_ares_session_id)
    except Exception:
        return None
    if not row:
        return None
    ...
    root_id = current_ares_session_id
    compression_depth = 0
    cursor_parent = parent_id
    seen = {current_ares_session_id}
    ...
    rotated = bool(
        ...
        and previous_ares_session_id != current_ares_session_id
    )

    provenance: Dict[str, Any] = {
        "acpSessionId": acp_session_id,
        "currentAresSessionId": current_ares_session_id,
        "rootAresSessionId": root_id,
        "parentAresSessionId": parent_id,
        "sessionKind": "continuation" if is_continuation else "root",
        "compressionDepth": compression_depth,
    }
    if previous_ares_session_id:
        provenance["previousAresSessionId"] = previous_ares_session_id
    if rotated:
...
def session_provenance_meta(
    ...
    *,
    ...
    """Return a ready ``_meta`` payload: ``{"ares": {"sessionProvenance": ...}}``."""
    ...
    if prov is None:
        return None
    return {"ares": {"sessionProvenance": prov}}
