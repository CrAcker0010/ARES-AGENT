"""CLI entry point for the ares-agent ACP adapter.

Loads environment variables from ``~/.ares/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from ARES_HOME (default ``~/.ares``)."""
    from ares_cli.env_loader import load_ares_dotenv

    ares_home = get_ares_home()
    loaded = load_ares_dotenv(ares_home=ares_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", ares_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="ares-acp",
        description="Run Ares Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Ares version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _run_check() -> None:
    ...
    from acp_adapter.server import AresACPAgent  # noqa: F401

    print("Ares ACP check OK")


def _run_setup() -> None:
    from ares_cli.main import main as ares_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "ares", "model"]
        ares_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``ares postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting ares-agent ACP adapter")
    ...
    import acp
    from .server import AresACPAgent
