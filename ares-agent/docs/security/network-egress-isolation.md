...
When running Ares inside Docker, the default `network_mode: host` gives the
...
The Ares [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   ARES_UID=$(id -u) ARES_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=ares,ares-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://ares-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Ares trust model and vulnerability reporting
