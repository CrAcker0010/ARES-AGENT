...
Ares supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.ares/config.yaml`:
...
Or set the env var: `ARES_KANBAN_DISPATCH_IN_GATEWAY=false`
