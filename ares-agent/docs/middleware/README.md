...
Ares middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Ares'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Ares calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Ares retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Ares guardrails,
...
- `telemetry_schema_version`: currently `ares.observer.v1`
- `middleware_schema_version`: currently `ares.middleware.v1`
...
Ares stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Ares runs
them as a nested chain in registration order. Middleware failures are fail-open:
Ares logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Ares applies middleware in this order:
...
For each tool call, Ares applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Ares pre-execution path against the effective arguments:
...
```bash
ares plugins enable <plugin-name>
```

For isolated local testing, use one `ARES_HOME` for plugin enablement and the
agent run:

```bash
export ARES_HOME=/tmp/ares-middleware-test
mkdir -p "$ARES_HOME"
ares plugins enable <plugin-name>
ares chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run ares plugins enable <plugin-name>
uv run ares chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["ares_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/ares-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Ares expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Ares treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Ares preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Ares does not convert downstream
