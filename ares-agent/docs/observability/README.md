...
Ares observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Ares' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Ares catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Ares only builds
