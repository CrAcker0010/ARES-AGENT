...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``ares tools`` repaint, tool-registration scans,
    ...
    2. ``~/.ares/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_ares_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.ares/.env`` first, then ``os.environ``.

    Wraps :func:`ares_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from ares_cli.config import get_env_value as _ares_get_env_value

        value = _ares_get_env_value(name)
...
def ares_xai_user_agent() -> str:
    """Return a stable Ares-specific User-Agent for xAI HTTP calls."""
    try:
        from ares_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Ares-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Ares-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``ares_cli.config.get_env_value`` so keys
    stored in ``~/.ares/.env`` (the standard Ares location) are honored —
