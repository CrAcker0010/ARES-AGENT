...
Backend is selected during ``ares tools`` setup (web.backend in config.yaml).
When available, Ares can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Ares config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Ares' config/.env layer (``ares config set``, ``ares tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.ares/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``ares tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `ares tools` repaint.
        try:
