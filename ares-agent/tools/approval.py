...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("ARES_YOLO_MODE", ""))
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    """
    try:
        from ares_cli.plugins import invoke_hook
    except Exception:
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("ARES_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("ARES_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("ARES_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set ARES_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind ARES_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds ARES_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("ARES_CRON_SESSION"):
        return False
    if env_var_enabled("ARES_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $ARES_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_ARES_ENV_PATH = (
    r'(?:~\/\.ares/|'
    r'(?:\$home|\$\{home\})/\.ares/|'
    r'(?:\$ares_home|\$\{ares_home\})/)'
    r'\.env\b'
)
# ~/.ares/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _ARES_ENV_PATH; matches the ARES_HOME override form as
# well as ~/.ares/.
_ARES_CONFIG_PATH = (
    r'(?:~\/\.ares/|'
    r'(?:\$home|\$\{home\})/\.ares/|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bares\s+gateway\s+(stop|restart)\b', "stop/restart ares gateway (kills running agents)"),
    (r'\bares\s+update\b', "ares update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `ares gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart ares-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart ares-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(ares|gateway|cli\.py)\b', "kill ares/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill ares` but not
    # `kill -9 $(pgrep -f ares)` because the substitution is opaque
    ...
    # In-place edit of a Ares-managed security file (~/.ares/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_ARES_CONFIG_PATH}|{_ARES_ENV_PATH})', "in-place edit of Ares config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_ARES_CONFIG_PATH}|{_ARES_ENV_PATH})', "in-place edit of Ares config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_ARES_CONFIG_PATH}|{_ARES_ENV_PATH})', "in-place edit of Ares config/env (perl/ruby)"),
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["ARES_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "ARES_SPINNER_PAUSE" in os.environ:
            del os.environ["ARES_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("ARES_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set ARES_INTERACTIVE or ARES_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("ARES_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("ARES_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("ARES_EXEC_ASK")
