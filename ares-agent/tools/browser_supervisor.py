...
One ``CDPSupervisor`` runs per Ares ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "ares-dialog-bridge.invalid"
...
(() => {
  if (window.__aresDialogBridgeInstalled) return;
  window.__aresDialogBridgeInstalled = true;
  const ENDPOINT = "http://ares-dialog-bridge.invalid/";
