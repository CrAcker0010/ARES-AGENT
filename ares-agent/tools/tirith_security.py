...
it is automatically downloaded from GitHub releases to $ARES_HOME/bin/tirith.
...
def _get_ares_home() -> str:
    """Return the Ares home directory, respecting ARES_HOME env var."""
    return str(get_ares_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_ares_home(), ".tirith-install-failed")
...
def _ares_bin_dir() -> str:
    """Return $ARES_HOME/bin, creating it if needed."""
    d = os.path.join(_get_ares_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ARES_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_ares_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ARES_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ARES_HOME/bin/tirith
    ...
    ares_bin = os.path.join(_ares_bin_dir(), "tirith")
    if os.path.isfile(ares_bin) and os.access(ares_bin, os.X_OK):
        _resolved_path = ares_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return ares_bin
