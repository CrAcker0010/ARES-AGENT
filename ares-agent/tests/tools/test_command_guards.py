...
def _clean_state():
    ...
    saved = {}
    for k in ("ARES_INTERACTIVE", "ARES_GATEWAY_SESSION", "ARES_EXEC_ASK", "ARES_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["ARES_INTERACTIVE"] = "1"
        session_key = os.getenv("ARES_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No ARES_INTERACTIVE or ARES_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
