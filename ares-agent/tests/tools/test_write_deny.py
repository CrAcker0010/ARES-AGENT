...
class TestWriteDenyExactPaths:
    ...
    def test_ares_env(self):
        # ``.env`` under the active ARES_HOME (profile-aware, not just
        # ``~/.ares``) must be write-denied. The hermetic test conftest
        # points ARES_HOME at a tempdir — resolve via get_ares_home()
        # to match the denylist.
        from ares_constants import get_ares_home
        path = str(get_ares_home() / ".env")
        assert _is_write_denied(path) is True

    def test_ares_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.ares/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "ares_root"
        ...
        # Sanity check: ARES_HOME does point to the profile dir, not the root.
        from ares_constants import get_ares_home, get_default_ares_root
        assert get_ares_home() == profile_home
        assert get_default_ares_root() == root
...
class TestWriteAllowed:
    ...
    def test_ares_config_not_env(self):
        path = os.path.join(str(Path.home()), ".ares", "config.yaml")
