...
When ARES_UID is remapped at container boot, ``usermod -u`` only rewrites
files under the ares user's home directory ($ARES_HOME == /opt/data).
Runtime-writable trees under ``/opt/ares`` must be explicitly chowned to the
new UID before services drop privileges. ``/opt/ares/gateway`` is one such
