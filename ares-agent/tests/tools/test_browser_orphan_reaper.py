...
def _make_socket_dir(tmpdir, session_name, pid=None, owner_pid=None):
    ...
    Args:
        ...
        owner_pid: owning ares PID to write to <session>.owner_pid
...
class TestReapOrphanedBrowserSessions:
    ...
    def test_non_ares_dirs_are_ignored(self, fake_tmpdir):
...
class TestOwnerPidCrossProcess:
    ...
    The owner_pid file records which ares process owns a daemon so that
    concurrent ares processes don't reap each other's active browser
