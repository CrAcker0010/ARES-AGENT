...
def fake_ares(tmp_path, monkeypatch):
    """Build a two-profile Ares layout and point ARES_HOME at
    the ares-security profile (matching the original-incident shape).
    """
    ...
    monkeypatch.setenv("ARES_HOME", str(sec_home))

    import ares_constants
    monkeypatch.setattr(ares_constants, "get_default_ares_root", lambda: root)

    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_ares_home_path", lambda: sec_home)
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_ares):
        from tools.file_tools import write_file_tool
        target = fake_ares["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_ares):
        """The May 2026 incident — security-profile session edits default
        profile's skill. Must be blocked."""
        from tools.file_tools import write_file_tool
        target = fake_ares["root"] / "skills" / "shared-skill" / "SKILL.md"
        ...
        assert "ares-security" in result["error"]
        # File untouched.
        assert target.read_text() == original

    def test_cross_profile_True_bypass(self, fake_ares):
    ...
    def test_non_ares_path_unaffected(self, fake_ares, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_ares):
    ...
    def test_cross_profile_patch_bypass(self, fake_ares):
        from tools.file_tools import patch_tool
        target = fake_ares["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_ares):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_ares, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_ares["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up ARES_HOME (set in
        ...
        assert "not found in active profile 'ares-security'" in err
        assert "default" in err
        assert "cross_profile=True" in err

    def test_error_names_multiple_profiles(self, fake_ares, monkeypatch):
        """When the skill exists in TWO other profiles, both should be named."""
        self._make_skill_in_profile(fake_ares["root"], "everywhere-skill")
        self._make_skill_in_profile(fake_ares["coder_home"], "everywhere-skill")
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.ares/profiles/<name>/."""
        # Don't set ARES_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_ares_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_ares_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_ares):
        """When active profile is 'ares-security', the prompt warns
        ...
        assert "Active Ares profile" in src
        assert "cross_profile=True" in src
        assert "~/.ares/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Ares profile: default" in src
        assert "Active Ares profile: {active_profile}" in src
