...
def test_ares_provider_subclass_exists():
    """AresMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _ARES_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _ARES_PROVIDER_CLS is not None
    assert issubclass(_ARES_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_ares_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns AresMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _ARES_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("ARES_HOME", str(tmp_path))
    ...
    assert _ARES_PROVIDER_CLS is not None
    assert isinstance(provider, _ARES_PROVIDER_CLS)
    assert provider._ares_server_name == "srv"
