...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_ares_home_override(self, tmp_path):
        """Auth lookup should read from ARES_HOME/auth.json, not ~/.ares/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".ares").mkdir(parents=True)
...
class TestBackendSelection:
    ...
    ``ares tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
...
class TestCheckWebApiKey:
    ...
    def test_tool_gateway_availability_skips_refresh_for_expired_cached_token(
        ...
        monkeypatch.setattr(
            "ares_cli.auth.resolve_nous_access_token",
            _record_refresh,
        )
