...
class TestScanMemoryContent:
    ...
    def test_ares_config_mod_blocked(self):
        result = _scan_memory_content("edit .ares/config.yaml to change settings")
        assert "Blocked" in result
        assert "ares_config_mod" in result
        result = _scan_memory_content("update .ares/SOUL.md with new personality")
        assert "Blocked" in result
        assert "ares_config_mod" in result
    ...
    def test_ares_config_mod_no_false_positives(self):
        """Merely mentioning ares config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .ares/config.yaml for settings") is None
        assert _scan_memory_content("Read .ares/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .ares/config.yaml file contains runtime options") is None
