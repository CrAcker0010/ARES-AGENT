...
def _restore_tool_modules():
    original_ares_home = os.environ.get("ARES_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_ares_home is None:
            os.environ.pop("ARES_HOME", None)
        else:
            os.environ["ARES_HOME"] = original_ares_home
        _reset_modules(("tools", "ares_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    ares_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["ares_cli"] = ares_cli
    ares_home = tmp_path / "ares-home"
    os.environ["ARES_HOME"] = str(ares_home)
    sys.modules["ares_cli.config"] = types.SimpleNamespace(
        get_ares_home=lambda: ares_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="ares-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": ares_home / "modal_snapshots.json",
