...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad ARES_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: ARES_GATEWAY_SESSION set, a bound session
    key, and isolated gateway queues/callbacks. Yields the session_key."""
    monkeypatch.setenv("ARES_GATEWAY_SESSION", "1")
    monkeypatch.delenv("ARES_INTERACTIVE", raising=False)
    monkeypatch.delenv("ARES_CRON_SESSION", raising=False)
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("ARES_CRON_SESSION", "1")
    monkeypatch.delenv("ARES_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_ares_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "ARES_HOME": "/h", "ARES_PROFILE": "p",
        "ARES_CONFIG": "/c.yaml", "ARES_ENV": "/e",
        # other ARES_* → dropped (broad prefix removed)
        "ARES_BASE_URL": "https://x", "ARES_INTERACTIVE": "1",
        "ARES_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("ARES_HOME", "ARES_PROFILE", "ARES_CONFIG", "ARES_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "ARES_BASE_URL", "ARES_INTERACTIVE", "ARES_KANBAN_DB",
...
def test_env_scrub_logs_dropped_ares_vars(caplog):
    """Dropping a non-allowlisted, non-secret ARES_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset ARES_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "ARES_HOME": "/h",          # allowlisted → kept, not logged
        "ARES_BASE_URL": "https://x",   # dropped → logged
        "ARES_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "ARES_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "ARES_HOME" in out and "PATH" in out
    assert "ARES_BASE_URL" not in out and "ARES_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "ARES_BASE_URL" in msgs and "ARES_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "ARES_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped ARES_* vars."""
    import logging
