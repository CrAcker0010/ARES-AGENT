...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._ares_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(ares_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(ares_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_ares_bin(self):
        """After _INSTALL_FAILED, manual install in ARES_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        ares_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(ares_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(ares_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._ares_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == ares_bin
            assert _tirith_mod._resolved_path == ares_bin
            mock_clear.assert_called_once()
...
class TestAresHomeIsolation:
    def test_ares_bin_dir_respects_ares_home(self):
        """_ares_bin_dir must use ARES_HOME, not hardcoded ~/.ares."""
        from tools.tirith_security import _ares_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"ARES_HOME": tmpdir}):
            result = _ares_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_ares_home(self):
        """_failure_marker_path must use ARES_HOME, not hardcoded ~/.ares."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"ARES_HOME": "/custom/ares"}):
            result = _failure_marker_path()
        assert result == "/custom/ares/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets ARES_HOME; verify it's active."""
        ares_home = os.getenv("ARES_HOME")
        assert ares_home is not None, "ARES_HOME should be set by conftest"
        assert "ares_test" in ares_home, "Should point to test temp dir"

    def test_get_ares_home_fallback(self):
        """Without ARES_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_ares_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove ARES_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".ares")
            result = _get_ares_home()
