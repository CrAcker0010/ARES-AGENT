...
def _isolate(tmp_path, monkeypatch):
    """Isolate ARES_HOME so tests don't touch real config."""
    home = tmp_path / ".ares"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal AresCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = AresCLI.__new__(AresCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["ares-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Ares' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Ares' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Ares' minimum context length."""
