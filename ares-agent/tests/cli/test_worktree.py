...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("ares/ares-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned ares/* and pr-* branches."""

    def test_prunes_orphaned_ares_branch(self, git_repo):
        """ares/ares-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "ares/ares-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "ares/ares-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "ares/ares-deadbeef" in result.stdout
        ...
        assert "ares/ares-deadbeef" in orphaned
        ...
        assert "ares/ares-deadbeef" not in result.stdout
