...
class TestGetDefaultAresRoot:
    ...
    def test_no_ares_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\ares, not ~/.ares."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("ARES_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(ares_constants.sys, "platform", "win32")

        assert get_default_ares_root() == local_appdata / "ares"

    def test_no_ares_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/ares without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("ARES_HOME", raising=False)
        ...
        assert get_default_ares_root() == home / "AppData" / "Local" / "ares"


class TestGetAresHome:
    """Tests for get_ares_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When ARES_HOME is unset on Windows, use %LOCALAPPDATA%\\ares."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("ARES_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(ares_constants.sys, "platform", "win32")
        monkeypatch.setattr(ares_constants, "_profile_fallback_warned", False)

        assert get_ares_home() == local_appdata / "ares"
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".ares"
