...
Override the image with ``ARES_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("ARES_TEST_IMAGE", "ares-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``ARES_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("ARES_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged ares user
# ---------------------------------------------------------------------------
#
# Background: every Ares runtime path inside the container drops to UID
# 10000 (the ``ares`` user) via ``s6-setuidgid ares``. ``docker exec``
...
# fails (PermissionError swallowed) for ares, so a test that ran as root
...
# helpers below run every probe under ``-u ares`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: ares).
