...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    ares_state = ModuleType("ares_state")
    ares_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "ares_state", ares_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Ares"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Ares"
