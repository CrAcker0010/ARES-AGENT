...
class TestGuiMode:
    ...
    def test_gui_log_created(self, ares_home):
        ares_logging.setup_logging(ares_home=ares_home, mode="gui")
        root = logging.getLogger()
    ...
    def test_gui_log_created_after_cli_init(self, ares_home):
        ares_logging.setup_logging(ares_home=ares_home, mode="cli")
        ares_logging.setup_logging(ares_home=ares_home, mode="gui")
    ...
    def test_gui_log_receives_only_gui_components(self, ares_home):
        ares_logging.setup_logging(ares_home=ares_home, mode="gui")

        logging.getLogger("ares_cli.web_server").info("dashboard online")
        ...
        gui_log = ares_home / "logs" / "gui.log"
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        ...
        # ares_plugins facility (plugin-installed gateway adapters log
        # under that prefix).
        assert ("gateway", "ares_plugins") == ares_logging.COMPONENT_PREFIXES["gateway"]
    ...
    def test_gui_prefix(self):
        prefixes = ares_logging.COMPONENT_PREFIXES["gui"]
        assert "ares_cli.web_server" in prefixes
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> ares_logging._ManagedRotatingFileHandler:
        handler = ares_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that ares_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        handler.flush()
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        ares_logging.setup_logging(ares_home=ares_home, mode="gateway")
        gw_path = ares_home / "logs" / "gateway.log"
        rotated = ares_home / "logs" / "gateway.log.1"
...
class TestSafeStderr:
    ...
    def test_returns_stderr_on_utf8_system(self, monkeypatch):
        ...
        result = ares_logging._safe_stderr()
