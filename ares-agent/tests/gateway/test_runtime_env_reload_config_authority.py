...
ARES_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env ARES_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_max_turns(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(gateway_run, "_ares_home", ares_home)
    monkeypatch.setenv("ARES_MAX_ITERATIONS", "9000")
    monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)

    gateway_run._reload_runtime_env_preserving_config_authority()

    assert os.environ["OPENROUTER_API_KEY"] == "fresh-key"
    assert os.environ["ARES_MAX_ITERATIONS"] == "9000"
...
def test_reload_runtime_env_keeps_env_max_iterations_when_config_omits_key(
    ...
    ares_home = tmp_path / ".ares"
    ares_home.mkdir()
    (ares_home / "config.yaml").write_text(yaml.safe_dump({"agent": {}}), encoding="utf-8")
    (ares_home / ".env").write_text("ARES_MAX_ITERATIONS=123\n", encoding="utf-8")
    ...
    assert os.environ["ARES_MAX_ITERATIONS"] == "123"
