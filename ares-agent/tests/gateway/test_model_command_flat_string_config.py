...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "ares_cli.model_switch.switch_model",
        lambda **kw: _fake_switch_result(),
    )
    # save_config writes to ``get_ares_home() / config.yaml`` — point it here.
    monkeypatch.setattr("ares_constants.get_ares_home", lambda: ares_home)
    monkeypatch.setattr("ares_cli.config.get_ares_home", lambda: ares_home)
    return cfg_path
