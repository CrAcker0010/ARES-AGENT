...
class TestBlueBubblesHelpers:
    ...
    def test_default_mention_patterns_match_ares_variants(self, monkeypatch):
        adapter = _make_adapter(monkeypatch, require_mention=True)

        assert adapter.require_mention is True
        assert adapter._message_matches_mention_patterns("Ares, summarize this")
        assert adapter._message_matches_mention_patterns("@Ares agent help")
        assert not adapter._message_matches_mention_patterns("casual family chatter")
        assert not adapter._message_matches_mention_patterns("antiares should not match")
    ...
    def test_custom_mention_patterns_override_defaults(self, monkeypatch):
        ...
        assert not adapter._message_matches_mention_patterns("Ares what is next?")

    def test_clean_mention_text_strips_leading_wake_word(self, monkeypatch):
        adapter = _make_adapter(monkeypatch, require_mention=True)

        assert adapter._clean_mention_text("Ares, summarize this") == "summarize this"
        assert adapter._clean_mention_text("Ares agent: summarize this") == "summarize this"
        assert adapter._clean_mention_text("please ask Ares about this") == "please ask Ares about this"
