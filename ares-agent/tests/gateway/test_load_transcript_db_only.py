...
def test_load_transcript_returns_db_messages_when_no_jsonl(tmp_path, monkeypatch):
    ...
    ~/.ares/state.db. (DEFAULT_DB_PATH is a module-level constant computed
    at ares_state import time, before pytest's ARES_HOME monkeypatch
    fires — the autouse fixture's ARES_HOME override doesn't help here.)
    """
    import ares_state
