"""
Tests for mcp_serve — Ares MCP server.
...
def _isolate_ares_home(tmp_path, monkeypatch):
    """Redirect ARES_HOME to a temp directory."""
    monkeypatch.setenv("ARES_HOME", str(tmp_path))
    try:
        import ares_constants
        monkeypatch.setattr(ares_constants, "get_ares_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking ares_state schema."""
