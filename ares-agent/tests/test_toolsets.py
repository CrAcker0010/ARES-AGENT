...
class TestToolsetConsistency:
    ...
    def test_ares_platforms_share_core_tools(self):
        """All ares-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on ares-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["ares-cli", "ares-telegram", "ares-discord", "ares-whatsapp", "ares-slack", "ares-signal", "ares-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_ares_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("ares-whatsapp")

    def test_ares_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("ares-api-server")
