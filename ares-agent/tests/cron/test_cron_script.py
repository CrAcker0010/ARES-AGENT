...
- Path resolution (absolute, relative to ARES_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp ARES_HOME."""
    ares_home = tmp_path / ".ares"
    ares_home.mkdir()
    (ares_home / "cron").mkdir()
    (ares_home / "cron" / "output").mkdir()
    (ares_home / "scripts").mkdir()
    monkeypatch.setenv("ARES_HOME", str(ares_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "ARES_DIR", ares_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", ares_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", ares_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", ares_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.ares/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("ARES_SESSION_PLATFORM") is None
        assert os.environ.get("ARES_SESSION_CHAT_ID") is None
        assert os.environ.get("ARES_SESSION_CHAT_NAME") is None
