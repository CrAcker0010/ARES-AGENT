...
ARES_HOME scoping, and tick() serialization for profile jobs.
"""
...
def isolated_cron_profile_home(tmp_path, monkeypatch):
    """Create an isolated Ares root with a named profile and temp cron store."""
    root = tmp_path / "ares-root"
...
class TestCronjobToolProfile:
    ...
    def test_schema_advertises_profile(self):
        ...
        assert "ares profile" in desc_lower
        assert "context-local" in desc_lower
        assert "subprocess" in desc_lower
        assert "temporarily sets ares_home" not in desc_lower
...
class TestRunJobProfileContext:
    ...
    def _install_agent_stubs(monkeypatch, observed: dict):
        ...
        class FakeAgent:
            def __init__(self, **kwargs):
                ...
                observed["env_home_during_init"] = os.environ.get("ARES_HOME")
                observed["profile_env_only_during_init"] = os.environ.get(
                    "ARES_PROFILE_TEST_ONLY"
                )
                observed["profile_env_shared_during_init"] = os.environ.get(
                    "ARES_PROFILE_TEST_SHARED"
                )
                observed["ares_home_during_init"] = str(get_ares_home())
                observed["scheduler_home_during_init"] = str(sched._get_ares_home())
                observed["skip_context_files"] = kwargs.get("skip_context_files")

            def run_conversation(self, *_a, **_kw):
                from ares_constants import get_ares_home

                observed["env_home_during_run"] = os.environ.get("ARES_HOME")
                ...
                observed["ares_home_during_run"] = str(get_ares_home())
                observed["scheduler_home_during_run"] = str(sched._get_ares_home())
        ...
        from ares_cli import runtime_provider as runtime_provider
        ...
        monkeypatch.setattr(sched, "_ares_home", None)
        monkeypatch.setenv("ARES_CRON_TIMEOUT", "0")
    ...
    def test_run_job_sets_and_restores_profile_home(
        ...
        assert observed["ares_home_during_init"] == str(profile_home.resolve())
        assert observed["ares_home_during_run"] == str(profile_home.resolve())
        ...
        assert os.environ["ARES_HOME"] == str(root)
        assert sched._get_ares_home() == root
    ...
    def test_profile_dotenv_environment_is_restored(
        ...
        observed: dict = {}
        self._install_agent_stubs(monkeypatch, observed)
        monkeypatch.setenv("ARES_PROFILE_TEST_SHARED", "outer")
        monkeypatch.delenv("ARES_PROFILE_TEST_ONLY", raising=False)

        def fake_load_dotenv(path, *_a, **_kw):
            observed.setdefault("dotenv_paths", []).append(str(path))
            os.environ["ARES_PROFILE_TEST_SHARED"] = "profile-value"
            os.environ["ARES_PROFILE_TEST_ONLY"] = "profile-only"
            os.environ["ARES_CRON_TIMEOUT"] = "123"
            return True
        ...
        assert os.environ["ARES_PROFILE_TEST_SHARED"] == "outer"
        assert "ARES_PROFILE_TEST_ONLY" not in os.environ
        assert os.environ["ARES_CRON_TIMEOUT"] == "0"
        assert os.environ["ARES_HOME"] == str(root)
        assert sched._get_ares_home() == root
    ...
    def test_run_job_without_profile_leaves_ares_home_untouched(
        self, isolated_cron_profile_home, monkeypatch
    ):
        import cron.scheduler as sched
        ...
        assert observed["ares_home_during_init"] == str(root)
        assert os.environ["ARES_HOME"] == str(root)
