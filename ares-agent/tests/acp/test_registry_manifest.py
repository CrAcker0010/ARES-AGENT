"""Tests for ACP Registry metadata shipped with Ares."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "ares-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Ares Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/NousResearch/ares-agent"
    assert data["website"].startswith("https://ares-agent.nousresearch.com/")
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"ares-agent[acp]=={data['version']}"
    assert uvx["args"] == ["ares-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"ares-agent[acp]=={_pyproject_version()}"
    )
