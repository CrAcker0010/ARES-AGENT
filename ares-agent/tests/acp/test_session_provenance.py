...
def test_root_session_no_compression(db):
    ...
    assert prov["currentAresSessionId"] == "root1"
    assert prov["rootAresSessionId"] == "root1"
    assert prov["parentAresSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_ares_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentAresSessionId"] == "old"
    assert prov["rootAresSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousAresSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootAresSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootAresSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_ares_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousAresSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"ares"}
    assert "sessionProvenance" in meta["ares"]
    assert meta["ares"]["sessionProvenance"]["currentAresSessionId"] == "root1"
