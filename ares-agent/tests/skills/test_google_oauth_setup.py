...
class TestAresConstantsFallback:
    """Tests for _ares_home.py fallback when ares_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_ares_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _ares_home.py with ares_constants blocked."""
        monkeypatch.setitem(sys.modules, "ares_constants", None)
        spec = importlib.util.spec_from_file_location("_ares_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_ares_home_env_var(self, monkeypatch, tmp_path):
        """When ares_constants is missing, ARES_HOME comes from env var."""
        monkeypatch.setenv("ARES_HOME", str(tmp_path / "custom-ares"))
        module = self._load_helper(monkeypatch)
        assert module.get_ares_home() == tmp_path / "custom-ares"

    def test_fallback_defaults_to_dot_ares(self, monkeypatch):
        """When ares_constants is missing and ARES_HOME unset, default to ~/.ares."""
        monkeypatch.delenv("ARES_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_ares_home() == Path.home() / ".ares"

    def test_fallback_ignores_empty_ares_home(self, monkeypatch):
        """Empty/whitespace ARES_HOME is treated as unset."""
        monkeypatch.setenv("ARES_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_ares_home() == Path.home() / ".ares"

    def test_fallback_display_ares_home_shortens_path(self, monkeypatch):
        """Fallback display_ares_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("ARES_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_ares_home() == "~/.ares"

    def test_fallback_display_ares_home_profile_path(self, monkeypatch):
        """Fallback display_ares_home() handles profile paths under ~/."""
        monkeypatch.setenv("ARES_HOME", str(Path.home() / ".ares/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_ares_home() == "~/.ares/profiles/coder"

    def test_fallback_display_ares_home_custom_path(self, monkeypatch):
        """Fallback display_ares_home() returns full path for non-home locations."""
        monkeypatch.setenv("ARES_HOME", "/opt/ares-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_ares_home() == "/opt/ares-custom"

    def test_delegates_to_ares_constants_when_available(self):
        """When ares_constants IS importable, _ares_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_ares_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import ares_constants
        assert module.get_ares_home is ares_constants.get_ares_home
        assert module.display_ares_home is ares_constants.display_ares_home
...
class TestInstallDeps:
    ...
    Regression coverage for the Ares Docker image, whose venv is built with
    ...
    def test_returns_false_when_no_pip_and_no_uv(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "ares-agent[google]" in out
