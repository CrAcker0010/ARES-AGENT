...
def server():
    with patch.dict(
        "sys.modules",
        {
            "ares_constants": MagicMock(
                get_ares_home=MagicMock(return_value="/tmp/ares_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'ares-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'ares-release' patched"
    }
