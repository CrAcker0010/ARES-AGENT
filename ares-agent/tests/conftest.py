"""Shared fixtures for the ares-agent test suite.
...
2. **Isolated ARES_HOME.** ARES_HOME points to a per-test tempdir so
   code reading ``~/.ares/*`` via ``get_ares_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".ares"`` instead of the canonical
   ``get_ares_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ARES_SESSION_* inheritance** — the agent's current gateway
...
# ARES_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_ARES_BEHAVIORAL_VARS = frozenset({
    "ARES_YOLO_MODE",
    "ARES_INTERACTIVE",
    "ARES_QUIET",
    "ARES_TOOL_PROGRESS",
    ...
    "ARES_SESSION_CHAT_ID",
    "ARES_SESSION_CHAT_NAME",
    "ARES_SESSION_THREAD_ID",
    "ARES_SESSION_SOURCE",
    "ARES_SESSION_KEY",
    "ARES_GATEWAY_SESSION",
    "ARES_CRON_SESSION",
    "_ARES_GATEWAY",
    "ARES_PLATFORM",
    "ARES_MODEL",
    ...
    "ARES_TIMEZONE",
    "ARES_REDACT_SECRETS",
    "ARES_BACKGROUND_NOTIFICATIONS",
    "ARES_EXEC_ASK",
    "ARES_HOME_MODE",
    "ARES_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.ares/kanban.db instead of the per-test ARES_HOME.
    "ARES_KANBAN_DB",
    "ARES_KANBAN_BOARD",
    "ARES_KANBAN_HOME",
    "ARES_KANBAN_WORKSPACES_ROOT",
    "ARES_KANBAN_LOGS_ROOT",
    "ARES_KANBAN_TASK",
    "ARES_KANBAN_WORKSPACE",
    "ARES_KANBAN_RUN_ID",
    "ARES_KANBAN_CLAIM_LOCK",
    "ARES_KANBAN_DISPATCH_IN_GATEWAY",
    "ARES_TENANT",
    ...
    "ARES_DASHBOARD_OAUTH_CLIENT_ID",
    "ARES_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ARES_HOME to per-test tempdirs so code that
    reads ``~/.ares/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral ARES_* vars that could change test semantics.
    for name in _ARES_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect ARES_HOME to a per-test tempdir. Code that reads
    #    ``~/.ares/*`` via ``get_ares_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.ares/*`` via
    #    ``Path.home() / ".ares"`` instead of ``get_ares_home()``
    #    is a bug to fix at the callsite.
    fake_ares_home = tmp_path / "ares_test"
    fake_ares_home.mkdir()
    (fake_ares_home / "sessions").mkdir()
    (fake_ares_home / "cron").mkdir()
    (fake_ares_home / "memories").mkdir()
    (fake_ares_home / "skills").mkdir()
    monkeypatch.setenv("ARES_HOME", str(fake_ares_home))
    ...
    #    ~/.ares/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import ares_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_ares_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal ares config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``ares-gateway`` process
...
# correlated with a ``tests/ares_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> ares-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart ares-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting ares/python patterns are also blocked.
    """
    ...
    def _matches_ares_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _ARES_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_ares_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch ares-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m ares_cli.main".
                if (
                    "ares" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live ares-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting ares/python could hit the live gateway. "
