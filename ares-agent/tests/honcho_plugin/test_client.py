...
from ares_cli.profiles import _get_default_ares_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "ares"
        assert config.workspace_id == "ares"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.ares/honcho.json
        monkeypatch.setenv("ARES_HOME", str(tmp_path / "isolated"))
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"ares": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="ares-agent"
        ):
            result = config.resolve_session_name("/home/user/ares-agent/subdir")
        assert result == "ares-agent"
...
class TestResolveConfigPath:
    def test_prefers_ares_home_when_exists(self, tmp_path):
        ares_home = tmp_path / "ares"
        ares_home.mkdir()
        local_cfg = ares_home / "honcho.json"
        local_cfg.write_text('{"apiKey": "local"}')

        with patch.dict(os.environ, {"ARES_HOME": str(ares_home)}):
            result = resolve_config_path()
        assert result == local_cfg

    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: ARES_HOME points at ~/.ares/profiles/<name>, so
        # _get_default_ares_home() must resolve back to ~/.ares — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".ares"
        ...
        assert _get_default_ares_home() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_ares_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".ares".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".ares"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "ares_coder"
        assert profile_host_key("default") == "ares"

    def test_default_returns_ares(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("ARES_HONCHO_HOST", None)
            os.environ.pop("ARES_HOME", None)
            assert resolve_active_host() == "ares"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"ARES_HONCHO_HOST": "ares.coder"}):
            assert resolve_active_host() == "ares.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ARES_HONCHO_HOST", None)
            with patch("ares_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "ares_coder"

    def test_default_profile_returns_ares(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ARES_HONCHO_HOST", None)
            with patch("ares_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "ares"

    def test_custom_profile_returns_ares(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ARES_HONCHO_HOST", None)
            with patch("ares_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "ares"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ARES_HONCHO_HOST", None)
            # Temporarily remove ares_cli.profiles to simulate import failure
            saved = sys.modules.get("ares_cli.profiles")
            sys.modules["ares_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "ares"
            finally:
                if saved is not None:
                    sys.modules["ares_cli.profiles"] = saved
                else:
                    sys.modules.pop("ares_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="ares_coder")
        assert config.host == "ares_coder"
        assert config.workspace_id == "ares"  # shared workspace
        assert config.ai_peer == "ares_coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="ares")
        assert config.host == "ares"
        assert config.workspace_id == "ares"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "ares": {"aiPeer": "ares", "peerName": "alice"},
                "ares_coder": {
                    "aiPeer": "ares_coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="ares_coder", config_path=config_file,
        )
        assert config.host == "ares_coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "ares_dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="ares_dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "ares_dreamer"
    ...
    def test_from_global_config_reads_legacy_dot_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "ares.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        config = HonchoClientConfig.from_global_config(
            host="ares_dreamer",
        ...
        assert config.host == "ares_dreamer"
        assert config.peer_name == "dreamer-user"
        assert config.workspace_id == "ares_dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"ares": {"enabled": True, "aiPeer": "ares"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"ares": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_ares_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="ares",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("ares_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_defaults_to_30s_when_no_timeout_configured(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("ares_cli.config.load_config", return_value={}):
            client = get_honcho_client(cfg)
    ...
    def test_ares_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="ares",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("ares_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
