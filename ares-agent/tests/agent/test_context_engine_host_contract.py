...
engine plugins (e.g. ares-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``ares plugins`` picker."""
    ...
    monkeypatch.setattr(
        "ares_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "ares_cli.plugins.get_plugin_context_engine",
