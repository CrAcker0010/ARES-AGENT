...
def test_ares_client_tag_includes_current_version():
    """The client tag must reflect ares_cli.__version__ verbatim."""
    from ares_cli import __version__
    from agent.portal_tags import ares_client_tag

    assert ares_client_tag() == f"client=ares-client-v{__version__}"


def test_ares_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import ares_client_tag

    tag = ares_client_tag()
    assert tag.startswith("client=ares-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import ares_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=ares-agent" in tags
    assert ares_client_tag() in tags
    assert len(tags) == 2
