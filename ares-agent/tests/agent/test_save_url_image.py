...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate ARES_HOME under tmp_path."""
    monkeypatch.setenv("ARES_HOME", str(tmp_path / ".ares"))
    (tmp_path / ".ares").mkdir()

    # Force the constants/image cache helpers to re-read ARES_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("ares_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_ares_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under ARES_HOME — gateway cleanup
