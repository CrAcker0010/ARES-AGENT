...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Ares passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("ARES_HOME", "/users/alice/.ares/profiles/backend-worker")
        monkeypatch.setenv("ARES_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "ARES_KANBAN_DB",
            "/users/alice/.ares/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.ares/kanban/boards/smoke"]'
            in cmd
        )
