...
def isolated_home(monkeypatch):
    """Temp ARES_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="ares_test_31179_")
    ares_home = os.path.join(test_home, ".ares")
    os.makedirs(ares_home)
...
def _fresh_modules():
    """Drop cached ares modules so each test reloads against current env."""
