...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_ares_env(self):
        """Ares' own .env inside ARES_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: ares internal .env is in ~/.ares/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.ares/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.ares/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Ares cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        ares_home = tmp_path / ".ares"
        cache = ares_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._ares_home_path", return_value=ares_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Ares cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        ares_home = tmp_path / ".ares"
        hub = ares_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_ares_home(self, tmp_path):
        """The env basename guard does not depend on ARES_HOME resolution."""
        ares_home = tmp_path / ".ares"
        ares_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        ares_home = tmp_path / ".ares"
        cache = ares_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
