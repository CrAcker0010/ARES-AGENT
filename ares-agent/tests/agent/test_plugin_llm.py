...
""",
        ...
        from ares_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        ares_home = tmp_path / ".ares"
        ares_home.mkdir()
        (ares_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("ARES_HOME", str(ares_home))
        from ares_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from ares_cli.plugins import PluginContext, PluginManifest, PluginManager
