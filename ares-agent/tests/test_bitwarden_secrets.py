...
binary download are exercised manually by `ares secrets bitwarden
...
def ares_home(tmp_path, monkeypatch):
    """Point Ares at an isolated home directory."""
    home = tmp_path / ".ares"
    home.mkdir()
    monkeypatch.setenv("ARES_HOME", str(home))
    # Some modules cache get_ares_home; clear if needed.
    import ares_constants
    if hasattr(ares_constants, "_ARES_HOME_CACHE"):
        ares_constants._ARES_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(ares_home, monkeypatch):
...
def test_install_bws_happy_path(ares_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(ares_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(ares_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from ares_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under ares_home/cache/."""
    home = tmp_path / ".ares"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
