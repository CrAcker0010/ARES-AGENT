...
Run from the PR worktree:
    ...
    cd ~/.ares/ares-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.ares/ares-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.ares/ares-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a ares-agent checkout"
)
...
# Isolated ARES_HOME for the config write.
