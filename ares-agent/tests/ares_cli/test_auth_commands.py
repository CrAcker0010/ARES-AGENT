...
def test_auth_add_google_gemini_cli_sets_active_provider(tmp_path, monkeypatch):
    """ares auth add google-gemini-cli must set active_provider in auth.json.
    ...
    # pool entry from pool.add_entry() still present for ares auth list
...
def test_auth_add_qwen_oauth_sets_active_provider(tmp_path, monkeypatch):
    """ares auth add qwen-oauth must set active_provider in auth.json.
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """ares auth add xai-oauth must write providers singleton and set active_provider.
    ...
    monkeypatch.setattr(
        "ares_cli.auth._xai_oauth_loopback_login",
        lambda **kwargs: {
            "tokens": {
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    with patch(
        ...
        "ares_cli.copilot_auth.get_copilot_api_token",
