...
# ``XAI_BASE_URL`` / ``ARES_XAI_BASE_URL`` are a credential-leak vector
...
def _seed_xai_oauth_state(
    ares_home: Path, state: dict, *, active_provider: str = "xai-oauth"
) -> None:
    ares_home.mkdir(parents=True, exist_ok=True)
    auth_store = {
        "version": 1,
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    """
    ares_home = tmp_path / "ares"
    _seed_xai_oauth_state(ares_home, dict(_STALE_XAI_OAUTH_STATE), active_provider="nous")
    monkeypatch.setenv("ARES_HOME", str(ares_home))
    ...
    monkeypatch.setattr("ares_cli.auth._refresh_xai_oauth_tokens", _terminal_refresh)
...
def test_resolve_credentials_does_not_quarantine_on_transient_refresh_failure(
    ...
    """
    ares_home = tmp_path / "ares"
    _seed_xai_oauth_state(ares_home, dict(_STALE_XAI_OAUTH_STATE))
    monkeypatch.setenv("ARES_HOME", str(ares_home))
    ...
    monkeypatch.setattr("ares_cli.auth._refresh_xai_oauth_tokens", _transient_refresh)
...
def test_refresh_xai_oauth_pure_403_marked_tier_denied_not_relogin(monkeypatch):
    ...
    being active. Re-running ``ares model`` won't help in that
...
def test_format_auth_error_tier_denied_does_not_suggest_relogin():
    ...
    Regression for #26847: telling a tier-gated user to ``ares model``
    ...
    assert "ares model" not in rendered.lower()
