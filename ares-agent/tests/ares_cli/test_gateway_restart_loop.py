...
Covers:
- Defense 1: gateway stop/restart refuse when _ARES_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "ares gateway stop",
        "ares gateway start",
        "ares  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "ARES GATEWAY RESTART",           # uppercase
    ])
    def test_ares_gateway_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.ares.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.ares.gateway.plist",
        "launchctl stop ai.ares.gateway",
        "systemctl restart ares-gateway",
        "systemctl stop ares-gateway.service",
        "systemctl start ares-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill ares gateway process",
        "pkill -f ares.*gateway",
    ])
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_ares_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade ares then run ares gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.ares.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
...
class TestGatewaySelfTargetingGuard:
    """Verify ares gateway stop/restart refuse when _ARES_GATEWAY=1."""

    def test_stop_refuses_inside_gateway(self, monkeypatch):
        monkeypatch.setenv("_ARES_GATEWAY", "1")
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_ARES_GATEWAY", raising=False)
