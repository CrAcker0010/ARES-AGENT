...
class TestFindStaleDashboardPids:
    ...
    def test_exclude_pids_filters_specified_pids(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(11111, "ares dashboard --port 9119"),
                    _ps_line(22222, "ares dashboard --port 9120"),
                    _ps_line(33333, "ares dashboard --port 9121"),
                ]) + "\n",
                stderr="",
            )
    ...
    def test_exclude_pids_none_is_noop(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=_ps_line(12345, "ares dashboard --port 9119") + "\n",
                stderr="",
            )
