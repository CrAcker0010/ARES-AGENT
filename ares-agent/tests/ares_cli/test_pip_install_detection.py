...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    image, so ``ares update`` refused to run. With a ``.git`` checkout it
    must resolve to ``git``.
    """
    (tmp_path / ".git").mkdir()
    with patch("ares_cli.config.get_managed_system", return_value=None), \
...
def test_banner_warns_on_pip_install(tmp_path):
    ...
    with patch("ares_cli.config.get_ares_home", return_value=hh), \
         patch("ares_constants.get_ares_home", return_value=hh):
        buf = io.StringIO()
