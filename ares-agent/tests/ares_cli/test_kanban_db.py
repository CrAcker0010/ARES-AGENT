...
def test_connect_honors_kanban_busy_timeout_env(kanban_home, monkeypatch):
    ...
    """
    monkeypatch.setenv("ARES_KANBAN_BUSY_TIMEOUT_MS", "123456")
...
def test_connect_rejects_tls_record_in_sqlite_header(tmp_path, monkeypatch):
    ...
    home.mkdir()
    monkeypatch.setenv("ARES_HOME", str(home))
    monkeypatch.delenv("ARES_KANBAN_DB", raising=False)
...
def test_claim_uses_env_default_ttl(kanban_home, monkeypatch):
    monkeypatch.setenv("ARES_KANBAN_CLAIM_TTL_SECONDS", "3600")
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x", assignee="a")
...
def test_detect_crashed_workers_skips_freshly_claimed_tasks(
    ...
    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.delenv("ARES_KANBAN_CRASH_GRACE_SECONDS", raising=False)
...
def test_detect_crashed_workers_grace_period_env_override(
    ...
    """ARES_KANBAN_CRASH_GRACE_SECONDS env var adjusts the window."""
    import ares_cli.kanban_db as _kb

    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("ARES_KANBAN_CRASH_GRACE_SECONDS", "5")
...
def test_resolve_crash_grace_seconds_handles_bad_env(monkeypatch):
    ...
    for bad_val in ("notanumber", "-5", ""):
        monkeypatch.setenv("ARES_KANBAN_CRASH_GRACE_SECONDS", bad_val)
...
def test_rate_limit_exit_requeues_without_counting_failure(
    ...
    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("ARES_KANBAN_CRASH_GRACE_SECONDS", "0")
...
def test_respawn_guard_defers_rate_limited_within_cooldown(
    ...
    monkeypatch.setenv("ARES_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "300")
    now = 5_000_000
...
def test_respawn_guard_rate_limit_cooldown_zero_allows_immediately(
    ...
    monkeypatch.setenv("ARES_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "0")
    now = 6_000_000
...
def test_resolve_rate_limit_cooldown_handles_bad_env(monkeypatch):
    ...
    for bad_val in ("notanumber", "-5", ""):
        monkeypatch.setenv(
            "ARES_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", bad_val
        )
        assert (
...
def test_cleanup_workspace_removes_managed_scratch_dir(kanban_home):
    ...
    with kb.connect() as conn:
        ...
        assert ws.is_dir()
        kb.complete_task(conn, t, result="ok")
    assert not ws.exists(), "Ares-managed scratch dir should be cleaned up"
...
def test_cleanup_workspace_honors_workspaces_root_env_override(tmp_path, monkeypatch):
    """``ARES_KANBAN_WORKSPACES_ROOT`` extends the managed-scratch set.
    ...
    monkeypatch.setenv("ARES_KANBAN_WORKSPACES_ROOT", str(workspaces_override))
    kb.init_db()
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """Ares' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` Ares' DB/metadata/logs on task completion.
    """
...
class TestSharedBoardPaths:
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        assert env["ARES_KANBAN_BRANCH"] == "wt/t_dispatch_env"
...
def test_resolve_ares_argv_prefers_path_shim(monkeypatch):
    ...
    monkeypatch.delenv("ARES_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/ares")
    argv = kb._resolve_ares_argv()
    assert argv == ["/usr/local/bin/ares"]


def test_resolve_ares_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    """A relative executable override must not remain workspace-cwd-dependent."""
    import ares_cli.kanban_db as kb

    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("ARES_BIN", ".\\ares.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_ares_argv() == [os.path.abspath(".\\ares.exe")]


def test_resolve_ares_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "ares.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("ARES_BIN", raising=False)
    monkeypatch.setenv("PATH", str(bin_dir))
    monkeypatch.setenv("PATHEXT", ".CMD")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_ares_argv() == [sys.executable, "-m", "ares_cli.main"]


def test_resolve_ares_argv_honors_ares_bin_path_override(monkeypatch, tmp_path):
    """An explicit path-like ARES_BIN lets service managers pin the executable."""
    import shutil
    import ares_cli.kanban_db as kb

    shim = tmp_path / "bin" / "ares"
    shim.parent.mkdir()
    shim.write_text("#!/bin/sh\n", encoding="utf-8")
    monkeypatch.setenv("ARES_BIN", str(shim))
    monkeypatch.setattr(shutil, "which", lambda name: None)

    assert kb._resolve_ares_argv() == [str(shim)]


def test_resolve_ares_argv_ares_bin_bare_name_uses_path(monkeypatch, tmp_path):
    """Bare ARES_BIN values keep PATH semantics instead of cwd shadowing."""
    import stat
    import ares_cli.kanban_db as kb

    cwd_ares = tmp_path / "ares"
    cwd_ares.write_text("wrong\n", encoding="utf-8")
    cwd_ares.chmod(cwd_ares.stat().st_mode | stat.S_IXUSR)
    path_ares = tmp_path / "bin" / "ares"
    path_ares.parent.mkdir()
    path_ares.write_text("right\n", encoding="utf-8")
    path_ares.chmod(path_ares.stat().st_mode | stat.S_IXUSR)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("PATH", str(path_ares.parent))
    monkeypatch.setenv("ARES_BIN", "ares")

    assert kb._resolve_ares_argv() == [str(path_ares)]


def test_resolve_ares_argv_ares_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    """Bare ARES_BIN does not accept current-directory shadow executables."""
    import sys
    import ares_cli.kanban_db as kb

    (tmp_path / "ares.exe").write_text("wrong\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
...
def test_resolve_ares_argv_ares_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
    """A PATH-resolved ARES_BIN batch shim is not used as worker argv[0]."""
    import sys
    import ares_cli.kanban_db as kb
...
def test_resolve_ares_argv_ares_bin_unresolved_bare_name_falls_back(monkeypatch):
    """Unresolved ARES_BIN command names do not delegate cwd search to Popen."""
    import sys
    import ares_cli.kanban_db as kb
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="ares_cli.kanban_db"):
        with kb.connect() as conn:
...
def test_write_txn_healthy_commit_no_exception(tmp_path):
    ...
    from ares_cli.kanban_db import connect, write_txn
    db = tmp_path / "test.db"
...
def test_write_txn_raises_on_truncated_file(tmp_path):
    ...
    with pytest.raises(sqlite3.DatabaseError, match="torn-extend|page count mismatch"):
        with unittest.mock.patch("ares_cli.kanban_db.os.path.getsize", side_effect=fake_getsize):
            with write_txn(conn) as c:
                c.execute(
                    "INSERT INTO tasks (id, title, assignee, status, priority, created_at) "
...
def test_write_txn_post_commit_check_fires_every_call(tmp_path):
    ...
    from ares_cli.kanban_db import connect, write_txn
    import ares_cli.kanban_db as kanban_db_module
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
    call_count = 0
...
def test_connect_sets_wal_autocheckpoint_100(tmp_path):
    ...
    from ares_cli.kanban_db import connect
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    import struct
    from ares_cli.kanban_db import connect, _check_file_length_invariant
...
def test_reap_worker_zombies_returns_count():
    ...
    with patch("ares_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("ares_cli.kanban_db._record_worker_exit"):
...
def test_reap_worker_zombies_noop_on_windows(monkeypatch):
    ...
    monkeypatch.setattr("ares_cli.kanban_db.os.name", "nt")
    with patch("ares_cli.kanban_db.os.waitpid") as mock_waitpid:
...
def test_reap_worker_zombies_noop_no_children():
    ...
    with patch("ares_cli.kanban_db.os.waitpid", side_effect=ChildProcessError):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_reap_worker_zombies_records_exit_status():
    ...
    with patch("ares_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch(
            "ares_cli.kanban_db._record_worker_exit",
...
def test_reap_worker_zombies_handles_waitpid_os_error():
    ...
    with patch("ares_cli.kanban_db.os.waitpid", side_effect=OSError("test error")):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_zombie_reaper_survives_all_boards_failing():
    ...
    for tick in range(5):
        ...
        with patch(
            "ares_cli.kanban_db.os.waitpid", side_effect=make_fake_waitpid(pids)
        ):
            with patch("ares_cli.kanban_db._record_worker_exit"):
                pids = kb.reap_worker_zombies()
...
def test_dispatch_once_still_reaps_via_extracted_fn(kanban_home):
    ...
    with patch("ares_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("ares_cli.kanban_db._record_worker_exit"):
            with patch("ares_cli.kanban_db.os.name", "posix"):
                pids = kb.reap_worker_zombies()
