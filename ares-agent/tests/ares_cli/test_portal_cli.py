"""Tests for `ares portal` dispatch.

`ares portal` (no subcommand) is the human-readable alias for the Nous Portal
one-shot onboarding (`ares auth add nous --type oauth` / `ares setup
--portal`). The prior status default moved to `ares portal info`, with
...
from ares_cli import portal_cli
...
def test_bare_portal_and_login_run_one_shot(monkeypatch, sub):
    """`ares portal`, `ares portal login` -> one-shot onboarding."""
    ...
    monkeypatch.setattr(
        "ares_cli.setup._run_portal_one_shot", fake_one_shot
    )
...
def test_info_and_status_alias_run_status(monkeypatch, sub):
    """`ares portal info` and the `status` back-compat alias -> status."""
    calls = {"login": 0, "status": 0}

    monkeypatch.setattr(
        "ares_cli.setup._run_portal_one_shot",
...
def test_login_cancelled_returns_one(monkeypatch):
    ...
    monkeypatch.setattr("ares_cli.setup._run_portal_one_shot", boom)
    monkeypatch.setattr(portal_cli, "load_config", lambda: {})
...
def test_one_shot_delegates_to_model_flow_nous(monkeypatch):
    ...
    # _model_flow_nous lives in ares_cli.main and is imported lazily inside
    # _run_portal_one_shot, so patch it at the source module.
    monkeypatch.setattr("ares_cli.main._model_flow_nous", fake_model_flow)
    ...
    assert calls["model_flow"] == 1, (
        "`ares portal` must route through _model_flow_nous so the model "
...
def test_one_shot_swallows_cancel_and_systemexit(monkeypatch, exc):
    ...
    monkeypatch.setattr("ares_cli.main._model_flow_nous", boom)
    monkeypatch.setattr("ares_cli.config.load_config", lambda: {})
