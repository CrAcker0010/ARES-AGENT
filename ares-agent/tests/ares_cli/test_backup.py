...
class TestBackup:
    ...
    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside ARES_HOME."""
        ares_home = tmp_path / ".ares"
        ares_home.mkdir()
        _make_ares_tree(ares_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(ares_home / "skills" / "outside-link.txt", outside)
...
class TestPreUpdateBackup:
    ...
    def test_skips_symlinked_files(self, ares_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside ARES_HOME."""
        from ares_cli.backup import create_pre_update_backup
...
class TestRestoreCronJobsIfEmptied:
    """`ares update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, ares_home: Path, label="pre-update"):
        from ares_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, ares_home=ares_home, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from ares_cli.backup import restore_cron_jobs_if_emptied
        ares_home = tmp_path / ".ares"
        jobs_path = ares_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(ares_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, ares_home=ares_home)
    ...
    def test_noop_when_snapshot_id_missing(self, tmp_path):
        ...
        jobs_path = ares_home / "cron" / "jobs.json"
        self._seed_jobs(jobs_path, [])
        assert restore_cron_jobs_if_emptied(None, ares_home=ares_home) is None
        assert restore_cron_jobs_if_emptied("", ares_home=ares_home) is None
