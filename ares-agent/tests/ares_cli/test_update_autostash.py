...
def _setup_setting_test(monkeypatch, tmp_path, mode):
    ...
    monkeypatch.setattr(
        ares_main, "_discard_stashed_changes",
...
def test_non_interactive_discard_throws_changes_away(monkeypatch, tmp_path):
    ...
    ares_main.cmd_update(SimpleNamespace(gateway=True))
...
def test_interactive_update_ignores_discard_setting(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(ares_main.sys.stdin, "isatty", lambda: True)
    monkeypatch.setattr(ares_main.sys.stdout, "isatty", lambda: True)

    ares_main.cmd_update(SimpleNamespace())  # no gateway, no --yes
...
def test_non_interactive_defaults_to_stash_when_setting_absent(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(ares_config, "load_config", lambda *a, **kw: {"model": {}})
...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    ``ares update``'s ``git stash push --include-untracked`` does not sweep it
    ...
    repo_gitignore = Path(ares_main.__file__).resolve().parents[1] / ".gitignore"
    ...
    marker = tmp_path / ".ares-bootstrap-complete"
    marker.write_text("")

    # Exact flags used by ares update (ares_cli/main.py).
    git("stash", "push", "--include-untracked", "-m", "ares-update-autostash")

    assert marker.exists(), (
        ".ares-bootstrap-complete was swept into the update autostash — it must "
    ...
    assert ".ares-bootstrap-complete" not in status
