"""Tests for `ares memory setup [provider]` routing.
...
``ares memory setup honcho``) without the interactive picker — which matters
because the per-provider ``ares <provider>`` subcommand is only registered
...
from ares_cli import memory_setup
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "ares memory setup" in out
