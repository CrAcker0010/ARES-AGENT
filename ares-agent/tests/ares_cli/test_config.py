...
class TestOptionalEnvVarsRegistry:
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """ARES_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `ares setup`) lets a
        ...
        via config.yaml; ARES_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from ares_cli.config import OPTIONAL_ENV_VARS
        assert "ARES_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or any ``ARES_*`` runtime flag.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Ares
    process on next startup via the dotenv → ``os.environ`` chain in
    ``ares_cli/env_loader.py``.
    ...
    def _ares_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "ARES_SPOTIFY_CLIENT_ID",
            "ARES_QWEN_BASE_URL",
            "ARES_MAX_ITERATIONS",
        ],
    )
    def test_ares_integration_keys_still_writable(self, allowed_key):
        """``ARES_*`` overall is NOT blocked — only the four runtime
        location names (HOME/PROFILE/CONFIG/ENV) are. Integration
        credentials following the ``ARES_*`` convention must keep
    ...
    def test_arbitrary_user_key_still_works(self):
        ...
        denylist and outside ``ARES_*``) keep working. The denylist
