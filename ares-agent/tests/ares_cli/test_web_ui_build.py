...
Critical invariant: the dashboard Vite build outputs to ares_cli/web_dist/
...
class TestBuildWebUISkipsWhenFresh:
    ...
    def test_runs_npm_when_dist_missing(self, tmp_path):
        ...
        with patch("ares_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             ...
             patch("ares_cli.main._run_with_idle_timeout", return_value=build_ok) as mock_idle:
            result = _build_web_ui(web_dir)
    ...
    def test_npm_install_uses_workspace_web_scope(self, tmp_path):
        ...
        with patch("ares_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("ares_cli.main.subprocess.run", return_value=mock_cp) as mock_run, \
             patch("ares_cli.main._run_with_idle_timeout", return_value=build_ok):
            result = _build_web_ui(web_dir)
        assert result is True
    ...
    def test_web_build_uses_idle_timeout_helper(self, tmp_path):
        ...
        with patch("ares_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("ares_cli.main.subprocess.run", return_value=install_cp), \
             patch("ares_cli.main._run_with_idle_timeout", return_value=build_cp) as mock_idle:
            result = _build_web_ui(web_dir)
    ...
    def test_termux_web_install_is_workspace_scoped(self, tmp_path, monkeypatch):
        ...
        build_cp = __import__("subprocess").CompletedProcess([], 0, stdout="", stderr="")
        with patch("ares_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("ares_cli.main.subprocess.run", return_value=install_cp) as mock_run, \
             patch("ares_cli.main._run_with_idle_timeout", return_value=build_cp):
            result = _build_web_ui(web_dir)
...
class TestBuildWebUIRetryAndStaleFallback:
    ...
    def test_retries_build_once_on_failure(self, tmp_path):
        ...
        with patch("ares_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             ...
             patch("ares_cli.main.subprocess.run", return_value=install_ok), \
             patch("ares_cli.main._run_with_idle_timeout",
