...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale ARES_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        if ghost is not None:
            env_lines.append(f"ARES_MAX_ITERATIONS={ghost}\n")
        (ares_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "ARES_HOME", ares_home)
        monkeypatch.setattr(doctor_mod, "get_ares_home", lambda: ares_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("ARES_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("ARES_MAX_ITERATIONS", raising=False)
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale ARES_MAX_ITERATIONS" in out
        env_after = (ares_home / ".env").read_text(encoding="utf-8")
        assert "ARES_MAX_ITERATIONS" not in env_after
