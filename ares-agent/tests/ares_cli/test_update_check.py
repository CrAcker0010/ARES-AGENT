...
def test_check_for_updates_invalidates_on_version_change(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ARES_HOME", str(tmp_path))
    monkeypatch.delenv("ARES_REVISION", raising=False)
    with patch("ares_cli.banner.subprocess.run") as mock_run, \
         patch("ares_cli.banner.check_via_pypi", return_value=0) as mock_pypi:
...
def test_check_for_updates_docker_returns_none(tmp_path, monkeypatch):
    ...
    ARES_REVISION (nix-only), so without a docker guard check_for_updates()
    ...
    container, and `ares update` correctly refusing to run there. The guard
    ...
    with patch("ares_cli.config.detect_install_method", return_value="docker"), \
         patch("ares_cli.banner.subprocess.run") as mock_run, \
         patch("ares_cli.banner.check_via_pypi") as mock_pypi:
        result = banner.check_for_updates()
...
def test_check_for_updates_non_docker_still_checks(tmp_path, monkeypatch):
    ...
    with patch("ares_cli.config.detect_install_method", return_value="pip"), \
         patch("ares_cli.banner.subprocess.run") as mock_run, \
         patch("ares_cli.banner.check_via_pypi", return_value=1) as mock_pypi:
        result = banner.check_for_updates()
