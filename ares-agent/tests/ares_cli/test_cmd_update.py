...
class TestCmdUpdatePip:
    ...
    def test_update_pip_exports_virtualenv_from_sys_prefix(
        ...
        monkeypatch.setattr(hm.sys, "prefix", "/tmp/ares-launcher-venv")
        monkeypatch.setattr(hm.sys, "base_prefix", "/usr")

        hm._cmd_update_pip(mock_args)

        assert mock_run.call_count == 1
        assert mock_run.call_args.args[0] == ["/usr/bin/uv", "pip", "install", "--upgrade", "ares-agent"]
        assert mock_run.call_args.kwargs["env"]["VIRTUAL_ENV"] == "/tmp/ares-launcher-venv"
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_on_fork_checks_upstream_when_origin_up_to_date(
        ...
        origin but behind NousResearch/ares-agent silently misses updates.
        """
        from ares_cli import main as hm
        ...
        with patch.object(
            hm,
            "_get_origin_url",
            return_value="https://github.com/example/ares-agent.git",
...
class TestCmdUpdateMigrationPrompt:
    ...
    def test_version_bump_only_applies_silently_without_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "ares_cli.config.get_missing_env_vars", return_value=[]
        ), patch(
            "ares_cli.config.get_missing_config_fields", return_value=[]
        ), patch(
            "ares_cli.config.check_config_version", return_value=(5, 24)
        ), patch(
            "ares_cli.config.migrate_config",
    ...
    def test_new_options_are_listed_by_name_before_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "ares_cli.config.get_missing_env_vars", return_value=env_items
        ), patch(
            "ares_cli.config.get_missing_config_fields", return_value=cfg_items
        ), patch(
            "ares_cli.config.check_config_version", return_value=(1, 24)
        ), patch(
            "ares_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": [], "warnings": []},
        ), patch("ares_cli.main.sys") as mock_sys:
...
class TestCmdUpdateBranchFlag:
    """``ares update --branch <name>`` targets the requested branch.
...
class TestCmdUpdateCheckBranchFlag:
    """``ares update --check --branch <name>`` honors the branch override.
    ...
    @patch("ares_cli.config.detect_install_method", return_value="git")
    @patch("subprocess.run")
    ...
    @patch("ares_cli.config.detect_install_method", return_value="pip")
    @patch("ares_cli.banner.check_via_pypi", return_value=0)
    @patch("subprocess.run")
...
class TestCmdUpdateZipBranchRefusal:
    """``ares update --branch=<non-main>`` must refuse on the ZIP fallback path.
    ...
    def test_zip_fallback_refuses_non_main_branch(self, capsys):
        from ares_cli.main import _update_via_zip
