...
class TestCaptureLogSnapshotRedaction:
    ...
    def test_default_redacts_email_addresses_for_public_share(
        ...
        log_path = ares_home_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, ares_home_with_secret):
        from ares_cli.debug import _capture_log_snapshot
...
class TestCollectDebugReport:
    ...
    def test_report_includes_desktop_log(self, ares_home):
        from ares_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_share_uploads_four_pastes(self, ares_home, capsys):
        ...
        assert "--- ares dump ---" in desktop_paste
...
class TestBuildDebugShare:
    ...
    Backs both ``ares debug share`` (CLI) and ``POST /api/ops/debug-share``
    (dashboard). The dashboard renders ``urls`` as real, copyable links, so the
    contract here is the return value, not stdout.
    """

    def test_returns_structured_urls(self, ares_home):
        from ares_cli.debug import build_debug_share, DebugShareResult
        ...
        with patch("ares_cli.dump.run_dump"), patch(
            "ares_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("ares_cli.debug._schedule_auto_delete"):
    ...
    def test_skips_missing_logs_without_failure(self, ares_home):
        from ares_cli.debug import build_debug_share

        # Remove desktop.log so it should be neither uploaded nor reported failed.
        (ares_home / "logs" / "desktop.log").unlink()

        with patch("ares_cli.dump.run_dump"), patch(
            "ares_cli.debug.upload_to_pastebin",
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, ares_home):
        from ares_cli.debug import build_debug_share
    ...
    def test_optional_log_failure_is_collected_not_raised(self, ares_home):
        from ares_cli.debug import build_debug_share
    ...
    def test_required_report_failure_raises(self, ares_home):
        from ares_cli.debug import build_debug_share
