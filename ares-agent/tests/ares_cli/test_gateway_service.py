...
class TestLaunchdServiceRecovery:
    ...
    def test_launchd_fallback_exits_when_spawn_fails(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "nohup ares gateway run" in out
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        # WorkingDirectory is anchored at the target user's ARES_HOME (stable,
        ...
        assert "WorkingDirectory=/home/alice/.ares/ares-agent" not in unit
...
class TestServiceWorkingDirIsStable:
    ...
    (ARES_HOME), never the source checkout / worktree, so a relocated or
    deleted checkout can't crash-loop the unit on CHDIR (status=200).
    """

    def test_stable_working_dir_uses_ares_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".ares"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_ares_home", lambda: home)
    ...
    def test_stable_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        # ARES_HOME points somewhere that does not exist -> fall back.
        missing = tmp_path / "does-not-exist" / ".ares"
        monkeypatch.setattr(gateway_cli, "get_ares_home", lambda: missing)
        assert gateway_cli._stable_service_working_dir() == str(gateway_cli.PROJECT_ROOT)

    def test_user_unit_workingdirectory_is_ares_home_not_checkout(self, tmp_path, monkeypatch):
        home = tmp_path / ".ares"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_ares_home", lambda: home)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_workingdirectory_is_ares_home(self, tmp_path, monkeypatch):
        import re
