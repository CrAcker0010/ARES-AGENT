...
from ares_cli.nous_account import NousPortalAccountInfo, NousToolAccessInfo
...
def _capture_checklist(monkeypatch, *, selected_idx):
    ...
    monkeypatch.setattr(
        "ares_cli.config.save_config", lambda cfg: None, raising=False
    )
    return captured
...
# ---------------------------------------------------------------------------
# ensure_nous_portal_access — inline login gate for `ares tools`
# ---------------------------------------------------------------------------
