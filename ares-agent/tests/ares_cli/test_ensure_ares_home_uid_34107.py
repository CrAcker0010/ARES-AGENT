"""Regression tests for #34107 — Docker UID/GID handling in ensure_ares_home.

When Ares runs in Docker with ``ARES_UID=1000`` / ``ARES_GID=911``,
the entrypoint chowns the top-level ``ARES_HOME`` once at startup. But
...
The fix is a ``_chown_to_ares_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_ares_uid_gid
# ---------------------------------------------------------------------------


class TestResolveAresUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("ARES_UID", "1000")
        monkeypatch.setenv("ARES_GID", "911")
        from ares_cli.config import _resolve_ares_uid_gid
        uid, gid = _resolve_ares_uid_gid()
        assert uid == 1000
        assert gid == 911

    def test_returns_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("ARES_UID", raising=False)
        monkeypatch.delenv("ARES_GID", raising=False)
        from ares_cli.config import _resolve_ares_uid_gid
        uid, gid = _resolve_ares_uid_gid()
        assert uid is None
        assert gid is None
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("ARES_UID", "not-a-number")
        monkeypatch.setenv("ARES_GID", "911")
        from ares_cli.config import _resolve_ares_uid_gid
        uid, gid = _resolve_ares_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("ARES_UID", "")
        monkeypatch.setenv("ARES_GID", "")
        from ares_cli.config import _resolve_ares_uid_gid
        uid, gid = _resolve_ares_uid_gid()
        assert uid is None
        assert gid is None
...
class TestChownToAresUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_ares_uid(d)  # must not raise
