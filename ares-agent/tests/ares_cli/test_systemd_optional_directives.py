...
class TestStripOptionalSystemdDirectives:
    def test_removes_restart_max_delay_sec(self):
        from ares_cli.gateway import _strip_optional_systemd_directives
        text = """[Service]
Restart=always
RestartSec=5
...
[Service]
Type=simple
ExecStart=/usr/bin/python -m ares_cli.main gateway run
Restart=always
RestartSec=5
KillMode=mixed
KillSignal=SIGTERM
...
WantedBy=default.target
"""
        unit_file = tmp_path / "ares-gateway.service"
