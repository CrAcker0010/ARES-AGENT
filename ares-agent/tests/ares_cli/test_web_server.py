...
@pytest.fixture
def _install_example_plugin(_isolate_ares_home):
    """Drop the example-dashboard fixture into the per-test ARES_HOME
    ...
    ``ARES_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, ares-achievements, model providers
    ...
    #      and start serving requests against a torn-down ARES_HOME.
    app = web_server.app
...
class TestSessionTokenInjection:
    """The desktop shell mints ARES_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        ...
        monkeypatch.setenv("ARES_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
        try:
            importlib.reload(ws)
            assert ws._SESSION_TOKEN == "desktop-seeded-token"
        finally:
            monkeypatch.delenv("ARES_DASHBOARD_SESSION_TOKEN", raising=False)
            importlib.reload(ws)
...
class TestWebServerEndpoints:
    ...
    def test_get_media_404_for_missing_file(self):
        ...
        missing = get_ares_home() / "images" / "nope.png"
    ...
    def test_get_sessions_forwards_min_messages(self, monkeypatch):
        ...
        monkeypatch.setattr("ares_state.SessionDB", _FakeDB)
    ...
    def test_update_ares_returns_docker_guidance_without_spawning(self, monkeypatch):
        import ares_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn ares update")

        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(web_server, "_spawn_ares_action", fail_spawn)
        web_server._ACTION_PROCS.pop("ares-update", None)
        web_server._ACTION_RESULTS.pop("ares-update", None)

        resp = self.client.post("/api/ares/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "ares-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull nousresearch/ares-agent:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/ares-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull nousresearch/ares-agent:latest" in line for line in status_data["lines"])

    def test_update_ares_spawns_on_non_docker_install(self, monkeypatch):
        import ares_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "_spawn_ares_action", fake_spawn)
        web_server._ACTION_PROCS.pop("ares-update", None)
        web_server._ACTION_RESULTS.pop("ares-update", None)
        ...
        assert resp.status_code == 200
        assert resp.json() == {"ok": True, "pid": 12345, "name": "ares-update"}
        assert calls == [(["update"], "ares-update")]
    ...
    def test_action_status_reaps_completed_process(self, monkeypatch):
        ...
        web_server._ACTION_PROCS.pop("ares-update", None)
        web_server._ACTION_RESULTS.pop("ares-update", None)
        web_server._ACTION_PROCS["ares-update"] = proc

        resp = self.client.get("/api/actions/ares-update/status")
        assert resp.status_code == 200
        data = resp.json()
        ...
        assert "ares-update" not in web_server._ACTION_PROCS
        assert web_server._ACTION_RESULTS["ares-update"] == {
            "exit_code": 0,
            "pid": 42424,
        }
    ...
    def test_get_env_vars_marks_channel_managed_keys(self):
        from ares_cli.web_server import _channel_managed_env_keys
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        from ares_cli.web_server import (
    ...
    def test_update_messaging_platform_saves_env_and_enablement(self):
        from ares_cli.config import load_config, load_env
    ...
    def test_telegram_onboarding_worker_request_uses_httpx(self, monkeypatch):
        ...
        assert kwargs["json"] == {"bot_name": "Ares Agent"}
        assert kwargs["headers"]["Accept"] == "application/json"
        assert kwargs["headers"]["Authorization"] == "Bearer poll-secret"
        assert kwargs["headers"]["Content-Type"] == "application/json"
        assert kwargs["headers"]["User-Agent"].startswith("AresDashboard/")
    ...
    def test_telegram_onboarding_start_strips_poll_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            ...
            return {
                ...
                "suggested_username": "ares_pair123_bot",
                "deep_link": "https://t.me/newbot/AresSetupBot/ares_pair123_bot",
                "qr_payload": "https://t.me/newbot/AresSetupBot/ares_pair123_bot",
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "ares_pair_ready_bot",
                    "deep_link": "https://t.me/newbot/AresSetupBot/ares_pair_ready_bot",
                    "qr_payload": "https://t.me/newbot/AresSetupBot/ares_pair_ready_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "ares_pair_ready_bot",
    ...
    def test_telegram_onboarding_apply_requires_ready_pairing(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "ares_pair_waiting_bot",
                "deep_link": "https://t.me/newbot/AresSetupBot/ares_pair_waiting_bot",
                "qr_payload": "https://t.me/newbot/AresSetupBot/ares_pair_waiting_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_telegram_onboarding_cancel_clears_local_session(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "ares_pair_cancel_bot",
                "deep_link": "https://t.me/newbot/AresSetupBot/ares_pair_cancel_bot",
                "qr_payload": "https://t.me/newbot/AresSetupBot/ares_pair_cancel_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_set_model_main_nous_applies_gateway_defaults(self, monkeypatch):
        ...
        resp = self.client.post(
            "/api/model/set",
            json={"scope": "main", "provider": "nous", "model": "ares-4"},
        )
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_apply_main_model_assignment_base_url_and_context_reconcile(self):
        ...
        from ares_cli.web_server import _apply_main_model_assignment
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from ares_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_reports_stale_auxiliary_pins(self):
        ...
        cfg = load_config()
        cfg["model"] = {"provider": "nous", "default": "ares-4"}
        cfg["auxiliary"] = {
    ...
    def test_recommended_default_nous_honors_free_tier(self, monkeypatch):
        ...
        model (mirroring `ares model`), not the first curated paid entry."""
...
class TestNewEndpoints:
    ...
    def test_profiles_create_with_clone_from_duplicates_source(self, monkeypatch):
        ...
        source_skill = get_ares_home() / "profiles" / "source-prof" / "skills" / "custom" / "src-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = (
            get_ares_home() / "profiles" / "source-prof-copy" / "skills" / "custom" / "src-skill" / "SKILL.md"
        )
        assert cloned_skill.exists()
    ...
    def test_profile_model_round_trip(self, monkeypatch):
        ...
        import yaml
        cfg_path = get_ares_home() / "profiles" / "model-prof" / "config.yaml"
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``ares_cli/web_server.py``).
    ...
    def _setup_test_client(self, monkeypatch, _isolate_ares_home):
        ...
        monkeypatch.setattr(
            ares_state, "DEFAULT_DB_PATH", get_ares_home() / "state.db"
        )
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in ares_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_ares_home):
        ...
        # Pin the SessionDB to the isolated ARES_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_ares_home, _install_example_plugin):
        ...
        installs it into the per-test ``ARES_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into ARES_HOME by the class-level
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        assert env["ARES_TUI_INLINE"] == "1"
        assert env["ARES_TUI_DISABLE_MOUSE"] == "1"
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    ...
    gateway_url = env.get("ARES_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_ares_home, _install_example_plugin):
        ...
        the per-test ``ARES_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_ares_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.setenv("ARES_DESKTOP", "1")

        with self._client():
            assert called.wait(3.0), "expected cron tick under ARES_DESKTOP=1"

    def test_ticker_skipped_without_desktop(self, monkeypatch, _isolate_ares_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.delenv("ARES_DESKTOP", raising=False)
