...
``$ARES_HOME/plugins/``, runs ``PluginManager().discover_and_load()``,
and asserts the registration result.

Mirrors the structure of
``tests/ares_cli/test_plugin_scanner_recursion.py::TestRegisterImageGenProvider``.
"""
...
class TestRegisterTTSProvider:
    ...
    def test_accepts_valid_provider(self):
        ...
        ares_home = Path(os.environ["ARES_HOME"])
        _write_plugin(
            ares_home / "plugins",
            "my-tts-plugin",
            register_body=(
        ...
        _enable(ares_home, "my-tts-plugin")
    ...
    def test_rejects_non_provider(self, caplog):
        ...
        _enable(ares_home, "bad-tts-plugin")
    ...
    def test_rejects_builtin_shadow(self, caplog):
        ...
        _enable(ares_home, "shadow-tts-plugin")
