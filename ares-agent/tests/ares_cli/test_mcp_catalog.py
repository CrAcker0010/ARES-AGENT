"""Tests for ares_cli.mcp_catalog and ares_cli.mcp_picker.
...
def catalog_dir(tmp_path, monkeypatch):
    ...
    cat.mkdir()
    monkeypatch.setenv("ARES_OPTIONAL_MCPS", str(cat))
    return cat


@pytest.fixture(autouse=True)
def _isolate_ares_home(tmp_path, monkeypatch):
    """Redirect all config I/O to a temp ARES_HOME."""
    hh = tmp_path / "ares-home"
    hh.mkdir()
    monkeypatch.setenv("ARES_HOME", str(hh))
    monkeypatch.setattr(
        "ares_cli.config.get_ares_home", lambda: hh
    )
    monkeypatch.setattr(
        "ares_cli.config.get_config_path", lambda: hh / "config.yaml"
    )
    monkeypatch.setattr(
        "ares_cli.config.get_env_path", lambda: hh / ".env"
    )
    # mcp_catalog grabs get_ares_home() lazily through ares_constants
    monkeypatch.setattr(
        "ares_constants.get_ares_home", lambda: hh
    )
    return hh
...
def _entry(name: str):
    ...
    from ares_cli.mcp_catalog import get_entry
...
class TestManifestParsing:
    def test_minimal_valid(self, catalog_dir):
        ...
        from ares_cli.mcp_catalog import list_catalog
...
class TestInstall:
    def test_install_simple_stdio_writes_config(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from ares_cli.mcp_catalog import install_entry
    ...
    def test_install_required_env_missing_raises(self, catalog_dir, monkeypatch):
        ...
        from ares_cli import mcp_catalog
        from ares_cli.mcp_catalog import install_entry, CatalogError
...
class TestUninstall:
    def test_uninstall_removes_server_block(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from ares_cli.mcp_catalog import install_entry, uninstall_entry
        from ares_cli.config import load_config
    ...
    def test_uninstall_missing_returns_false(self):
        from ares_cli.mcp_catalog import uninstall_entry
...
class TestPicker:
    def test_show_catalog_empty(self, catalog_dir, capsys):
        from ares_cli.mcp_picker import show_catalog
    ...
    def test_install_by_name_unknown(self, catalog_dir, capsys):
        from ares_cli.mcp_picker import install_by_name
    ...
    def test_run_picker_non_tty_falls_back(self, catalog_dir, capsys, monkeypatch):
        ...
        from ares_cli.mcp_picker import run_picker
...
class TestCatalogDiagnostics:
    def test_future_manifest_version_skipped_with_diagnostic(self, catalog_dir):
        ...
        from ares_cli.mcp_catalog import list_catalog, catalog_diagnostics
    ...
    def test_picker_surfaces_future_manifest_warning(self, catalog_dir, capsys, monkeypatch):
        ...
        entries so users running headless or after `ares setup` know to update."""
        body = _basic_manifest()
        ...
        assert "requires a newer Ares" in out
...
class TestGitInstallShaRef:
    def test_sha_ref_skips_branch_attempt(self, catalog_dir, monkeypatch, tmp_path):
        ...
        from ares_cli import mcp_catalog
        from ares_cli.mcp_catalog import _do_git_install
    ...
    def test_branch_ref_uses_branch_clone(self, catalog_dir, monkeypatch):
        ...
        from ares_cli import mcp_catalog
        from ares_cli.mcp_catalog import _do_git_install, get_entry
...
class TestShippedCatalog:
    def test_all_shipped_manifests_parse(self, monkeypatch):
        ...
        """
        # Use the actual repo's optional-mcps directory (no ARES_OPTIONAL_MCPS
        # override) so this test catches real manifests.
        monkeypatch.delenv("ARES_OPTIONAL_MCPS", raising=False)
        from ares_cli.mcp_catalog import _catalog_root, _parse_manifest
