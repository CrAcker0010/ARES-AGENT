"""
Email platform adapter for the Ares gateway.

Allows users to interact with Ares by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "ares-agent" "version" "{_ares_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Ares Agent")
        ...
        msg_id = f"<ares-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
