...
# Audio file extensions Ares recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Ares' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Ares-created Telegram private-chat
...
from ares_constants import get_default_ares_root, get_ares_dir, get_ares_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.ares/.env manually."
)
...
# Default location: {ARES_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_ares_dir("cache/images", "image_cache")
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; AresAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
...
AUDIO_CACHE_DIR = get_ares_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_ares_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_ares_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_ares_dir("cache/screenshots", "browser_screenshots")
_ARES_HOME = get_ares_home()
_ARES_ROOT = get_default_ares_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "ARES_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "ARES_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "ARES_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "ARES_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _ARES_HOME / "image_cache",
    _ARES_HOME / "audio_cache",
    _ARES_HOME / "video_cache",
    _ARES_HOME / "document_cache",
    _ARES_HOME / "browser_screenshots",
    ...
    _ARES_HOME / "cache" / "images",
    _ARES_HOME / "cache" / "audio",
    _ARES_HOME / "cache" / "videos",
    _ARES_HOME / "cache" / "documents",
    _ARES_HOME / "cache" / "screenshots",
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Ares profile and shared Ares root both contain control
    # files and credentials. Only cache subdirectories under them are
    # explicitly allowlisted above.
    for ares_root in (_ARES_HOME, _ARES_ROOT):
        denied.append(ares_root / ".env")
        denied.append(ares_root / "auth.json")
        denied.append(ares_root / "credentials")
        denied.append(ares_root / "config.yaml")
    return denied
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Ares
    secrets (``~/.ares/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``ARES_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Ares-managed cache, under an operator-allowlisted root
    (``ARES_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # The denylist still blocks /etc, /proc, ~/.ssh, ~/.aws, ~/.ares/.env,
    # ~/.ares/auth.json, etc. — so the obvious prompt-injection sites
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?ares\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+ares[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("ARES_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "ARES_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "ARES_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.ares/media/generated/stale.png"}
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          ARES_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          ARES_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          ARES_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("ARES_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("ARES_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("ARES_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _ares_run_generation is set on
