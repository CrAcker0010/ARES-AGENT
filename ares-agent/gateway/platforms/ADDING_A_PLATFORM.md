...
There are two ways to add a platform to the Ares gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.ares/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Ares code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `ares gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `ares_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Ares core.
...
And add it to the `ares-gateway` composite:

```python
"ares-gateway": {
    "includes": [..., "ares-your-platform"]
}
```
...
## 12. Status Display (`ares_cli/status.py`)
...
## 13. Gateway Setup Wizard (`ares_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ ares_cli/ toolsets.py \
  --include="*.py" -l | sort -u
