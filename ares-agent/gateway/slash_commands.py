...
Module-level run.py helpers a handler needs (``_ares_home``,
...
class GatewaySlashCommandsMixin:
        ...
        if action in {"pause", "resume"}:
            ...
            if action == "pause":
                ...
                return (
                    ...
                    f"`ares gateway restart` to reset."
                )
            # action == "resume"
            if platform not in failed:
                return (
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        from gateway.run import _ares_home
        ...
        try:
            ...
            atomic_json_write(
                _ares_home / ".restart_notify.json",
                notify_data,
                indent=None,
            )
        ...
        try:
            ...
            atomic_json_write(
                _ares_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running Ares Agent version."""
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        from ares_cli.commands import gateway_help_lines
        lines = [
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from gateway.run import _ares_home, _load_gateway_config
        import yaml
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — Ares default runtime
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return t("gateway.personality.none_configured", path=display_ares_home())
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        ...
        try:
            import yaml as _y
            _cfg_path = _ares_home / "config.yaml"
            if _cfg_path.exists():
    ...
    async def _handle_reasoning_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _ares_home, _platform_config_key
        import yaml
    ...
    async def _handle_fast_command(self, event: MessageEvent) -> str:
        ...
        from gateway.run import _ares_home, _load_gateway_config, _resolve_gateway_model
        import yaml
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _ares_home, _load_gateway_config, _platform_config_key
    ...
    async def _handle_footer_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _ares_home, _load_gateway_config, _platform_config_key, _resolve_gateway_model
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return (
                ...
                "  `ares bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        full log uploads should use ``ares debug share`` from the CLI.
        """
        import asyncio
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Ares Agent to the latest version.

        Spawns ``ares update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``ares update`` may trigger. Marker
        files are written so either the current gateway process or the next one
        can notify the user when the update finishes.
        """
        from gateway.run import _ares_home, _resolve_ares_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
        ...
        if is_managed():
            return f"✗ {format_managed_message('update Ares Agent')}"
        ...
        if not ares_cmd:
            return t("gateway.update.ares_cmd_not_found")
        ...
        # Spawn `ares update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `ares update --gateway`
        ...
        try:
            if sys.platform == "win32":
                ...
                # ares_cmd is a list of argv parts we can pass directly
                ...
                ares_cmd_str = " ".join(shlex.quote(part) for part in ares_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {ares_cmd_str} update --gateway"
