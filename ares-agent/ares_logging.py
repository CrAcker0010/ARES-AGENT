...
Component separation:
    ...
    gui.log receives dashboard-side records from ``ares_cli.web_server``,
    ``ares_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the ares group. However, both ``_open()`` (initial creation)
