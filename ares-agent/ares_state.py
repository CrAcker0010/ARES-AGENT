...
class SessionDB:
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `ares update` to rebuild the venv with a "
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._ares_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._ares_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another Ares surface and jump
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``ARES_DISABLE_FTS_TRIGRAM`` or not yet created), so
