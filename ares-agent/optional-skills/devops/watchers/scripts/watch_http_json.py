...
Usage (via cron with --no-agent):
    ...
    ares cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$ARES_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Ares-Watcher/1.0"})
