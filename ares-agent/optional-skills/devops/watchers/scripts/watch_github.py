...
Usage (via cron with --no-agent):
    ...
    ares cron create ares-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$ARES_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name ares-issues --repo NousResearch/ares-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.ares/.env to avoid the 60 req/hr
