...
ares kanban watch --tenant <project-slug>

# Snapshot of the board
ares kanban list --tenant <project-slug>
ares kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
ares kanban stats --tenant <project-slug>

# Visual dashboard (browser)
ares dashboard

# Inspect a specific task (includes comments + events)
ares kanban show <task-id>

# Follow a single task's event stream
ares kanban tail <task-id>
```

Verify available subcommands with `ares kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `ares kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `ares kanban block <id>` then `ares kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `ares kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `ares kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `ares profile list` (profile exists?), `ares status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(ares kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
ares kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
ares kanban block <task-id>
ares kanban archive <task-id>

# Diagnose what happened
ares kanban show <task-id>      # task body, comments, recent events
ares kanban tail <task-id>      # follow the live event stream
ares kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `ares kanban create` for the director
...
while true; do
    clear
    ares kanban list --tenant <slug>
    echo "---"
    ares kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `ares kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `ares profile list` and start
