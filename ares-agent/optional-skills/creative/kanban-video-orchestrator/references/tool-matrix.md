...
Maps each role archetype to the Ares skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public ares-agent
...
## Ares skills relevant to video production

### Visual / rendering skills (`ares-agent/skills/creative/`)
...
### Design / pre-production skills (`ares-agent/skills/creative/`)
...
### Audio / media skills (`ares-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`ares-agent/skills/devops/`)
...
These are **not** Ares skills but external CLIs / APIs that profiles invoke.
...
## Built-in Ares tools for media review

These are native Ares tools — not invoked via terminal but through their own
...
key is present in `~/.ares/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Ares profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.ares/.env` → environment variable.
...
When a new Ares-public video skill ships:
