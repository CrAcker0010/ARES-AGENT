...
the actual `setup.sh` that creates the project workspace, configures Ares
...
2. **Create profiles** — `ares profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.ares/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `ares kanban create` assigned to the director
...
```bash
ares profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.ares/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.ares/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Ares
...
Each profile gets a `SOUL.md` at `~/.ares/profiles/<name>/SOUL.md` that
...
```bash
ares kanban create "Direct production of <video title>" \
    --assignee director \
...
`~/.ares/.env` and macOS Keychain (if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    if grep -q "^${var}=" ~/.ares/.env 2>/dev/null && \
       [ -n "$(grep "^${var}=" ~/.ares/.env | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ~/.ares/.env or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY ares ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY ares OPENROUTER_API_KEY || exit 1
# ...
```
