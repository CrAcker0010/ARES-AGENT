#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `ares kanban list` and
...
def kanban_list(tenant: str) -> list[dict]:
    ...
    output isn't supported by the installed ares CLI."""
    try:
        out = subprocess.run(
            ["ares", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `ares kanban list`
    out = subprocess.run(
        ["ares", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def main():
    ...
    if not ares_available():
        print("ERROR: 'ares' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
