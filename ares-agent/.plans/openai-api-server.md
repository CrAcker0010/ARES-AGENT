# OpenAI-Compatible API Server for Ares Agent
...
SSE streaming. By exposing this endpoint, ares-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  ares-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.ares/.env`
2. Run `ares gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with ares-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns ares-agent as a model) |
...
Authorization: Bearer ares-api-key-here
...
But ares-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "ares-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured ares-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of ares-agent's core prompt
...
Once implemented, ares-agent works as a drop-in backend for:
