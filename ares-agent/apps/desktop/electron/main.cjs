...
const { canImportAresCli, verifyAresCli } = require('./backend-probes.cjs')
...
const USER_DATA_OVERRIDE = process.env.ARES_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.ARES_DESKTOP_DEV_SERVER
...
// switches only apply pre-launch. Override with ARES_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[ares] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[ares] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
          continue
        }
...
if (INSTALL_STAMP) {
  console.log(
    `[ares] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[ares] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// ARES_HOME — the user-facing root for everything Ares-related. Mirrors
// scripts/install.ps1's $AresHome and scripts/install.sh's $ARES_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\ares (matches install.ps1)
//   macOS / Linux: ~/.ares (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.ares directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\ares yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// ARES_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// ARES_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.ares / %LOCALAPPDATA%\ares.
function resolveAresHome() {
  if (process.env.ARES_HOME) return path.resolve(process.env.ARES_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'ares-home')
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'ares')
    const legacy = path.join(app.getPath('home'), '.ares')
    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.ares setup (no LOCALAPPDATA install yet) so users don't lose state.
    if (!directoryExists(localappdata) && directoryExists(legacy)) return legacy
    return localappdata
  }
  return path.join(app.getPath('home'), '.ares')
}

const ARES_HOME = resolveAresHome()
// ACTIVE_ARES_ROOT — the canonical mutable Ares install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_ARES_ROOT = path.join(ARES_HOME, 'ares-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_ARES_ROOT, 'venv')
...
// boots, going straight to `resolveAresBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_ARES_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_ARES_ROOT, '.ares-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
// active-profile.json records which Ares profile the desktop launches its
// local backend as. When set, startAres() passes `ares --profile <name>
// dashboard …`, which deterministically pins ARES_HOME (see
// _apply_profile_override in ares_cli/main.py) and bypasses the sticky
// ~/.ares/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors ares_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// aresDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under ARES_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by ares_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(ARES_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (ares_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.ARES_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.ARES_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
// an in-place `ares update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveAresVersion(),
...
const MEDIA_PROTOCOL = 'ares-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(ares-media://…)`.
...
// (the desktop's launch profile) stays managed by aresProcess +
// connectionPromise + startAres(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.ARES_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.ARES_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startAres() calls (e.g. the renderer's
...
function rememberLog(chunk) {
  ...
  aresLog.push(...lines)
  if (aresLog.length > 300) {
    aresLog.splice(0, aresLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-ares-wsl-windows-fonts.conf')
    let existing = ''
    try {
...
function broadcastBootProgress() {
  ...
  webContents.send('ares:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the ares:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('ares:bootstrap:event', ev)
}
...
function isAresSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'ares_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.ARES_DESKTOP_PYTHON
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\ares\git\... — checked
  // first so users who installed via install.ps1 are detected before we
  // start probing system-wide locations.
  const localAppData = process.env.LOCALAPPDATA || ''
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'ares', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'ares', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// install only has PortableGit under %LOCALAPPDATA%\ares\git (never on
...
function resolveGitBinary() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'ares', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'ares', 'git', 'bin', 'git.exe'))
  }
...
function recentAresLog() {
  return aresLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's ares root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_ARES_ROOT.
// ARES_DESKTOP_ARES_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.ARES_DESKTOP_ARES_ROOT && path.resolve(process.env.ARES_DESKTOP_ARES_ROOT),
    !IS_PACKAGED && isAresSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isAresSourceRoot(ACTIVE_ARES_ROOT) ? ACTIVE_ARES_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_ARES_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('ares:updates:progress', merged)
  }
}
...
// ARES_HOME/ares-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_ares_home). That binary owns
// ALL repo mutation — running `ares update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'ares-setup.exe' : 'ares-setup'
  const candidate = path.join(ARES_HOME, name)
...
// Path to the venv shim whose lock decides whether `ares update` can write
// fresh entry points. On Windows this is the file the running backend
// `ares.exe` holds open; on POSIX it's never mandatory-locked.
function venvAresShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'ares.exe')
    : path.join(updateRoot, 'venv', 'bin', 'ares')
}
...
// only signals the direct child, so on Windows a backend `ares.exe` that
// spawned its own grandchildren (a `ares` REPL, a pty terminal session, the
...
// `aresProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `ares.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a ares
// REPL, a pty terminal, the gateway) keep `ares.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (aresProcess && Number.isInteger(aresProcess.pid)) pids.push(aresProcess.pid)
  ...
  if (aresProcess && !aresProcess.killed) {
    try {
      aresProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvAresShimPath(updateRoot)
...
// `ares update`). Instead we spawn the staged Ares-Setup binary with
// --update and quit, so it can run `ares update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri ares-setup. Unlike Windows
      ...
      // whole update itself: `ares update` (backend) + `ares desktop
    ...
    if (!updater) {
      ...
      // `ares desktop`, never the Tauri installer that self-copies
      // ares-setup.exe into ARES_HOME). They DO have a working `ares`
      ...
      // checkout they're on — bare `ares update` defaults to main and would
      ...
      const updateRoot = resolveUpdateRoot()
      let command = 'ares update'
      try {
        const head = await runGit(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: updateRoot })
        const current = (head.stdout || '').trim()
        if (head.code === 0 && current && current !== 'HEAD') {
          const branch = await resolveHealedBranch(updateRoot, current)
          if (branch !== 'main') command = `ares update --branch ${branch}`
        }
      } catch {
        // Best-effort: fall back to bare `ares update` if branch detection fails.
      }
      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })
      return { ok: true, manual: true, command, aresRoot: updateRoot }
    }

    emitUpdateProgress({ stage: 'restart', message: 'Handing off to the Ares updater…', percent: 100 })
    ...
    // ares.exe (held by the backend child / its grandchildren) and the update
    // bricks. See releaseBackendLockForUpdate for the full failure analysis.
    await releaseBackendLockForUpdate(updateRoot)

    // Detached so the updater outlives this process — it needs us GONE before
    // `ares update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: ARES_HOME,
      env: {
        ...process.env,
        ARES_HOME,
        PATH: [path.join(ARES_HOME, 'node', 'bin'), venvBin, process.env.PATH].filter(Boolean).join(path.delimiter)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
...
// Resolve the ares CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `ares` on PATH.
function resolveAresCliBinary(updateRoot) {
  const venvAres = path.join(updateRoot, 'venv', 'bin', 'ares')
  if (fileExists(venvAres)) return venvAres
  return findOnPath('ares') || null
}
...
// macOS/Linux in-app update: backend (`ares update`) + OS-aware GUI rebuild
// (`ares desktop --build-only`), then atomically swap the running .app bundle
// with the freshly built one and relaunch. Degrades to "backend updated,
// restart to load the new GUI" if the swap can't be performed.
async function applyUpdatesPosixInApp() {
  const updateRoot = resolveUpdateRoot()
  const ares = resolveAresCliBinary(updateRoot)
  if (!ares) {
    emitUpdateProgress({ stage: 'manual', message: 'ares update', percent: null })
    return { ok: true, manual: true, command: 'ares update', aresRoot: updateRoot }
  }

  // Put the Ares-managed Node and the venv on PATH so `ares desktop`'s
  // npm build can find them on a machine with no system Node.
  const extraPath = [path.join(ARES_HOME, 'node', 'bin'), path.join(updateRoot, 'venv', 'bin')]
    .filter(Boolean)
    .join(path.delimiter)
  const env = {
    ARES_HOME,
  ...
  // `ares update` reaps stale `ares dashboard` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads ARES_DESKTOP_CHILD_PID and excludes
  ...
  if (aresProcess && Number.isInteger(aresProcess.pid)) {
    desktopChildPids.push(aresProcess.pid)
  }
  for (const entry of backendPool.values()) {
  ...
  if (desktopChildPids.length) {
    env.ARES_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  const updated = await runStreamedUpdate(ares, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'ares update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'ares update failed' }
  }

  emitUpdateProgress({ stage: 'rebuild', message: 'Rebuilding the desktop app…', percent: 60 })
  const rebuilt = await runStreamedUpdate(ares, ['desktop', '--build-only'], {
    cwd: updateRoot,
    env,
    stage: 'rebuild'
  })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.ares-update-new"; then
    rm -rf "$DST.ares-update-old" 2>/dev/null || true
    mv "$DST" "$DST.ares-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.ares-update-new" "$DST"
    rm -rf "$DST.ares-update-old" 2>/dev/null || true
  fi
fi
...
/usr/bin/open "$DST"
`
  const scriptPath = path.join(app.getPath('temp'), `ares-desktop-update-${Date.now()}.sh`)
  try {
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `ares
  ...
  return isAresSourceRoot(ACTIVE_ARES_ROOT) && fileExists(getVenvPython(VENV_ROOT))
}
...
function resolveWebDist() {
  const override = process.env.ARES_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set ARES_DESKTOP_WEB_DIST.`
    )
  }
  return fallback
}
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: ares desktop --force-build`
  )
  return candidates[0]
}

function resolveAresCwd() {
...
function createPythonBackend(root, label, dashboardArgs, options = {}) {
  ...
  return {
    ...
    label,
    command: python,
    args: ['-m', 'ares_cli.main', ...dashboardArgs],
    env: {
...
// createActiveBackend — build a backend pointing at ACTIVE_ARES_ROOT, the
...
function createActiveBackend(dashboardArgs) {
  ...
  return {
    kind: 'python',
    label: `Ares at ${ACTIVE_ARES_ROOT}`,
    command: fileExists(venvPython) ? venvPython : findSystemPython(),
    args: ['-m', 'ares_cli.main', ...dashboardArgs],
    env: {
      PYTHONPATH: [ACTIVE_ARES_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
    },
    root: ACTIVE_ARES_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveAresBackend(dashboardArgs) {
  // 1. Explicit override -- ARES_DESKTOP_ARES_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.ARES_DESKTOP_ARES_ROOT && path.resolve(process.env.ARES_DESKTOP_ARES_ROOT)
  if (overrideRoot && isAresSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Ares source at ${overrideRoot}`, dashboardArgs)
  ...
  //    installed `ares` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isAresSourceRoot.)
  if (!IS_PACKAGED && isAresSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Ares source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
    if (backend) return backend
  }

  // 3. Bootstrap-complete ACTIVE_ARES_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\ares\ares-agent (Windows) or ~/.ares/ares-agent.
  ...
  //    to spawning ares. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `ares update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(dashboardArgs)
  }

  // 4. Existing `ares` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    ARES_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.ARES_DESKTOP_IGNORE_EXISTING !== '1') {
    let aresCommand = null
    const aresOverride = process.env.ARES_DESKTOP_ARES

    if (aresOverride) {
      const resolvedOverride = findOnPath(aresOverride)
      if (resolvedOverride) {
        aresCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(aresOverride, { isWsl: IS_WSL })) {
        aresCommand = aresOverride
      } else {
        rememberLog(`Ignoring Windows Ares override under WSL: ${aresOverride}`)
      }
    } else {
      aresCommand = findOnPath('ares')
    }

    if (aresCommand) {
      if (looksLikeDesktopAppBinary(aresCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Ares CLI: ${aresCommand}`)
        aresCommand = null
      }
    }

    if (aresCommand) {
      // Smoke-test the candidate before trusting it. A `ares` shim
      ...
      const shellForProbe = isCommandScript(aresCommand)
      if (verifyAresCli(aresCommand, { shell: shellForProbe })) {
        return {
          label: `existing Ares CLI at ${aresCommand}`,
          command: aresCommand,
          args: dashboardArgs,
          bootstrap: false,
          env: {},
      ...
      rememberLog(
        `Ignoring existing Ares CLI at ${aresCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed ares_cli module via system Python.
  ...
  if (python) {
    ...
    // having ares_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\ares\ares-agent\venv.
    if (canImportAresCli(python)) {
      return {
        kind: 'python',
        label: `installed ares_cli module via ${python}`,
        command: python,
        args: ['-m', 'ares_cli.main', ...dashboardArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: ares_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveAresBackend was the old "no payload" path and forced the
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveAresBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    const bootstrapResult = await runBootstrap({
      ...
      aresHome: ARES_HOME,
      logRoot: path.join(ARES_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Ares install was cancelled.')
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Ares bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          ...
          `Check ${path.join(ARES_HOME, 'logs', 'desktop.log')} for the full transcript.`
      )
      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startAres() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // ares:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveAresBackend(backend.args))
  }
  ...
  if (!isAresSourceRoot(ACTIVE_ARES_ROOT)) {
    throw new Error(
      `Ares install at ${ACTIVE_ARES_ROOT} is missing or incomplete. ` +
  ...
  // %LOCALAPPDATA%\ares\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `ares` on PATH, this check still helps.
  ...
  if (!fileExists(venvPython)) {
    ...
    // isAresSourceRoot() and the bootstrap writes the marker only after
  ...
  backend.label = `Ares at ${ACTIVE_ARES_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('ares:link-titles', { cache: false })
...
function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveAresCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('ares:preview-file-changed', payload)
}
...
async function waitForAres(baseUrl, token) {
...
function sendBackendExit(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('ares:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('ares:close-preview-requested')
}
...
function sendPowerResume() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('ares:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('ares:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen) {
  ...
  webContents.send('ares:window-state-changed', state)
}
...
const ZOOM_STORAGE_KEY = 'ares:desktop:zoomLevel'
...
//
//   * REST is authed by HttpOnly session cookies (``ares_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / ares #37247).
//     Both are set as HttpOnly cookies (``ares_session_at`` ~15 min,
//     ``ares_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:ares-remote-oauth'
...
// a valid stored value (pins the root ARES_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.ARES_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.ARES_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    // scope is never overridden by ARES_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (ares #37247), and the
...
//   2. env override (ARES_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.ARES_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.ARES_DESKTOP_REMOTE_TOKEN
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'ARES_DESKTOP_REMOTE_URL is set but ARES_DESKTOP_REMOTE_TOKEN is not. ' +
...
function globalRemoteActive() {
  if (process.env.ARES_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startAres())
...
// startAres() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetAresConnection() nulls aresProcess.
  const dying = aresProcess && !aresProcess.killed ? aresProcess : null
  resetAresConnection()
...
// profile to startAres() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startAres() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForAres(remote.baseUrl, remote.token)
    return {
      ...remote,
      profile,
      logs: aresLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  // --profile wins over the inherited ARES_HOME env (see _apply_profile_override
  // step 3 in ares_cli/main.py), so the child re-homes to this profile.
  const dashboardArgs = ['--profile', profile, 'dashboard', '--no-open', '--host', '127.0.0.1', '--port', String(port)]
  const backend = await ensureRuntime(resolveAresBackend(dashboardArgs))
  const aresCwd = resolveAresCwd()
  ...
  const child = spawn(backend.command, backend.args, {
    ...
    env: {
      ...
      ARES_HOME,
      ...backend.env,
      ARES_DASHBOARD_SESSION_TOKEN: token,
      // Marks this dashboard backend as desktop-spawned so it runs the cron
      // scheduler tick loop (the gateway isn't running under the app).
      ARES_DESKTOP: '1',
      ARES_WEB_DIST: webDist
    },
    shell: backend.shell,
    stdio: ['ignore', 'pipe', 'pipe']
  })
  ...
  child.once('exit', (code, signal) => {
    ...
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(new Error(`Ares backend for profile "${profile}" exited before it became ready (${signal || code}).`))
    }
  })

  const baseUrl = `http://127.0.0.1:${port}`
  await Promise.race([waitForAres(baseUrl, token), startFailed])
  ready = true
...
async function startAres() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startAres() call re-throws the same error
  ...
  connectionPromise = (async () => {
    ...
    // deterministic (it wins over the sticky ~/.ares/active_profile file) and
    // resolves ARES_HOME the same way `ares -p <name>` does on the CLI. An
    ...
    aresProcess = spawn(backend.command, backend.args, {
      cwd: aresCwd,
      env: {
        ...process.env,
        // Explicitly pin ARES_HOME for the child so Python's get_ares_home()
        // resolves to the SAME location our resolveAresHome() picked. Without
        // this pin, Python falls back to ~/.ares on every platform — fine on
        // mac/linux (where our default matches), but on Windows our default is
        // %LOCALAPPDATA%\ares, which differs from C:\Users\<u>\.ares.
        // Mismatch would split config / sessions / .env / logs across two
        // directories. install.ps1 sets ARES_HOME via setx; the desktop
    ...
    aresProcess.stdout.on('data', rememberLog)
    aresProcess.stderr.on('data', rememberLog)
    let backendReady = false
    let rejectBackendStart = null
    const backendStartFailed = new Promise((_resolve, reject) => {
      rejectBackendStart = reject
    })
    aresProcess.once('error', error => {
    ...
    aresProcess.once('exit', (code, signal) => {
    ...
    await Promise.race([waitForAres(baseUrl, token), backendStartFailed])
    backendReady = true
    updateBootProgress({
      phase: 'backend.ready',
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startAres().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('ares:connection', async (_event, profile) => ensureBackend(profile))
...
ipcMain.handle('ares:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetAresConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Ares backend failed liveness probe; dropping stale connection.')
    resetAresConnection()
    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('ares:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)
  return { ok: true }
})
ipcMain.handle('ares:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('ares:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startAres() call restarts the
...
ipcMain.handle('ares:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startAres() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('ares:bootstrap:cancel', async () => {
...
ipcMain.handle('ares:boot-progress:get', async () => bootProgressState)
ipcMain.handle('ares:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('ares:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('ares:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('ares:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('ares:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('ares:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('ares:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('ares:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('ares:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('ares:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new ARES_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('ares:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('ares:requestMicrophoneAccess', async () => {
...
ipcMain.handle('ares:api', async (_event, request) => {
...
ipcMain.handle('ares:notify', (_event, payload) => {
...
ipcMain.handle('ares:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('ares:readFileText', async (_event, filePath) => {
...
ipcMain.handle('ares:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('ares:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))
  return true
})

ipcMain.handle('ares:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('ares:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('ares:saveClipboardImage', async () => {
...
ipcMain.handle('ares:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('ares:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('ares:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('ares:titlebar-theme', (_event, payload) => {
...
ipcMain.handle('ares:openExternal', (_event, url) => {
...
// it via writeDefaultProjectDir so resolveAresCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('ares:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: path.join(app.getPath('home'), 'ares-projects')
}))

ipcMain.handle('ares:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('ares:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('ares:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('ares:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('ares:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: aresLog.slice(-200) }))
...
function terminalChannel(id, suffix) {
  return `ares:terminal:${id}:${suffix}`
}
...
ipcMain.handle('ares:fs:readDir', async (_event, dirPath) => {
...
ipcMain.handle('ares:fs:gitRoot', async (_event, startPath) => {
...
ipcMain.handle('ares:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Ares.')
  }
...
ipcMain.handle('ares:terminal:write', (_event, id, data) => {
...
ipcMain.handle('ares:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('ares:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('ares:updates:check', async () =>
...
ipcMain.handle('ares:updates:apply', async (_event, payload) =>
...
ipcMain.handle('ares:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('ares:updates:branch:set', async (_event, name) => {
...
// ares_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Ares version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveAresVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'ares_cli', '__init__.py')
...
// just before showing it, so an in-place `ares update` is reflected without
...
ipcMain.handle('ares:version', async () => ({
  appVersion: resolveAresVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  aresRoot: resolveUpdateRoot()
}))
...
// the actual removal via `ares uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (ares_cli/uninstall.py + ares_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_ARES_ROOT
  ...
  const fallback = () => ({
    ares_home: ARES_HOME,
    agent_installed: isAresSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(py, ['-m', 'ares_cli.main', 'uninstall', '--gui-summary'], {
        cwd: agentRoot,
        env: { ...process.env, ARES_HOME, NO_COLOR: '1' },
        stdio: ['ignore', 'pipe', 'ignore']
      })
      child.stdout.on('data', chunk => {
...
async function runDesktopUninstall(mode) {
  ...
  // Python with PYTHONPATH=<agentRoot> so `import ares_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_ARES_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `ares-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `ares-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('ares:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('ares:uninstall:run', async (_event, payload) => {
