...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'ares-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/ares-runner-test',
    sourceRepoRoot: null,
    aresHome: '/tmp/ares-runner-test',
    logRoot: '/tmp/ares-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'ares-agent', 'scripts')
