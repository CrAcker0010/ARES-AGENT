 ...
 *
 * Drives apps/desktop's first-launch install of Ares Agent by spawning
...
function bootstrapCacheDir(aresHome) {
  return path.join(aresHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.ares/ares-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(aresHome) {
  if (!aresHome) return null
  const candidate = path.join(aresHome, 'ares-agent', 'scripts', installScriptName())
  try {
    fs.accessSync(candidate, fs.constants.R_OK)
    return candidate
  } catch {
...
function cachedScriptPath(aresHome, commit) {
  return path.join(bootstrapCacheDir(aresHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(commit, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/NousResearch/ares-agent/${commit}/scripts/${scriptName}`
...
async function resolveInstallScript({ installStamp, sourceRepoRoot, aresHome, emit, _download = downloadInstallScript }) {
  ...
  const cached = cachedScriptPath(aresHome, installStamp.commit)
  try {
  ...
  try {
    ...
    const installed = installedAgentInstallScript(aresHome)
    if (installed) {
      emit({
        type: 'log',
        line:
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, aresHome } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(ps, fullArgs, {
      ...
      env: {
        ...process.env,
        // Pass ARES_HOME through so install.ps1 respects the caller's
        // choice rather than re-computing the default.
        ARES_HOME: aresHome || process.env.ARES_HOME || ''
      }
    })
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, aresHome } = {}) {
  return new Promise((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, aresHome }) {
  const args = ['--dir', activeRoot, '--ares-home', aresHome]
  if (installStamp && installStamp.branch) {
    args.push('--branch', installStamp.branch)
  }
  if (installStamp && installStamp.commit) {
...
async function fetchManifest({ scriptPath, installerKind, emit, aresHome, activeRoot, installStamp }) {
  const isPosix = installerKind === 'posix'
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, aresHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, aresHome, activeRoot, abortSignal, installStamp }) {
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, aresHome })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(aresHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, aresHome, emit })
