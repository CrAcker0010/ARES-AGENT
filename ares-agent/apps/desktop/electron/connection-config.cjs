 ...
 *     `X-Ares-Session-Token` header; WS uses `?token=`.
...
// ares_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - ares_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - ares_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / ares #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-ares_session_at', '__Secure-ares_session_at', 'ares_session_at']
const RT_COOKIE_VARIANTS = ['__Host-ares_session_rt', '__Secure-ares_session_rt', 'ares_session_rt']
...
/**
 * True if any cookie in `cookies` is a ares session ACCESS-token cookie
