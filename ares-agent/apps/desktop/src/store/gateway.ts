import type { ConnectionState, GatewayEvent } from '@ares/shared'
...
const isOpen = (gateway: AresGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<AresGateway | null>(null)
...
let primaryGateway: AresGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: AresGateway | null, profile = 'default'): void {
...
export function activeGateway(): AresGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<AresGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
