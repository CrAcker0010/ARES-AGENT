...
import { getProfiles, setApiRequestProfile } from '@/ares'
...
import type { ProfileInfo } from '@/types/ares'
...
// /api/profiles/active `current`). "default" is the root ~/.ares. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'ares.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'ares.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.aresDesktop.api<ActiveProfileResponse>({ path: '/api/profiles/active' })
...
// Persist the choice and relaunch the backend under the new ARES_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.aresDesktop.profile.set(name)
}
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'ares.desktop.showAllProfiles'
...
// Switch to the default (root ~/.ares) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.aresDesktop?.touchBackend?.(target).catch(() => undefined)
}
