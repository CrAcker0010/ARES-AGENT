...
import type { ModelOptionProvider, OAuthProvider, OAuthStartResponse } from '@/types/ares'
...
const CONFIGURED_CACHE_KEY = 'ares-desktop-onboarded-v1'
const SKIP_CACHE_KEY = 'ares-onboarding-skipped-v1'
const POLL_MS = 2000
...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'Ares is ready', message: `${provider} connected.` })
}

// Human-friendly labels for tools auto-routed through the Nous Tool Gateway,
// mirroring ares_cli/nous_subscription._GATEWAY_TOOL_LABELS so the GUI and
...
async function fetchProviderDefaultModel(
  ...
  // `ares model` does (for Nous it honors the user's free/paid tier, so a
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but Ares still cannot resolve a usable provider. ${detail}`
    : 'Connected, but Ares still cannot resolve a usable provider.'
}
...
async function openSignInUrl(url: string) {
  if (window.aresDesktop?.openExternal) {
    try {
      await window.aresDesktop.openExternal(url)
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `Ares still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Ares still cannot reach ${url}.` }
    }
