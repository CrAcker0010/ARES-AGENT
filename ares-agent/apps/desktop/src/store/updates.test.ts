...
const checkAresUpdateSpy = vi.fn()
const updateAresSpy = vi.fn()
const getActionStatusSpy = vi.fn()

vi.mock('@/ares', () => ({
  checkAresUpdate: (...args: unknown[]) => checkAresUpdateSpy(...args),
  updateAres: (...args: unknown[]) => updateAresSpy(...args),
...
describe('checkBackendUpdates', () => {
  ...
  it('maps the backend /update/check onto the backend status, including commits', async () => {
    setRemote(true)
    checkAresUpdateSpy.mockResolvedValue({
      install_method: 'git',
    ...
    expect(checkAresUpdateSpy).toHaveBeenCalled()
  ...
  it('is a no-op in local mode (backend check only runs when remote)', async () => {
    ...
    expect(checkAresUpdateSpy).not.toHaveBeenCalled()
  })
})
...
describe('applyBackendUpdate recovery', () => {
  ...
  it('waits for the backend to return after the restart drops the connection, then clears the overlay', async () => {
    updateAresSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValue(new Error('ECONNREFUSED'))
    checkAresUpdateSpy.mockResolvedValue({ install_method: 'git', current_version: '0.16.0', behind: 0, update_available: false, can_apply: true, update_command: 'ares update', message: null })
  ...
  it('surfaces an error when the backend never comes back after the restart', async () => {
    updateAresSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValue(new Error('ECONNREFUSED'))
    checkAresUpdateSpy.mockRejectedValue(new Error('ECONNREFUSED'))
