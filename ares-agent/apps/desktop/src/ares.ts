import { JsonRpcGatewayClient } from '@ares/shared'
...
export class AresGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Ares gateway connection closed',
      connectErrorMessage: 'Could not connect to Ares gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Ares gateway is not connected',
...
// the call; each pooled backend already has its own ARES_HOME, so no backend
...
export async function listSessions(
  ...
  const result = await window.aresDesktop.api<PaginatedSessions>({
...
export function setSessionArchived(id: string, archived: boolean, profile?: string | null): Promise<{ ok: boolean }> {
  return window.aresDesktop.api<{ ok: boolean }>({
    ...(profile ? { profile } : {}),
...
export function searchSessions(query: string): Promise<SessionSearchResponse> {
  return window.aresDesktop.api<SessionSearchResponse>({
...
export function getSessionMessages(id: string, profile?: string | null): Promise<SessionMessagesResponse> {
  ...
  return window.aresDesktop.api<SessionMessagesResponse>({
...
export function renameSession(
  ...
  return window.aresDesktop.api<{ ok: boolean; title: string }>({
    ...(profile ? { profile } : {}),
    path: `/api/sessions/${encodeURIComponent(id)}`,
    method: 'PATCH',
...
export function getGlobalModelInfo(): Promise<ModelInfoResponse> {
  return window.aresDesktop.api<ModelInfoResponse>({
    ...profileScoped(),
    path: '/api/model/info'
  })
}
...
export function getStatus(): Promise<StatusResponse> {
  return window.aresDesktop.api<StatusResponse>({
    path: '/api/status'
  })
}
...
export function getLogs(params: {
  ...
  return window.aresDesktop.api<LogsResponse>({
    ...profileScoped(),
    path: suffix ? `/api/logs?${suffix}` : '/api/logs'
  })
}

export function getAresConfig(): Promise<AresConfig> {
  return window.aresDesktop.api<AresConfig>({
    ...profileScoped(),
    path: '/api/config'
  })
}

export function getAresConfigRecord(): Promise<AresConfigRecord> {
  return window.aresDesktop.api<AresConfigRecord>({
    ...profileScoped(),
    path: '/api/config'
  })
}

export function getAresConfigDefaults(): Promise<AresConfigRecord> {
  return window.aresDesktop.api<AresConfigRecord>({
    ...profileScoped(),
...
export function getAresConfigSchema(): Promise<ConfigSchemaResponse> {
  return window.aresDesktop.api<ConfigSchemaResponse>({
    ...profileScoped(),
    path: '/api/config/schema'
  })
}

export function saveAresConfig(config: AresConfigRecord): Promise<{ ok: boolean }> {
  return window.aresDesktop.api<{ ok: boolean }>({
    ...profileScoped(),
    path: '/api/config',
    method: 'PUT',
...
export function getEnvVars(): Promise<Record<string, EnvVarInfo>> {
  return window.aresDesktop.api<Record<string, EnvVarInfo>>({
    ...profileScoped(),
    path: '/api/env'
  })
}
...
export function validateProviderCredential(
  ...
  return window.aresDesktop.api<{ ok: boolean; reachable: boolean; message: string; models?: string[] }>({
    ...profileScoped(),
...
export function revealEnvVar(key: string): Promise<{ key: string; value: string }> {
  return window.aresDesktop.api<{ key: string; value: string }>({
    ...profileScoped(),
    path: '/api/env/reveal',
    method: 'POST',
    body: { key }
...
export function listOAuthProviders(): Promise<OAuthProvidersResponse> {
  return window.aresDesktop.api<OAuthProvidersResponse>({
    ...profileScoped(),
...
export function startOAuthLogin(providerId: string): Promise<OAuthStartResponse> {
  return window.aresDesktop.api<OAuthStartResponse>({
    ...profileScoped(),
...
export function submitOAuthCode(providerId: string, sessionId: string, code: string): Promise<OAuthSubmitResponse> {
  return window.aresDesktop.api<OAuthSubmitResponse>({
    ...profileScoped(),
...
export function pollOAuthSession(providerId: string, sessionId: string): Promise<OAuthPollResponse> {
  return window.aresDesktop.api<OAuthPollResponse>({
    ...profileScoped(),
...
export function getSkills(): Promise<SkillInfo[]> {
  return window.aresDesktop.api<SkillInfo[]>({
    ...profileScoped(),
    path: '/api/skills'
  })
}
...
export function toggleSkill(name: string, enabled: boolean): Promise<{ ok: boolean; name: string; enabled: boolean }> {
  return window.aresDesktop.api<{ ok: boolean; name: string; enabled: boolean }>({
    ...profileScoped(),
...
export function getToolsets(): Promise<ToolsetInfo[]> {
  return window.aresDesktop.api<ToolsetInfo[]>({
    ...profileScoped(),
...
export function getToolsetConfig(name: string): Promise<ToolsetConfig> {
  return window.aresDesktop.api<ToolsetConfig>({
    ...profileScoped(),
...
export function selectToolsetProvider(
  ...
  return window.aresDesktop.api<{ ok: boolean; name: string; provider: string }>({
    ...profileScoped(),
...
export function runToolsetPostSetup(name: string, key: string): Promise<ActionResponse & { key: string }> {
  return window.aresDesktop.api<ActionResponse & { key: string }>({
    ...profileScoped(),
...
export function getMessagingPlatforms(): Promise<MessagingPlatformsResponse> {
  return window.aresDesktop.api<MessagingPlatformsResponse>({
...
export function updateMessagingPlatform(
  ...
  return window.aresDesktop.api<{ ok: boolean; platform: string }>({
...
export function testMessagingPlatform(platformId: string): Promise<MessagingPlatformTestResponse> {
  return window.aresDesktop.api<MessagingPlatformTestResponse>({
...
export function getCronJobs(): Promise<CronJob[]> {
  return window.aresDesktop.api<CronJob[]>({
    path: '/api/cron/jobs'
  })
}
...
export async function getCronJobRuns(jobId: string, limit = 20): Promise<SessionInfo[]> {
  const { runs } = await window.aresDesktop.api<{ runs: SessionInfo[] }>({
...
export function getProfiles(): Promise<ProfilesResponse> {
  return window.aresDesktop.api<ProfilesResponse>({
    path: '/api/profiles'
  })
}

export function createProfile(body: ProfileCreatePayload): Promise<{ name: string; ok: boolean; path: string }> {
  return window.aresDesktop.api<{ name: string; ok: boolean; path: string }>({
    path: '/api/profiles',
    method: 'POST',
    body
  })
...
export function deleteProfile(name: string): Promise<{ ok: boolean; path: string }> {
  return window.aresDesktop.api<{ ok: boolean; path: string }>({
    path: `/api/profiles/${encodeURIComponent(name)}`,
    method: 'DELETE'
  })
}
...
export function getProfileSoul(name: string): Promise<ProfileSoul> {
  return window.aresDesktop.api<ProfileSoul>({
...
export function getProfileSetupCommand(name: string): Promise<ProfileSetupCommand> {
  return window.aresDesktop.api<ProfileSetupCommand>({
    path: `/api/profiles/${encodeURIComponent(name)}/setup-command`
  })
}

export function getUsageAnalytics(days = 30): Promise<AnalyticsResponse> {
  return window.aresDesktop.api<AnalyticsResponse>({
    ...profileScoped(),
...
export function getGlobalModelOptions(): Promise<ModelOptionsResponse> {
  return window.aresDesktop.api<ModelOptionsResponse>({
    ...profileScoped(),
...
// curation `ares model` does — for Nous it honors the free/paid tier so a
// free user gets a free model instead of a paid default.
export function getRecommendedDefaultModel(provider: string): Promise<RecommendedDefaultModel> {
  return window.aresDesktop.api<RecommendedDefaultModel>({
    ...profileScoped(),
...
export function setGlobalModel(
  ...
  return window.aresDesktop.api<{ ok: boolean; provider: string; model: string }>({
    ...profileScoped(),
    path: '/api/model/set',
    method: 'POST',
    body: {
...
export function getAuxiliaryModels(): Promise<AuxiliaryModelsResponse> {
  return window.aresDesktop.api<AuxiliaryModelsResponse>({
    ...profileScoped(),
...
export function setModelAssignment(body: ModelAssignmentRequest): Promise<ModelAssignmentResponse> {
  return window.aresDesktop.api<ModelAssignmentResponse>({
    ...profileScoped(),
    path: '/api/model/set',
    method: 'POST',
    body
...
export function restartGateway(): Promise<ActionResponse> {
  return window.aresDesktop.api<ActionResponse>({
    path: '/api/gateway/restart',
    method: 'POST'
  })
}

export function updateAres(): Promise<ActionResponse> {
  return window.aresDesktop.api<ActionResponse>({
    path: '/api/ares/update',
    method: 'POST'
  })
}
...
export function checkAresUpdate(force = false): Promise<BackendUpdateCheckResponse> {
  return window.aresDesktop.api<BackendUpdateCheckResponse>({
    path: `/api/ares/update/check${force ? '?force=true' : ''}`
  })
}

export function getActionStatus(name: string, lines = 200): Promise<ActionStatusResponse> {
  return window.aresDesktop.api<ActionStatusResponse>({
    path: `/api/actions/${encodeURIComponent(name)}/status?lines=${Math.max(1, lines)}`
  })
}

export function transcribeAudio(dataUrl: string, mimeType?: string): Promise<AudioTranscriptionResponse> {
  return window.aresDesktop.api<AudioTranscriptionResponse>({
...
export function speakText(text: string): Promise<AudioSpeakResponse> {
  return window.aresDesktop.api<AudioSpeakResponse>({
...
export function getElevenLabsVoices(): Promise<ElevenLabsVoicesResponse> {
  return window.aresDesktop.api<ElevenLabsVoicesResponse>({
