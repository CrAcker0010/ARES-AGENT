...
import type { AresConfigRecord } from '@/types/ares'
...
describe('settings helpers', () => {
  ...
  it('reads and writes nested config paths', () => {
    const config: AresConfigRecord = { display: { theme: 'mono' } }
  ...
  describe('providerGroup', () => {
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // ARES_QWEN_ and ARES_GEMINI_ both share the ARES_ stem.
      expect(providerGroup('ARES_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
      expect(providerGroup('ARES_GEMINI_CLIENT_ID')).toBe('Gemini')
    })
