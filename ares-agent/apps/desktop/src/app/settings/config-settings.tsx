...
import type { ConfigFieldSchema, AresConfigRecord } from '@/types/ares'
...
export function ConfigSettings({
  ...
  const { t } = useI18n()
  const c = t.settings.config
  const [config, setConfig] = useState<AresConfigRecord | null>(null)
  const [_defaults, setDefaults] = useState<AresConfigRecord | null>(null)
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([getAresConfigRecord(), getAresConfigDefaults(), getAresConfigSchema()])
      .then(([c, d, s]) => {
        if (cancelled) {
          return
        }
  ...
  const updateConfig = (next: AresConfigRecord) => {
