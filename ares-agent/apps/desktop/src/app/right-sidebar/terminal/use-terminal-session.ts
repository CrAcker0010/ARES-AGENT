...
const ARES_PATHS_MIME = 'application/x-ares-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(ARES_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(ARES_PATHS_MIME)
...
export function useTerminalSession({ cwd, onAddSelectionToChat }: UseTerminalSessionOptions) {
  ...
  useEffect(() => {
    ...
    const terminalApi = window.aresDesktop?.terminal
    ...
    try {
      ...
      console.warn('[ares-terminal] WebGL unavailable; falling back to DOM', err)
    }
