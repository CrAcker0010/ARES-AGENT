...
import type { AresReadDirResult } from '@/global'

import { resetProjectTreeState, useProjectTree } from './use-project-tree'

const readDir = vi.fn<(path: string) => Promise<AresReadDirResult>>()

beforeEach(() => {
  resetProjectTreeState()
  readDir.mockReset()
  ;(window as unknown as { aresDesktop: { readDir: typeof readDir } }).aresDesktop = { readDir }
})

afterEach(() => {
  resetProjectTreeState()
  delete (window as unknown as { aresDesktop?: unknown }).aresDesktop
})

function ok(entries: { name: string; path: string; isDirectory: boolean }[]): AresReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: AresReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<AresReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.aresDesktop is missing', async () => {
    delete (window as unknown as { aresDesktop?: unknown }).aresDesktop
