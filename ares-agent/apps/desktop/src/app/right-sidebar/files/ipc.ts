...
import type { AresReadDirEntry, AresReadDirResult } from '@/global'

export type ProjectTreeEntry = AresReadDirEntry
...
async function gitRootFor(start: string) {
  ...
  if (!cached) {
    cached = window.aresDesktop.gitRoot(key)
...
async function readGitignore(dir: string): Promise<GitignoreRule | null> {
  if (!window.aresDesktop?.readDir || !window.aresDesktop.readFileDataUrl) {
    return null
  }

  try {
    const listing = await window.aresDesktop.readDir(dir)

    if (!listing.entries.some(e => e.name === '.gitignore' && !e.isDirectory)) {
      return null
    }

    const text = decodeDataUrl(await window.aresDesktop.readFileDataUrl(`${dir}/.gitignore`))
...
function ignoredBy(rules: GitignoreRule[], entry: AresReadDirEntry) {
...
async function filterIgnored(entries: AresReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<AresReadDirResult> {
  if (!window.aresDesktop) {
    return { entries: [], error: 'no-bridge' }
  }

  const result = await window.aresDesktop.readDir(dirPath)
