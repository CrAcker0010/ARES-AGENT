...
// bug that drives the REAL useGatewayBoot hook + REAL AresGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshAresConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { aresDesktop?: unknown }).aresDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { aresDesktop?: unknown }).aresDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForAres) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startAres()'s remote branch awaits waitForAres() for 45s before it
    ...
    ;(window as { aresDesktop?: unknown }).aresDesktop = desktop
    ...
    // After ~45s waitForAres gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Ares backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
