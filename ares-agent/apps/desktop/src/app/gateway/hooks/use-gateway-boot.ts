...
import type { AresConnection } from '@/global'
import { AresGateway } from '@/ares'
...
import type { RpcEvent } from '@/types/ares'

interface GatewayBootOptions {
  handleGatewayEvent: (event: RpcEvent) => void
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.aresDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: AresGateway | null) => void
  refreshAresConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    let cancelled = false
    const desktop = window.aresDesktop

    const publish = (next: AresConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Ares...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Ares…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Ares gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshAresConfig().catch(() => undefined)
    ...
    async function boot() {
      try {
        ...
        await callbacksRef.current.refreshAresConfig()
