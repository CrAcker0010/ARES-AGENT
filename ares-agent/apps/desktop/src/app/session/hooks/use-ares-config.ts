...
import { getAresConfig, getAresConfigDefaults } from '@/ares'
...
interface AresConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useAresConfig({ activeSessionIdRef, refreshProjectBranch }: AresConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshAresConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getAresConfig(), getAresConfigDefaults().catch(() => ({}))])
  ...
  return { refreshAresConfig, sttEnabled, voiceMaxRecordingSeconds }
}
