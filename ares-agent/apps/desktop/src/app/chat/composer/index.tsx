...
import { aresDirectiveFormatter } from '@/components/assistant-ui/directive-text'
...
import { extractDroppedFiles, ARES_PATHS_MIME } from '../hooks/use-composer-actions'
...
export function ChatBar({
  ...
  // cold start ("Starting Ares...") from a dropped connection we're trying to
  ...
  const replaceTriggerWithChip = (item: Unstable_TriggerItem) => {
    ...
    const serialized = aresDirectiveFormatter.serialize(item)
  ...
  const handleDragEnter = (event: ReactDragEvent<HTMLFormElement>) => {
    if (!onAttachDroppedItems || !dragHasAttachments(event.dataTransfer, ARES_PATHS_MIME)) {
      return
    }
  ...
  const handleDrop = (event: ReactDragEvent<HTMLFormElement>) => {
    ...
    if (Array.from(event.dataTransfer.types || []).includes(ARES_PATHS_MIME)) {
  ...
  const handleInputDragOver = (event: ReactDragEvent<HTMLDivElement>) => {
    if (!dragHasAttachments(event.dataTransfer, ARES_PATHS_MIME)) {
      return
    }
