...
import { ARES_PATHS_MIME } from '@/app/chat/hooks/use-composer-actions'
...
async function readTextPreview(filePath: string) {
  if (window.aresDesktop.readFileText) {
    try {
      return await window.aresDesktop.readFileText(filePath)
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error)

      if (!message.includes("No handler registered for 'ares:readFileText'")) {
        throw error
      }
    }
  }
  ...
  const dataUrl = await window.aresDesktop.readFileDataUrl(filePath)
...
function startLineDrag(event: ReactDragEvent<HTMLElement>, filePath: string, { end, start }: LineSelection) {
  ...
  event.dataTransfer.setData(ARES_PATHS_MIME, JSON.stringify([{ line: start, lineEnd, path: filePath }]))
