        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.aresDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Ares to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      target.kind !== 'file' ||
      !window.aresDesktop?.watchPreviewFile ||
      !window.aresDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.aresDesktop.onPreviewFileChanged(payload => {
    ...
    void window.aresDesktop
      .watchPreviewFile(target.url)
      .then(watch => {
        if (!active) {
          void window.aresDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.aresDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:ares-preview')
