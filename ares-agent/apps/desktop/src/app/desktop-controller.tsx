...
import { getCronJobs, getSessionMessages, listAllProfileSessions, type SessionInfo, triggerCronJob } from '../ares'
...
import { useAresConfig } from './session/hooks/use-ares-config'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.aresDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.aresDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.aresDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshAresConfig, sttEnabled, voiceMaxRecordingSeconds } = useAresConfig({
    activeSessionIdRef,
