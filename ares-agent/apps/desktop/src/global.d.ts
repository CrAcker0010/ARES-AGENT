...
declare global {
  interface Window {
    aresDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<AresConnection>
      ...
      profile: {
        ...
        // backend under the new ARES_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: AresApiRequest) => Promise<T>
      notify: (payload: AresNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<AresReadFileTextResult>
      selectPaths: (options?: AresSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<AresPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<AresPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: AresTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<AresReadDirResult>
      gitRoot?: (path: string) => Promise<string | null>
      terminal: {
        dispose: (id: string) => Promise<boolean>
        onData: (id: string, callback: (payload: string) => void) => () => void
        onExit: (id: string, callback: (payload: AresTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<AresTerminalSession>
        write: (id: string, data: string) => Promise<boolean>
      }
      onClosePreviewRequested?: (callback: () => void) => () => void
      onOpenUpdatesRequested?: (callback: () => void) => () => void
      onWindowStateChanged?: (callback: (payload: AresWindowState) => void) => () => void
      onPreviewFileChanged: (callback: (payload: AresPreviewFileChanged) => void) => () => void
...
export interface AresTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface AresTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `ares update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  aresRoot?: string
}
...
export interface AresConnection {
  baseUrl: string
...
export interface AresTitleBarTheme {
  background: string
  foreground: string
}

export interface AresWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface AresApiRequest {
  path: string
  method?: string
  body?: unknown
  timeoutMs?: number
...
export interface AresPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface AresReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface AresPreviewWatch {
  id: string
  path: string
}

export interface AresReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface AresReadDirResult {
  entries: AresReadDirEntry[]
  error?: string
}

export interface AresPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface AresSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
