...
export const ja = defineLocale({
  ...
  boot: {
    ...
    steps: {
      ...
      startingAresDesktop: 'Ares Desktop を起動中…'
    },
    errors: {
  ...
  settings: {
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone: 'Ares がローカル時刻のコンテキストを必要とするときに使用します。空欄ならシステムのタイムゾーンを使います。',
      agent: {
    ...
    gateway: {
      ...
      envOverrideDesc:
        '保存された設定を使用するには ARES_DESKTOP_REMOTE_URL と ARES_DESKTOP_REMOTE_TOKEN の設定を解除してください。',
      localTitle: 'ローカルゲートウェイ',
      localDesc: 'ローカルホストでプライベートな Ares バックエンドを起動します。これがデフォルトで、オフラインでも動作します。',
      ...
      remoteUrlDesc: 'リモートダッシュボードバックエンドのベース URL。/ares などのパスプレフィックスもサポートしています。',
  ...
  commandCenter: {
    ...
    aresActiveSessions: (version, count) => `Ares ${version} · アクティブセッション ${count}`,
  ...
  messaging: {
    ...
    fieldCopy: {
      ...
      MATRIX_USER_ID: { label: 'ボットユーザー ID', placeholder: '@ares:example.org' },
  ...
  updates: {
    ...
    applyingBody: 'Ares アップデーターが独自のウィンドウで引き継ぎ、完了後に Ares を再度開きます。',
    applyingBodyBackend: 'リモートバックエンドが更新を適用して再起動します。復帰すると Ares が自動的に再接続します。',
    applyingClose: 'Ares は更新を適用するために閉じます。',
  ...
  onboarding: {
    ...
    flowSubtitles: {
      ...
      loopback: 'サインインのためブラウザーを開きます — Ares が自動接続します',
  ...
  assistant: {
    ...
    approval: {
      ...
      alwaysDescription: pattern =>
        `これにより "${pattern}" パターンが永続的な許可リスト (~/.ares/config.yaml) に追加されます。Ares はこのセッションや将来のセッションで、このようなコマンドについて再度尋ねません。`,
      alwaysAllow: '常に許可'
    },
    clarify: {
