...
import { getAresConfigRecord, type AresConfigRecord, saveAresConfig } from '@/ares'
...
export interface I18nConfigClient {
  getConfig: () => Promise<AresConfigRecord>
  saveConfig: (config: AresConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.aresDesktop?.api) {
      return Promise.resolve({})
    }

    return getAresConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.aresDesktop?.api) {
...
export function getConfigDisplayLanguage(config: AresConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: AresConfigRecord, locale: Locale): AresConfigRecord {
