...
export const zh: Translations = {
  ...
  boot: {
    ...
    steps: {
      ...
      startingAresDesktop: '正在启动 Ares 桌面版…'
    },
    errors: {
  ...
  settings: {
    ...
    gateway: {
      ...
      envOverrideDesc: '取消设置 ARES_DESKTOP_REMOTE_URL 和 ARES_DESKTOP_REMOTE_TOKEN 后才会使用下面保存的设置。',
      localTitle: '本地网关',
      localDesc: '在 localhost 启动私有 Ares 后端。这是默认方式，并且可离线工作。',
      remoteTitle: '远程网关',
      remoteDesc:
        '将此桌面外壳连接到远程 Ares 后端。托管网关使用 OAuth 或用户名密码；自托管网关也可能使用会话 token。',
      remoteUrlTitle: '远程 URL',
      remoteUrlDesc: '远程 dashboard 后端的基础 URL。支持路径前缀，例如 /ares。',
      ...
      savedTitle: '网关设置已保存',
      restartingMessage: 'Ares Desktop 将使用已保存设置重新连接。',
  ...
  commandCenter: {
    ...
    aresActiveSessions: (version, count) => `Ares ${version} · 活跃会话 ${count}`,
  ...
  messaging: {
    ...
    fieldCopy: {
      ...
      MATRIX_USER_ID: { label: 'Bot 用户 ID', placeholder: '@ares:example.org' },
    ...
    platformIntro: {
      ...
      bluebubbles: '在装有 iMessage 的 Mac 上运行 BlueBubbles Server，暴露其 API，然后用服务器密码把 Ares 指向该 URL。',
      ...
      weixin: '登录微信公众平台，复制 AppID 和 Token，并把消息回调 URL 指向 Ares。',
  ...
  updates: {
    ...
    applyingBody: 'Ares 更新器会在自己的窗口中接管，并在完成后重新打开 Ares。',
    applyingBodyBackend: '远程后端正在应用更新并将重启。恢复后 Ares 会自动重新连接。',
    applyingClose: 'Ares 将关闭以应用更新。',
    errorTitle: '更新未完成',
  ...
  assistant: {
    ...
    approval: {
      ...
      alwaysDescription: pattern =>
        `这会将“${pattern}”模式加入永久允许列表 (~/.ares/config.yaml)。Ares 对类似命令将不再询问，包括当前会话和未来会话。`,
      alwaysAllow: '始终允许'
    },
    clarify: {
      notReady: '澄清请求尚未就绪',
