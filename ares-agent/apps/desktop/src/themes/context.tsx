...
const SKIN_KEY = 'ares-desktop-theme-v2'
const MODE_KEY = 'ares-desktop-mode-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.aresTheme = skinName
  root.dataset.aresMode = rendered
  ...
  window.aresDesktop?.setTitleBarTheme?.({
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.aresThemeFont = 'true'
...
/** Sync the desktop skin with the active Ares backend theme on connect. */
