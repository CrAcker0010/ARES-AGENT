...
import { formatRefValue, aresDirectiveFormatter } from './directive-text'
...
describe('aresDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = aresDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = aresDirectiveFormatter.parse('@file:src/main.tsx the entry point')
