...
const ARES_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type AresRefType = (typeof ARES_REF_TYPES)[number]
...
const ICON_PATHS: Record<AresRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as AresRefType] ?? ICON_FALLBACK
...
const ARES_DIRECTIVE_RE = new RegExp(
...
export const aresDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(ARES_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as AresRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: AresRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
/**
 * Renders text containing Ares directives (`@file:...`, `@image:...`) as
...
export function DirectiveContent({ text }: { text: string }) {
  ...
  const segments = useMemo(() => aresDirectiveFormatter.parse(cleanedText), [cleanedText])
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.aresDesktop && isRemoteGateway()
        ? gatewayMediaDataUrl(id)
        : window.aresDesktop?.readFileDataUrl(id)
