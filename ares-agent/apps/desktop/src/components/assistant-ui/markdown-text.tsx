...
async function mediaSrc(path: string): Promise<string> {
  ...
  if (window.aresDesktop && ['audio', 'video'].includes(mediaKind(path))) {
  ...
  if (window.aresDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.aresDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.aresDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
function OpenMediaButton({ kind, path }: { kind: 'audio' | 'video'; path: string }) {
  return (
    <button
      ...
      onClick={() => void window.aresDesktop?.openExternal(mediaExternalUrl(path))}
      type="button"
    >
      Open {kind} file
    </button>
