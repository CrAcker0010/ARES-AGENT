...
export type AresConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `ares model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
export interface ToolProvider {
   ...
   *  CLI `ares tools` active-provider detection). */
  is_active: boolean
}
...
/** Shape of `GET /api/ares/update/check` — the backend's own update state.
