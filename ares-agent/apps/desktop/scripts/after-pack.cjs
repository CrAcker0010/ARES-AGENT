 ...
 *
 * Stamps the Ares icon + identity onto the packed Windows Ares.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `ares desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Ares')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Ares'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Ares.exe keeps the stock Electron icon`)
  }
}
