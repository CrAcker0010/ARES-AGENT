...
// no longer bundles the Ares Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Ares.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Ares'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Ares.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'ares'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default ARES_HOME for non-sandboxed runs -- matches main.cjs's
// resolveAresHome(). On Windows it's %LOCALAPPDATA%\ares; elsewhere
// it's ~/.ares. The fresh-install sandbox launchFresh() sets its own
// ARES_HOME and never touches this.
const DEFAULT_ARES_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'ares')
  }
  return path.join(os.homedir(), '.ares')
})()
const VENV_ROOT = path.join(DEFAULT_ARES_HOME, 'ares-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'ares-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.ARES_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Ares-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Ares-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Ares-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Ares-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.ARES_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.ARES_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const aresHome = path.join(sandbox, 'ares-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(aresHome, { recursive: true })
  ...
  env.ARES_DESKTOP_CWD = cwd
  env.ARES_DESKTOP_IGNORE_EXISTING = '1'
  env.ARES_DESKTOP_TEST_MODE = 'fresh-install'
  env.ARES_DESKTOP_USER_DATA_DIR = userDataDir
  env.ARES_HOME = aresHome
  delete env.ARES_DESKTOP_ARES
  delete env.ARES_DESKTOP_ARES_ROOT
  ...
  console.log(`  ARES_HOME: ${aresHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(aresHome, 'ares-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Ares Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of ares_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'ares-agent', 'ares_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Ares
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + ARES_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  ARES_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
