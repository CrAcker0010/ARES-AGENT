#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Ares icon + version metadata onto the
// built Ares.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Ares.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `ares desktop`, the installer's
...
// path (which rebuilds via `ares desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.cjs <path-to-Ares.exe>
//
...
// Stamp the Ares icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  console.log('[set-exe-identity] done — Ares icon + identity stamped')
}
