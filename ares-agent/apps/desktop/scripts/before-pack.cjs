 ...
 * product name (`Ares`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Ares'
 *
 * This is a hard failure with no obvious cause for the user — `ares desktop`
 ...
 * (the mac-unpacked Ares.app bundle) and Windows (win-unpacked), so we
