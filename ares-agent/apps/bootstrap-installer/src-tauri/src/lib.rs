...
/// desktop app handing off via `Ares-Setup.exe --update`).
///
...
pub fn run() {
    // Tracing → bootstrap-installer.log under ARES_HOME/logs/ so install
    ...
    // Ares is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "Ares installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Ares is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "Ares" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "Ares" shortcuts that install.ps1 creates, and a
            // reliable detached relaunch there needs the DETACHED_PROCESS +
            // startup-grace handling used by launch_ares_desktop — out of
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                let install_root = paths::ares_home().join("ares-agent");
                if bootstrap::ares_is_installed(&install_root) {
                    match bootstrap::spawn_installed_desktop(&install_root) {
                        Ok(()) => {
                            // Brief grace so the spawned app is registered
                            // before we exit (mirrors launch_ares_desktop).
                            std::thread::sleep(std::time::Duration::from_millis(200));
                            tracing::info!(
                                "ares already installed — relaunched desktop; exiting installer"
                            );
                            app.handle().exit(0);
                            return Ok(());
                        }
        ...
        .expect("error while running Ares Setup");
}
