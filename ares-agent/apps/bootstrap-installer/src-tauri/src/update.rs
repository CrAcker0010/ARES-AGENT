...
//!
//! Driven when the installer is launched as `Ares-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Ares desktop process to fully exit (so the venv
//!      shim is free; otherwise `ares update` aborts with exit code 2),
//!   2. run `ares update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in ares_cli/main.py),
//!   3. run `ares desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `ares update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_ares) and
...
/// `ares update` exit code meaning "another ares process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// ares_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release the venv shim
/// before giving up and letting `ares update`'s own guard decide.
...
pub async fn start_update(app: AppHandle) -> Result<(), String> {
        ...
        .is_err()
    {
        ...
        let mut stages = vec![
            stage_info("update", "Updating Ares"),
...
async fn run_update(app: AppHandle) -> Result<()> {
    let ares_home = crate::paths::ares_home();
    let install_root = ares_home.join("ares-agent");
    ...
    let ares = resolve_ares(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the ares CLI under {}. Is Ares installed? \
    ...
    // async on Windows. If it still holds the venv shim, `ares update`
    // aborts with exit 2. Give it a bounded window to clear.
    wait_for_venv_free(&install_root, &app).await;

    // ---- stage 1: ares update -----------------------------------------
    // Pass --branch so `ares update` targets the branch this installer was
    ...
    // without --branch, `ares update` switches the checkout to `main` (a
    ...
    // --force skips `ares update`'s Windows running-exe guard (which would
    ...
    // time `ares update` runs there is no legitimate ares.exe to protect,
    // and the guard would only produce a false "Ares is still running" stop.
    ...
    // Retry-once for the update-boundary crash. `ares update` lazily imports
    ...
    // second `ares update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Ares" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Ares is still running. Close all Ares windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "ares update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::ares_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 2: ares desktop --build-only ----------------------------
    // `ares update` deliberately does NOT build apps/desktop (it installs
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `ares desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Ares manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_ares_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
async fn wait_for_venv_free(install_root: &Path, app: &AppHandle) {
    let shim = venv_ares(install_root);
    let deadline = Instant::now() + DESKTOP_EXIT_WAIT;

    emit_log(app, Some("update"), LogStream::Stdout, "[update] waiting for Ares to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend ares.exe (or a grandchild it spawned)
            ...
            // every ares.exe except ourselves, then give the OS a beat to
            // unload the image.
            emit_log(
                app,
                Some("update"),
                LogStream::Stdout,
                "[update] Ares still holding the venv shim; force-killing stragglers…",
            );
            force_kill_other_ares();
...
/// Force-kill any `ares.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `ares.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside `wait_for_venv_free`,
/// which completes BEFORE we spawn `venv\Scripts\ares.exe update`. At this
/// point no update-driven ares.exe exists yet, so the only ares.exe images
...
/// Spawn `ares <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv ares shim under an install root, regardless of existence.
fn venv_ares(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("ares.exe")
    } else {
        install_root.join("venv").join("bin").join("ares")
    }
}

/// Resolve the ares CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `ares` on PATH.
fn resolve_ares(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_ares(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "ares.exe" } else { "ares" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    ...
    let mut envs = vec![(
        "ARES_HOME".to_string(),
        ares_home.as_os_str().to_os_string(),
    )];
    if let Some(path) = path_with_prepended_entries(&[
        ares_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_ares_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Ares.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.ares-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.ares-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::ares_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_ares_is_under_install_root() {
        let root = Path::new("/x/ares-agent");
        let shim = venv_ares(root);
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Ares.app"]),
            Some(PathBuf::from("/Applications/Ares.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Ares.app");
        let tmp = base.join("Ares.app.ares-update-new");
        let old = base.join("Ares.app.ares-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Ares.app");
        let tmp = base.join("Ares.app.ares-update-new"); // intentionally absent
        let old = base.join("Ares.app.ares-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Ares.app");
        let tmp = base.join("Ares.app.ares-update-new"); // absent
        let old = base.join("Ares.app.ares-update-old");
        write_marker(&target, "OLD");
