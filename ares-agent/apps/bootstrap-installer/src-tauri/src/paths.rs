...
//!
//! Mirrors `ares_constants.get_ares_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\ares
//!   macOS:   ~/.ares
//!   Linux:   ~/.ares  (override via $ARES_HOME)
//!
//! NOTE (macOS): Python's get_ares_home(), scripts/install.sh, and the
//! Electron desktop's resolveAresHome() ALL use ~/.ares on macOS — there
...
/// Returns the canonical Ares home directory, respecting $ARES_HOME if set.
pub fn ares_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("ARES_HOME") {
    ...
    {
        // %LOCALAPPDATA%\ares — matches scripts/install.ps1's $AresHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("ares");
        }
    }

    // macOS + Linux + fallback: ~/.ares (matches Python get_ares_home(),
    // install.sh, and the Electron desktop's resolveAresHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".ares");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    ares_home().join("bootstrap-cache")
}
...
/// ARES_HOME so it survives repo checkout deletion (unlike anything under
/// ares-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\ares\ares-setup.exe`; on other
...
pub fn copy_self_to_ares_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to ARES_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_ARES_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".ares-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under ARES_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[ares-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("ARES_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_ares_home() -> String {
    ares_home().to_string_lossy().into_owned()
}
