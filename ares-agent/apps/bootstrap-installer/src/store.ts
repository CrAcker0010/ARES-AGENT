...
export const $aresHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  try {
    const [logPath, aresHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_ares_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $aresHome.set(aresHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Ares" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  // Update is driven by the desktop handing off (Ares-Setup.exe --update);
...
export async function launchAresDesktop(): Promise<void> {
  const installRoot = $bootstrap.get().installRoot
  if (!installRoot) throw new Error('no install root')
  await invoke('launch_ares_desktop', { installRoot })
}
