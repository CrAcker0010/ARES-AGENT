...
version: 1.1.1
author: xdevplatform + openclaw + Ares Agent
license: MIT
...
- The user must fill `~/.xurl` with secrets manually on their own machine. In Docker, this must be the `~` seen by Ares tool subprocesses; see the Docker note below.
...
> **Docker HOME pitfall:** In the official Ares Docker layout, `/opt/data` is `ARES_HOME`, but Ares tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for Ares-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
...
> ```
> If `HOME=/opt/data xurl auth status` succeeds but `HOME=/opt/data/home xurl auth status` shows no apps or tokens, Ares tool calls will not see the credentials.
...
- **Token storage:** `~/.xurl` is YAML. In Docker, use the Ares subprocess HOME (`/opt/data/home` in the official image) so tokens land under `/opt/data/home/.xurl`. Never read or send this file to LLM context.
...
- Ares adaptation: reformatted for Ares skill conventions; safety guardrails preserved verbatim.
