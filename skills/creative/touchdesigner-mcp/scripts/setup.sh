...
ARES_HOME_DIR="${ARES_HOME:-$HOME/.ares}"
ARES_CFG="${ARES_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Ares config has twozero_td MCP entry ──
if [[ ! -f "$ARES_CFG" ]]; then
    echo -e " ${FAIL} Ares config not found at ${ARES_CFG}"
    manual_steps+=("Create ${ARES_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ARES_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Ares config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Ares config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ARES_CFG} manually"); }
    manual_steps+=("Restart Ares session to pick up config change")
fi
