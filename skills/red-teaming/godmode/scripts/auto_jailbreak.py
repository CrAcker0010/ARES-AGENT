...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ARES_HOME", os.path.expanduser("~/.ares")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ARES_HOME", Path.home() / ".ares")) / "skills" / "red-teaming" / "godmode"
...
ARES_HOME = Path(os.getenv("ARES_HOME", Path.home() / ".ares"))
CONFIG_PATH = ARES_HOME / "config.yaml"
PREFILL_PATH = ARES_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Ares models — already uncensored, just needs clean prompt
    "ares": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "ares" in model_lower or "nous" in model_lower:
        return "ares"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Ares config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.ares/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Ares for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Ares for changes to take effect.")
